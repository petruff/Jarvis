...
const EXCLUDE_PATTERNS = [
  ...
  /docs\/standards\/Electron AAOS-LIVRO-DE-OURO/,
  /docs\/standards\/Electron AAOS-FRAMEWORK-MASTER\.md$/,
...
async function generateManifest() {
  ...
  if (!fs.existsSync(electron-aaosCoreDir)) {
    throw new Error(`.electron-aaos-core directory not found at ${electron-aaosCoreDir}`);
  }
  ...
  console.log(`Generating manifest for electron-aaos-core v${version}...`);

  const allFiles = [];

  // Scan folders
  for (const folder of FOLDERS_TO_COPY) {
    const folderPath = path.join(electron-aaosCoreDir, folder);
    if (fs.existsSync(folderPath)) {
      scanDirectory(folderPath, electron-aaosCoreDir, allFiles);
    }
  }

  // Add root files
  for (const file of ROOT_FILES_TO_COPY) {
    const filePath = path.join(electron-aaosCoreDir, file);
  ...
  for (const fullPath of allFiles) {
    try {
      const relativePath = path.relative(electron-aaosCoreDir, fullPath).replace(/\\/g, '/');
...
async function writeManifest(manifest) {
  const manifestPath = path.join(__dirname, '..', '.electron-aaos-core', 'install-manifest.yaml');
  ...
  // Add header comment
  const header = `# Electron AAOS-Core Install Manifest
...
async function main() {
  try {
    ...
    console.log('Electron AAOS-Core Install Manifest Generator');
    console.log('='.repeat(60));
    console.log('');
