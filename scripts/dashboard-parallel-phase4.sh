#!/bin/bash
# =============================================================================
# Electron AAOS Dashboard Parallel Dev - Phase 4 (5 workers - MAX)
# =============================================================================
...
SESSION_NAME="electron-aaos-dash-p4"
...
declare -a STORIES=(
    "1.3|Drag & Drop|docs/stories/electron-aaos-dashboard/epic-1-story-board.md|apps/dashboard/src/components/kanban/|Add @dnd-kit to KanbanBoard, enable drag between columns, persist order"
    "2.1|Terminal|docs/prd/electron-aaos-dashboard.md|apps/dashboard/src/components/terminal/|Create Terminal.tsx read-only output with ANSI colors"
    "3.1|Roadmap|docs/prd/electron-aaos-dashboard.md|apps/dashboard/src/app/(dashboard)/roadmap/|Create timeline view showing epics and milestones"
    "3.2|Context|docs/prd/electron-aaos-dashboard.md|apps/dashboard/src/app/(dashboard)/context/|Create file browser for project context files"
    "4.1|GitHub|docs/prd/electron-aaos-dashboard.md|apps/dashboard/src/app/(dashboard)/github/|Create GitHub view listing issues and PRs"
)
...
echo ""
echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║     ${YELLOW}Electron AAOS Dashboard Phase 4 (MAX PARALLELISM)${NC}             ${CYAN}║${NC}"
echo -e "${CYAN}╠═══════════════════════════════════════════════════════════╣${NC}"
