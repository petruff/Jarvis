# [P0] Missing Electron AAOS Core module: utils/repository-detector - Blocks Linux Installation
...
The Electron AAOS installer fails immediately on Linux systems with a module not found error, preventing any installation or testing. The installer cannot locate the Electron AAOS Core module `utils/repository-detector`, blocking all Linux platform validation for Story 1.10c.
...
```
Error: Cannot find Electron AAOS Core module: utils/repository-detector
Searched: ${PROJECT_ROOT}/.electron-aaos-core/utils/repository-detector
Please ensure @electron-aaos/electron-aaos-core is installed correctly.
    at loadElectron AAOSCore (${PROJECT_ROOT}/bin/electron-aaos-init.js:43:11)
```

### Stack Trace Location

- **File:** `bin/electron-aaos-init.js`
- **Line:** 43
- **Function:** `loadElectron AAOSCore`
...
- Node.js v18.20.8+
- Electron AAOS-Fullstack repository cloned
...
2. Create test directory:
   ...
   ```bash
   mkdir -p /tmp/electron-aaos-test-install
   cd /tmp/electron-aaos-test-install
   ```

3. Execute installer:

   ```bash
   node ${PROJECT_ROOT}/bin/electron-aaos-init.js --help
   ```
...
- Installer should locate Electron AAOS Core module successfully
...
2. **Missing File/Directory**
   - `.electron-aaos-core/utils/repository-detector.js` may not exist in repository
...
- [ ] Verify `.electron-aaos-core/utils/repository-detector.js` exists in repo
- [ ] Check `bin/electron-aaos-init.js:43` for path construction logic
...
1. **Verify module exists:**
   ...
   ```bash
   ls -la .electron-aaos-core/utils/repository-detector.js
   ```

2. **Fix path resolution in `bin/electron-aaos-init.js:43`:**

   ```javascript
   // Before (suspected):
   const detector = require('./.electron-aaos-core/utils/repository-detector');

   // After (cross-platform):
   const path = require('path');
   const detectorPath = path.resolve(__dirname, '..', '.electron-aaos-core', 'utils', 'repository-detector');
...
3. **Add file existence check:**
   ...
   if (!fs.existsSync(detectorPath + '.js')) {
     console.error('Electron AAOS Core module not found.');
...
4. **Test in WSL Ubuntu:**
   ```bash
   wsl bash -c "cd /tmp/electron-aaos-test-install && node /mnt/c/.../bin/electron-aaos-init.js --help"
   ```
...
- **Installer:** `bin/electron-aaos-init.js:43` (error location)
- **Missing Module:** `.electron-aaos-core/utils/repository-detector` (expected location)
