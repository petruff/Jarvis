...
All notable changes to Electron AAOS Electron AAOS will be documented in this file.
...
This release marks the **Open-Source Community Readiness** milestone, preparing Electron AAOS for public contribution while introducing the **Squad System** for extensibility.
...
- **Rebranding to Electron AAOS** (OSR-9)
  - Brand investigation complete
  - Namespace updated to Electron AAOSAI
...
- **Scripts Path Consolidation** (6.16)
  - Standardized script locations under `.electron-aaos-core/scripts/`
...
- **Agent Command Rationalization** (Story 6.1.2.3)
  - Command consolidation: `electron-aaos-master` 44→30 commands (32% reduction)
...
- **Scripts Location** - Consolidated under `.electron-aaos-core/scripts/` for consistency
...
### Removed
- **Private expansion packs** - Moved to separate private repository (`electron-aaos-expansion-packs`)
  - Removed `expansion-packs/creator/` (CreatorOS)
  - Removed `expansion-packs/innerlens/`
  - Removed `expansion-packs/mmos-mapper/`
  - Removed `expansion-packs/electron-aaos-infrastructure-devops/`
  - Removed `expansion-packs/meeting-notes/`
  - Repository: https://github.com/Electron AAOSAI/electron-aaos-expansion-packs (PRIVATE)
- **Internal development tools** - Moved to separate private repository (`electron-aaos-dev-tools`)
  ...
  - Repository: https://github.com/Electron AAOSAI/electron-aaos-dev-tools (PRIVATE)
- **hybrid-ops expansion pack** - Moved to separate repository for independent maintenance
  ...
  - Repository: https://github.com/Electron AAOSAI/electron-aaos-hybrid-ops-pedro-valerio
...
- Breaking Change: hybrid-ops no longer bundled with @electron-aaos/electron-aaos-core
...
- PRIMARY detection layer in `tools/electron-aaos-npx-wrapper.js` using `__dirname`
- SECONDARY fallback detection in `tools/installer/bin/electron-aaos.js` using `process.cwd()`
...
- Prevents confusion when running `npx @electron-aaos/electron-aaos-core install` from home directory

### Changed
- Early exit with `process.exit(1)` when NPX temporary context detected
- Help message provides actionable solution: `cd /path/to/your/project && npx @electron-aaos/electron-aaos-core install`
