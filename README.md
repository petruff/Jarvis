# Electron AAOS Electron AAOS: Framework Universal de Agentes IA 🚀

[![Versão NPM](https://img.shields.io/npm/v/electron-aaos-core.svg)](https://www.npmjs.com/package/electron-aaos-core)
[![Licença: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Versão Node.js](https://img.shields.io/badge/node-%3E%3D18.0.0-brightgreen.svg)](https://nodejs.org/)
[![CI](https://github.com/Electron AAOSAI/electron-aaos-core/actions/workflows/ci.yml/badge.svg)](https://github.com/Electron AAOSAI/electron-aaos-core/actions/workflows/ci.yml)
[![codecov](https://codecov.io/gh/Electron AAOSAI/electron-aaos-core/branch/main/graph/badge.svg)](https://codecov.io/gh/Electron AAOSAI/electron-aaos-core)
[![Documentação](https://img.shields.io/badge/docs-disponível-orange.svg)](https://electron-aaos.ai)
...
O Electron AAOS Electron AAOS segue uma hierarquia clara de prioridades:
...
**As Duas Inovações Chave do Electron AAOS Electron AAOS:**
...
### Entendendo o Fluxo de Trabalho Electron AAOS

**Antes de mergulhar, revise estes diagramas críticos de fluxo de trabalho que explicam como o Electron AAOS funciona:**
...
> ⚠️ **Estes diagramas explicam 90% da confusão sobre o fluxo Electron AAOS Electron AAOS Agentic Agile** - Entender a criação de PRD+Arquitetura e o fluxo de trabalho sm/dev/qa e como os agentes passam notas através de arquivos de histórias é essencial - e também explica por que isto NÃO é taskmaster ou apenas um simples executor de tarefas!

### O que você gostaria de fazer?

- **[Instalar e Construir software com Equipe Ágil Full Stack de IA](#início-rápido)** → Instruções de Início Rápido
- **[Aprender como usar o Electron AAOS](docs/guides/user-guide.md)** → Guia completo do usuário e passo a passo
...
## Importante: Mantenha Sua Instalação Electron AAOS Atualizada

**Mantenha-se atualizado sem esforço!** Para atualizar sua instalação Electron AAOS existente:

```bash
npx github:Electron AAOSAI/electron-aaos-core install
```
...
**Instale o Electron AAOS Electron AAOS com um único comando:**

```bash
# Criar um novo projeto com assistente interativo moderno
npx electron-aaos-core init meu-projeto
...
npx electron-aaos-core@latest init meu-projeto
```

### ✨ Assistente de Instalação Moderno

O Electron AAOS Electron AAOS agora inclui uma experiência de instalação interativa de última geração, inspirada em ferramentas modernas como Vite e Next.js:
...
- ✅ Configuração de todos os agentes e fluxos de trabalho Electron AAOS
...
Se você já tem o Electron AAOS instalado:
...
### Configure Seu IDE para Desenvolvimento Electron AAOS

O Electron AAOS Electron AAOS inclui regras pré-configuradas para IDE para melhorar sua experiência de desenvolvimento:
...
- 📝 Padrões de código específicos do Electron AAOS

### Início Mais Rápido com Interface Web (2 minutos)

1. **Instale o Electron AAOS**: Execute `npx electron-aaos-core init meu-projeto`
...
4. **Use comandos Electron AAOS**: Digite `*help` para ver comandos disponíveis
5. **Siga o fluxo**: Veja o [Guia do usuário](docs/guides/user-guide.md) para mais detalhes

### Referência de Comandos CLI

O Electron AAOS Electron AAOS oferece uma CLI moderna e cross-platform com comandos intuitivos:

```bash
# Gerenciamento de Projeto (com assistente interativo)
npx electron-aaos-core init <nome-projeto> [opções]
...
npx electron-aaos-core install [opções]
...
# Comandos do Sistema
npx electron-aaos-core --version   Exibir versão instalada
npx electron-aaos-core --help      Exibir ajuda detalhada
npx electron-aaos-core info        Exibir informações do sistema
npx electron-aaos-core doctor      Executar diagnósticos do sistema
npx electron-aaos-core doctor --fix Corrigir problemas detectados automaticamente

# Manutenção
npx electron-aaos-core update      Atualizar para versão mais recente
npx electron-aaos-core uninstall   Remover Electron AAOS Electron AAOS
```
...
🚀 Electron AAOS Electron AAOS Installation
...
Next steps:
  ...
  electron-aaos-core doctor     # Verify installation
  electron-aaos-core --help     # See available commands
```
...
✔ Electron AAOS Electron AAOS installed successfully
```
...
[DRY RUN] Would copy: .electron-aaos-core/ (45 files)
...
🏥 Electron AAOS System Diagnostics

✔ Node.js version: v20.10.0 (meets requirement: >=18.0.0)
✔ npm version: 10.2.3
✔ Git installed: version 2.43.0
✔ GitHub CLI: gh 2.40.1
✔ Electron AAOS Electron AAOS: v1.0.0

Configuration:
✔ .electron-aaos-core/ directory exists
...
```bash
$ npx electron-aaos-core --help

Usage: electron-aaos-core [options] [command]

Electron AAOS Electron AAOS: AI-Orchestrated System for Full Stack Development
...
Commands:
  init <project-name>          Create new Electron AAOS project with interactive wizard
  install [options]            Install Electron AAOS in current directory
  info                         Display system information
  doctor [options]             Run system diagnostics and health checks
  help [command]               display help for command

Run 'electron-aaos-core <command> --help' for detailed information about each command.
```
...
```bash
# Instalar Electron AAOS com configuração GitHub
npx github:Electron AAOSAI/electron-aaos-core setup

# Isto vai:
# 1. Verificar/instalar GitHub CLI
# 2. Autenticar com GitHub
# 3. Executar o instalador Electron AAOS
```

## 🌟 Além do Desenvolvimento de Software - Squads

O framework de linguagem natural do Electron AAOS funciona em QUALQUER domínio. Os Squads fornecem agentes IA especializados para escrita criativa, estratégia de negócios, saúde e bem-estar, educação e muito mais. Além disso, os Squads podem expandir o núcleo do Electron AAOS Electron AAOS com funcionalidade específica que não é genérica para todos os casos. [Veja o Guia de Squads](docs/guides/squads-guide.md) e aprenda a criar os seus próprios!

## Agentes Disponíveis

O Electron AAOS Electron AAOS vem com 11 agentes especializados:

### Agentes Meta

- **electron-aaos-master** - Agente mestre de orquestração (inclui capacidades de desenvolvimento de framework)
- **electron-aaos-orchestrator** - Orquestrador de fluxo de trabalho e coordenação de equipe
...
- 🚀 **[Guia de Squads](docs/guides/squads-guide.md)** - Estenda o Electron AAOS para qualquer domínio além do desenvolvimento de software
...
- 🎯 **[Princípios Orientadores](docs/GUIDING-PRINCIPLES.md)** - Filosofia e melhores práticas do Electron AAOS
- 🏛️ **[Visão Geral da Arquitetura](docs/architecture/ARCHITECTURE-INDEX.md)** - Visão detalhada da arquitetura do sistema
- ⚙️ **[Guia de Ajuste de Performance](docs/performance-tuning-guide.md)** - Otimize seu fluxo de trabalho Electron AAOS
...
## 🤖 Electron AAOS Autonomous Development Engine (ADE)

O Electron AAOS Electron AAOS v1.0.0 introduz o **Autonomous Development Engine (ADE)** - um sistema completo para desenvolvimento autônomo que transforma requisitos em código funcional.
...
Expansion packs permitem estender o Electron AAOS para qualquer domínio. Estrutura básica:
...
- **[hybrid-ops](https://github.com/Electron AAOSAI/electron-aaos-hybrid-ops-pedro-valerio)** - Operações híbridas humano-agente (repositório separado)

## Suporte & Comunidade

- 📖 [Guia da Comunidade](docs/community.md) - Como participar e contribuir
- 💬 [Discussões GitHub](https://github.com/Electron AAOSAI/electron-aaos-core/discussions) - Hub central da comunidade
...
O Electron AAOS Electron AAOS implementa um sistema de validação de múltiplas camadas para garantir qualidade do código e consistência:
...
# Validador Electron AAOS
node .electron-aaos-core/utils/electron-aaos-validator.js pre-commit   # Validação pre-commit
node .electron-aaos-core/utils/electron-aaos-validator.js pre-push     # Validação pre-push
node .electron-aaos-core/utils/electron-aaos-validator.js stories      # Validar todas stories
```
...
[![Contributors](https://contrib.rocks/image?repo=Electron AAOSAI/electron-aaos-core)](https://github.com/Electron AAOSAI/electron-aaos-core/graphs/contributors)

<sub>Construído com ❤️ para a comunidade de desenvolvimento assistido por IA</sub>

---

**[⬆ Voltar ao topo](#electron-aaos-electron-aaos-framework-universal-de-agentes-ia-)**
