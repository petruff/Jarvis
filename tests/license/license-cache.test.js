...
describe('license-cache', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-license-test-'));
  });
  ...
  describe('writeLicenseCache', () => {
    ...
    it('should create .electron-aaos directory if not exists', () => {
      const data = createTestCacheData();
      const electron-aaosDir = getAiosDir(testDir);

      expect(fs.existsSync(electron-aaosDir)).toBe(false);

      writeLicenseCache(data, testDir);

      expect(fs.existsSync(electron-aaosDir)).toBe(true);
    });
  ...
  describe('readLicenseCache', () => {
    ...
    it('should return null for corrupted JSON', () => {
      ...
      fs.mkdirSync(getAiosDir(testDir), { recursive: true });
  ...
  describe('Pending Deactivation (AC-7b)', () => {
    ...
    describe('markPendingDeactivationSynced', () => {
      ...
      it('should add syncedAt timestamp', () => {
        ...
        const filePath = path.join(getAiosDir(testDir), 'pending-deactivation.json');
