...
const {
  ...
  LEVELS,
  VALID_USER_PROFILES,
} = require('../../.electron-aaos-core/core/config/config-resolver');
const { globalConfigCache } = require('../../.electron-aaos-core/core/config/config-cache');
...
function createTempProject(files = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-config-test-'));
  const electron-aaosCoreDir = path.join(tmpDir, '.electron-aaos-core');
  fs.mkdirSync(electron-aaosCoreDir, { recursive: true });
...
describe('config-resolver', () => {
  ...
  describe('constants', () => {
    ...
    test('CONFIG_FILES.user points to ~/.electron-aaos/user-config.yaml', () => {
      const expected = path.join(os.homedir(), '.electron-aaos', 'user-config.yaml');
  ...
  describe('isLegacyMode', () => {
    test('returns true when core-config.yaml exists but framework-config.yaml does not', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/core-config.yaml': 'project:\n  name: legacy\n',
      });
    ...
    test('returns false when framework-config.yaml exists', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/core-config.yaml': 'project:\n  name: legacy\n',
        '.electron-aaos-core/framework-config.yaml': 'metadata:\n  version: "1.0"\n',
      });
  ...
  describe('getConfigAtLevel', () => {
    test('loads framework config at L1', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
      });

      try {
        const config = getConfigAtLevel(tmpDir, 'L1');
        expect(config).toBeTruthy();
        expect(config.metadata.framework_name).toBe('Electron AAOS-FullStack');
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
    ...
    test('loads project config at L2', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/project-config.yaml': { fixture: 'project-config.yaml' },
      });
  ...
  describe('loadLegacyConfig', () => {
    test('loads monolithic core-config.yaml', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/core-config.yaml': { fixture: 'legacy-core-config.yaml' },
      });
    ...
    test('includes deprecation warning', () => {
      ...
      const origEnv = process.env.Electron AAOS_SUPPRESS_DEPRECATION;

      try {
        delete process.env.Electron AAOS_SUPPRESS_DEPRECATION;
        ...
        if (origEnv === undefined) {
          ...
          process.env.Electron AAOS_SUPPRESS_DEPRECATION = origEnv;
        }
        cleanupTempDir(tmpDir);
      }
    });

    test('suppresses deprecation when Electron AAOS_SUPPRESS_DEPRECATION=true', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/core-config.yaml': { fixture: 'legacy-core-config.yaml' },
      });
      const origEnv = process.env.Electron AAOS_SUPPRESS_DEPRECATION;
      ...
      try {
        process.env.Electron AAOS_SUPPRESS_DEPRECATION = 'true';
  ...
  describe('loadLayeredConfig — 4-level resolution', () => {
    test('merges L1 and L2 correctly', () => {
      ...
      try {
        ...
        // L1 values present
        expect(result.config.metadata.framework_name).toBe('Electron AAOS-FullStack');
        // L2 overrides L1
    ...
    test('merges L1 + L2 + L4 correctly', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.electron-aaos-core/project-config.yaml': { fixture: 'project-config.yaml' },
        '.electron-aaos-core/local-config.yaml': { fixture: 'local-config.yaml' },
      });
    ...
    test('lints L1 and L2 for env patterns', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/framework-config.yaml': 'url: "${API_URL}"\n',
        '.electron-aaos-core/project-config.yaml': 'name: "clean"\n',
      });
  ...
  describe('resolveConfig — main entry point', () => {
    ...
    test('interpolates env vars after merge', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/framework-config.yaml': 'metadata:\n  name: "test"\n',
        '.electron-aaos-core/local-config.yaml': 'api_url: "${TEST_API_URL:-http://localhost}"\n',
      });
  ...
  describe('L5 User layer', () => {
    ...
    beforeEach(() => {
      ...
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-user-config-test-'));
    ...
    test('L5 overrides L4 values', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.electron-aaos-core/local-config.yaml': 'user_profile: "advanced"\n',
      });
  ...
  describe('setUserConfigValue', () => {
    ...
    beforeEach(() => {
      originalUserConfigPath = CONFIG_FILES.user;
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-user-write-test-'));
      CONFIG_FILES.user = path.join(tempUserDir, 'user-config.yaml');
      globalConfigCache.clear();
    });
  ...
  describe('toggleUserProfile', () => {
    ...
    beforeEach(() => {
      originalUserConfigPath = CONFIG_FILES.user;
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-toggle-test-'));
      CONFIG_FILES.user = path.join(tempUserDir, 'user-config.yaml');
      globalConfigCache.clear();
    });
  ...
  describe('integration — L5 with full hierarchy', () => {
    ...
    beforeEach(() => {
      originalUserConfigPath = CONFIG_FILES.user;
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-l5-integration-test-'));
      CONFIG_FILES.user = path.join(tempUserDir, 'user-config.yaml');
      globalConfigCache.clear();
    });
    ...
    test('resolveConfig returns user_profile from L5 overriding lower levels', () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.electron-aaos-core/project-config.yaml': 'user_profile: "advanced"\nproject:\n  name: test\n',
      });
