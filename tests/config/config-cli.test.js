/**
 * CLI tests for `electron-aaos config` subcommands
...
const { createConfigCommand } = require('../../.electron-aaos-core/cli/commands/config');
...
function createTempProject(files = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-cli-test-'));
...
describe('config CLI commands', () => {
  ...
  /**
   * Run `electron-aaos config <subArgs>` in-process via Commander.
  ...
  async function runConfigCmd(subArgs) {
    ...
    let exitCode = 0;
    try {
      await program.parseAsync(['node', 'electron-aaos', ...subArgs]);
    } catch (err) {
  ...
  describe('electron-aaos config show', () => {
    test('shows resolved config as YAML', async () => {
      ...
      try {
        ...
        expect(stdout).toContain('Electron AAOS-FullStack');
      } finally {
  ...
  describe('electron-aaos config validate', () => {
  ...
  // -----------------------------------------------------------------------
  // electron-aaos config diff
  // -----------------------------------------------------------------------

  describe('electron-aaos config diff', () => {
  ...
  // -----------------------------------------------------------------------
  // electron-aaos config migrate
  // -----------------------------------------------------------------------

  describe('electron-aaos config migrate', () => {
    const LEGACY_CONFIG = [
      'project:',
      '  name: "test-project"',
      '  version: "1.0.0"',
      ...
      'mcp:',
      '  enabled: false',
      'toolsLocation: .electron-aaos-core/tools',
      'lazyLoading:',
      '  enabled: true',
      '',
    ].join('\n');
    ...
    test('--dry-run shows preview without writing files', async () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/core-config.yaml': LEGACY_CONFIG,
      });
      ...
      try {
        ...
        expect(fs.existsSync(path.join(tmpDir, '.electron-aaos-core', 'framework-config.yaml'))).toBe(false);
        expect(fs.existsSync(path.join(tmpDir, '.electron-aaos-core', 'project-config.yaml'))).toBe(false);
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
    ...
    test('full migration creates split files and backup', async () => {
      ...
      try {
        ...
        // Split files created
        expect(fs.existsSync(path.join(tmpDir, '.electron-aaos-core', 'framework-config.yaml'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.electron-aaos-core', 'project-config.yaml'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.electron-aaos-core', 'local-config.yaml'))).toBe(true);

        // Backup created
        expect(fs.existsSync(path.join(tmpDir, '.electron-aaos-core', 'core-config.yaml.backup'))).toBe(true);
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
  ...
  // -----------------------------------------------------------------------
  // electron-aaos config validate — error paths
  // -----------------------------------------------------------------------

  describe('electron-aaos config validate — error paths', () => {
  ...
  describe('electron-aaos config init-local', () => {
    test('creates local-config.yaml from template', async () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/local-config.yaml.template': 'ide:\n  selected:\n    - vscode\n',
      });
      ...
      try {
        ...
        const localConfig = path.join(tmpDir, '.electron-aaos-core', 'local-config.yaml');
    ...
    test('warns if local-config.yaml already exists', async () => {
      const tmpDir = createTempProject({
        '.electron-aaos-core/local-config.yaml.template': 'ide:\n  selected:\n    - vscode\n',
        '.electron-aaos-core/local-config.yaml': 'existing: true\n',
      });
