...
const PKG_DIR = path.resolve(__dirname, '../../../packages/electron-aaos-install');
...
describe('Integration - Task 8.3: Local NPX Execution', () => {
  describe('Package Structure Validation', () => {
    it('should have valid package.json', () => {
      ...
      // Then
      expect(pkgJson.name).toBe('@electron-aaos/electron-aaos-install');
      expect(pkgJson.bin).toBeDefined();
      expect(pkgJson.bin['electron-aaos-install']).toBe('./bin/electron-aaos-install.js');
    ...
    it('should have executable shebang in bin files', () => {
      // Given
      const binFiles = ['bin/electron-aaos-install.js', 'bin/edmcp.js'];
  ...
  describe('CLI Execution Tests', () => {
    it('should execute electron-aaos-install --version via node', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/electron-aaos-install.js');
    ...
    it('should show help for electron-aaos-install', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/electron-aaos-install.js');
      ...
      // Then
      expect(result).toContain('electron-aaos-install');
  ...
  describe('NPX Local Execution Simulation', () => {
    it('should execute via npm exec (simulates npx .)', () => {
      ...
      // When
      const result = execSync('npm exec -- electron-aaos-install --version', {
        cwd: PKG_DIR,
        encoding: 'utf8',
        timeout: 30000,
