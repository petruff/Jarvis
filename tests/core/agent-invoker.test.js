...
const {
  ...
  SUPPORTED_AGENTS,
  InvocationStatus,
} = require('../../.electron-aaos-core/core/orchestration/agent-invoker');
...
describe('Agent Invoker (Story 0.7)', () => {
  ...
  beforeEach(async () => {
    ...
    const agentsDir = path.join(tempDir, '.electron-aaos-core', 'development', 'agents');
    ...
    const tasksDir = path.join(tempDir, '.electron-aaos-core', 'development', 'tasks');
...
describe('Integration with MasterOrchestrator', () => {
  ...
  it('should integrate AgentInvoker with MasterOrchestrator', async () => {
    const { MasterOrchestrator } = require('../../.electron-aaos-core/core/orchestration');
