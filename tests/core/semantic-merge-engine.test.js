...
const {
  ...
  ConflictSeverity,
  MergeDecision,
} = require('../../.electron-aaos-core/core/execution/semantic-merge-engine');
...
describe('SemanticMergeEngine Module', () => {
  ...
  describe('CustomRulesLoader', () => {
    ...
    test('constructor sets paths', () => {
      ...
      expect(loader.rulesPath).toContain('.electron-aaos');
    });
  ...
  describe('SemanticMergeEngine', () => {
    ...
    test('saveReport writes JSON and MD files', async () => {
      const engine = new SemanticMergeEngine({
        rootPath: tmpDir,
        storageDir: path.join(tmpDir, '.electron-aaos', 'merge'),
      });
      ...
      const mergeDir = path.join(tmpDir, '.electron-aaos', 'merge');
