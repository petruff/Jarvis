...
const { RegistryLoader } = require('../../../.electron-aaos-core/core/ids/registry-loader');
const { IncrementalDecisionEngine } = require('../../../.electron-aaos-core/core/ids/incremental-decision-engine');
const { FrameworkGovernor, TIMEOUT_MS, RISK_THRESHOLDS } = require('../../../.electron-aaos-core/core/ids/framework-governor');
  ...
  describe('postRegister()', () => {
    it('should register file via RegistryUpdater.onAgentTaskComplete', async () => {
      const result = await governor.postRegister(
        ...
        { type: 'task', purpose: 'Test task', agent: 'electron-aaos-master' },
      );
      expect(result.registered).toBeDefined();
      expect(result.filePath).toBe('.electron-aaos-core/development/tasks/test-task.md');
      expect(updater.onAgentTaskCompleteCalls.length).toBe(1);

      const call = updater.onAgentTaskCompleteCalls[0];
      expect(call.task.agent).toBe('electron-aaos-master');
      expect(call.artifacts).toContain('.electron-aaos-core/development/tasks/test-task.md');
    });

    it('should use onAgentTaskComplete (not processChanges) per SF-1', async () => {
      await governor.postRegister('.electron-aaos-core/development/tasks/new-task.md', {});
    ...
    it('should default agent to electron-aaos-master', async () => {
      ...
      it('should include recommendations when matches found', () => {
        const result = {
          ...
          recommendations: [{
            ...
            decision: 'ADAPT',
            relevanceScore: 0.75,
            entityPath: '.electron-aaos-core/development/tasks/create-doc.md',
          }],
        };
        const output = FrameworkGovernor.formatPreCheckOutput(result);
    ...
    describe('formatImpactOutput()', () => {
      ...
      it('should show safe-to-modify for zero consumers', () => {
        const result = {
          entityId: 'po',
          found: true,
          entityPath: '.electron-aaos-core/development/agents/po.md',
          entityType: 'agent',
          riskLevel: 'NONE',
          directConsumers: [],
          indirectConsumers: [],
