...
const { PermissionMode } = require('../../.electron-aaos-core/core/permissions/permission-mode');
const { OperationGuard } = require('../../.electron-aaos-core/core/permissions/operation-guard');
const {
  createGuard,
  checkOperation,
  getModeBadge,
  ...
  cycleMode,
  enforcePermission,
} = require('../../.electron-aaos-core/core/permissions');
...
describe('Permission Mode Integration (Story ACT-4)', () => {
  ...
  beforeEach(() => {
    ...
    const electron-aaosDir = path.join(tempDir, '.electron-aaos');
    fs.mkdirSync(electron-aaosDir, { recursive: true });
    configPath = path.join(electron-aaosDir, 'config.yaml');
  });
