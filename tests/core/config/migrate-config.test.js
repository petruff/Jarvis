...
const {
  ...
  USER_FIELDS,
  PROJECT_FIELDS,
} = require('../../../.electron-aaos-core/core/config/migrate-config');
...
describe('extractProjectFields', () => {
  ...
  it('should extract project.type from nested project object', () => {
    // Given
    const legacyConfig = {
      project: { type: 'EXISTING_Electron AAOS', version: '2.1.0' },
    };
    ...
    // Then
    expect(projectFields.project_type).toBe('EXISTING_Electron AAOS');
  });
  ...
  it('should not override explicit project_type with nested project.type', () => {
    ...
    const legacyConfig = {
      project_type: 'fullstack',
      project: { type: 'EXISTING_Electron AAOS' },
    };
...
describe('migrateConfig', () => {
  ...
  it('should be idempotent: re-running does not duplicate', () => {
    ...
    const userConfigPath = path.join(FAKE_HOME, '.electron-aaos', 'user-config.yaml');
    setupFileSystem({
      'core-config.yaml': LEGACY_CONFIG_YAML,
...
describe('ensureUserConfigDir', () => {
  it('should create ~/.electron-aaos/ directory if it does not exist', () => {
    // Given
    fs.existsSync.mockReturnValue(false);
    ...
    expect(fs.mkdirSync).toHaveBeenCalledWith(
      expect.stringContaining('.electron-aaos'),
