 ...
 * - electron-aaos-master has *update-source-tree command
 */
...
describe('Story ACT-8: Agent Config Enrichment', () => {
  ...
  beforeAll(() => {
    agentConfig = loadYaml('.electron-aaos-core/data/agent-config-requirements.yaml');
  });
  ...
  describe('YAML Validity', () => {
    ...
    test('all expected agents have entries', () => {
      const expectedAgents = [
        'electron-aaos-master', 'dev', 'qa', 'devops', 'github-devops',
        'architect', 'po', 'sm', 'data-engineer', 'db-sage',
        'pm', 'analyst', 'ux-design-expert', 'squad-creator',
        'electron-aaos-developer', 'electron-aaos-orchestrator', 'default',
      ];
  ...
  describe('7 Required Data Files Exist on Disk', () => {
    const requiredFiles = [
      ...
      { path: '.electron-aaos-core/data/technical-preferences.md', owner: '@architect' },
      { path: '.electron-aaos-core/product/data/test-levels-framework.md', owner: '@qa' },
      { path: '.electron-aaos-core/product/data/test-priorities-matrix.md', owner: '@qa' },
      { path: '.electron-aaos-core/product/data/brainstorming-techniques.md', owner: '@analyst' },
      { path: '.electron-aaos-core/product/data/elicitation-methods.md', owner: '@po' },
    ];
  ...
  describe('Enriched Agent: @analyst', () => {
    test('analyst now loads brainstorming-techniques, tech-stack, and source-tree', () => {
      ...
      expect(paths).toContain('.electron-aaos-core/product/data/brainstorming-techniques.md');
      expect(paths).toContain('docs/framework/tech-stack.md');
  ...
  describe('Enriched Agent: @sm', () => {
    test('sm now loads mode-selection, workflow-patterns, and coding-standards', () => {
      ...
      expect(paths).toContain('.electron-aaos-core/product/data/mode-selection-best-practices.md');
      expect(paths).toContain('.electron-aaos-core/data/workflow-patterns.yaml');
      expect(paths).toContain('docs/framework/coding-standards.md');
    });
...
describe('Story ACT-8: Document Governance', () => {
  ...
  describe('Governance Task', () => {
    test('update-source-tree.md task file exists', () => {
      expect(fileExists('.electron-aaos-core/development/tasks/update-source-tree.md')).toBe(true);
    });

    test('task file contains expected sections', () => {
      const content = readFile('.electron-aaos-core/development/tasks/update-source-tree.md');
  ...
  describe('electron-aaos-master Command', () => {
    test('electron-aaos-master.md contains *update-source-tree command', () => {
      const content = readFile('.electron-aaos-core/development/agents/electron-aaos-master.md');
      expect(content).toContain('update-source-tree');
      expect(content).toContain('Validate data file governance');
    });

    test('electron-aaos-master dependencies include update-source-tree.md', () => {
      const content = readFile('.electron-aaos-core/development/agents/electron-aaos-master.md');
