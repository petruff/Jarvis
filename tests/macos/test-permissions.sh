...
LOG_FILE="/tmp/electron-aaos-test-perms-$(date +%Y%m%d-%H%M%S).log"
...
test_script_permissions() {
    ...
    # Find Electron AAOS scripts
    if [[ -d "$HOME/.electron-aaos/bin" ]]; then
        SCRIPTS=$(find "$HOME/.electron-aaos/bin" -type f -name "*.sh" 2>/dev/null || true)
...
test_config_permissions() {
    ...
    CONFIG_FILES=(
        "$HOME/.electron-aaos/config.json"
        "$HOME/.electron-aaos/.electron-aaosrc"
    )
...
test_directory_permissions() {
    ...
    if [[ -d "$HOME/.electron-aaos" ]]; then
        DIR_PERMS=$(stat -f "%Lp" "$HOME/.electron-aaos" 2>/dev/null || stat -c "%a" "$HOME/.electron-aaos" 2>/dev/null)
...
test_no_sudo_required() {
    log_info "Test 4: Verifying no sudo required for Electron AAOS operations..."

    # Check if current user owns .electron-aaos directory
    if [[ -d "$HOME/.electron-aaos" ]]; then
        OWNER=$(stat -f "%Su" "$HOME/.electron-aaos" 2>/dev/null || stat -c "%U" "$HOME/.electron-aaos" 2>/dev/null)

        if [[ "$OWNER" == "$(whoami)" ]]; then
            pass_test "User owns .electron-aaos directory (no sudo needed)"
        else
            fail_test ".electron-aaos owned by: $OWNER (expected: $(whoami))"
        fi
    fi
}
