...
This guide provides step-by-step instructions for manually testing Electron AAOS on macOS (Intel and Apple Silicon).
...
```bash
# Backup existing Electron AAOS installation (if any)
if [ -d "$HOME/.electron-aaos" ]; then
    mv "$HOME/.electron-aaos" "$HOME/.electron-aaos.backup.$(date +%Y%m%d-%H%M%S)"
fi

# Verify backup
ls -la "$HOME/.electron-aaos.backup"*
```

### 2. Navigate to Test Directory

```bash
cd path/to/@electron-aaos/electron-aaos-core/tests/macos
chmod +x *.sh
```
...
```bash
# Check Electron AAOS command availability
which electron-aaos
# Expected: /usr/local/bin/electron-aaos or similar

# Verify version
electron-aaos --version

# Run health check
electron-aaos health
```
...
cat ~/.zshrc | grep -i electron-aaos

# Open new terminal window
# Run: electron-aaos --version
...
cat ~/.bashrc | grep -i electron-aaos
# Or check ~/.bash_profile

# Run Electron AAOS command
electron-aaos --version
...
```bash
# Check Electron AAOS installation path
which electron-aaos
# Should use forward slashes: /usr/local/bin/electron-aaos
# NOT backslashes: \usr\local\bin\electron-aaos
```
...
cd ~/.electron-aaos
pwd
# Should show full path: /Users/username/.electron-aaos
```

#### 3. Test Symlinks

```bash
# Check if electron-aaos command is a symlink
ls -la $(which electron-aaos)
...
```bash
# Check line endings of Electron AAOS config
file ~/.electron-aaos/config.json
# Should show: ASCII text (LF line endings)
# NOT: ASCII text, with CRLF line endings

# Check for carriage returns
grep -r $'\r' ~/.electron-aaos/ || echo "No CRLF found - PASS"
```
...
ls -la ~/.electron-aaos/bin/*.sh 2>/dev/null
# Should show: -rwxr-xr-x (executable)

# Try running a script without chmod
~/.electron-aaos/bin/some-script.sh
...
ls -l ~/.electron-aaos/config.json
...
- [ ] User owns all Electron AAOS files
- [ ] No sudo required
...
time npx @electron-aaosai/electron-aaos@latest init
...
- [ ] No "unidentified developer" warnings for Electron AAOS
...
# Simulate corruption
echo "corrupted" > ~/.electron-aaos/config.json

# Try to recover
mv ~/.electron-aaos ~/.electron-aaos.broken
mv ~/.electron-aaos.backup ~/.electron-aaos
...
- Permission denied: `chmod 000 ~/.electron-aaos`
...
npx @electron-aaosai/electron-aaos@latest init --verbose
```

#### Command Not Found

**Symptom:** `bash: electron-aaos: command not found`
...
# Check PATH
echo $PATH | grep -o '/[^:]*electron-aaos[^:]*'

# Manually add to PATH if needed
export PATH="$HOME/.electron-aaos/bin:$PATH"
```
...
chmod +x ~/.electron-aaos/bin/*.sh

# Fix ownership
sudo chown -R $(whoami) ~/.electron-aaos
...
**Symptom:** `electron-aaos health` shows unhealthy MCPs

**Solution:**

```bash
# Reinstall specific MCP
electron-aaos mcp reinstall <mcp-name>
...
2. **Test Logs:**
   - All files from `/tmp/electron-aaos-test-*.log`
