# macOS Testing Suite for Electron AAOS
**Story 1.10b - macOS Testing & Validation**

Comprehensive test automation framework for validating Electron AAOS installation on macOS (Intel and Apple Silicon).
...
2. **Log Files:**
   - Location: `/tmp/electron-aaos-test-*.log`
...
```bash
./run-all-tests.sh
# View report at: /tmp/electron-aaos-macos-test-report-*.txt
```
...
2. **Test Logs:**
   - Attach all `/tmp/electron-aaos-test-*.log` files
