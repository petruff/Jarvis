...
describe('v2.1 Module Structure Tests', () => {
  describe('INS-01: Source Path Configuration', () => {
    it('should point getAiosCorePath to .electron-aaos-core directory', () => {
      const electron-aaosCorePath = resourceLocator.getAiosCorePath();
      expect(electron-aaosCorePath).toContain('.electron-aaos-core');
      expect(electron-aaosCorePath).not.toMatch(/[^.]electron-aaos-core$/);
    });

    it('should have .electron-aaos-core as source for config-loader', () => {
      const electron-aaosCorePath = configLoader.getAiosCorePath();
      expect(electron-aaosCorePath).toContain('.electron-aaos-core');
    });
  });
  ...
  describe('INS-02: Agent Path Resolution', () => {
    ...
    it('should list agents from development/agents/', async () => {
      ...
      for (const agent of agents) {
        expect(agent.file).toContain('.electron-aaos-core/development/agents/');
      }
    });
  });
  ...
  describe('INS-04: Resource Locator v2.1 Paths', () => {
    it('should resolve agent dependencies with v2.1 module mapping', async () => {
      ...
      for (const depPath of deps.all) {
        // Should not have flat structure paths like .electron-aaos-core/tasks/
        expect(depPath).not.toMatch(/\.electron-aaos-core\/(tasks|templates|checklists|workflows|utils|data)\//);
  ...
  describe('INS-05: Manifest Files Location', () => {
    it('should have manifests directory in .electron-aaos-core', async () => {
