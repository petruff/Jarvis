...
describe('ACT-12: Language delegated to Claude Code settings.json', () => {
  ...
  beforeEach(async () => {
    tempDir = path.join(os.tmpdir(), `electron-aaos-test-lang-${Date.now()}`);
    await fse.ensureDir(tempDir);
    await fse.ensureDir(path.join(tempDir, '.electron-aaos-core'));
  });
  ...
  test('should NOT include language in generated core-config.yaml', async () => {
    ...
    const configPath = path.join(tempDir, '.electron-aaos-core', 'core-config.yaml');
