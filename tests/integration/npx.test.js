 ...
 * Tests for npx @electron-aaos/electron-aaos-core@latest execution
 */
...
describeIntegration('npx Execution', () => {
  ...
  describeIntegration('Package Configuration', () => {
    it('should have correct bin entries in package.json', () => {
      ...
      expect(packageJson.bin['electron-aaos']).toBe('./bin/electron-aaos.js');
      expect(packageJson.bin['@electron-aaos/electron-aaos-core']).toBe('./bin/electron-aaos.js');
    });
    ...
    it('should include required files in package', () => {
      ...
      // Note: .electron-aaos-core/ should be included if it exists
      const hasAiosCore = packageJson.files.some(f => 
        f === '.electron-aaos-core/' || f === 'electron-aaos-core/',
      );
  ...
  describeIntegration('CLI Execution', () => {
    it('should execute electron-aaos command with --version', (done) => {
      const cliPath = path.join(__dirname, '../../bin/electron-aaos.js');
    ...
    it('should execute electron-aaos command with --help', (done) => {
      const cliPath = path.join(__dirname, '../../bin/electron-aaos.js');
      ...
      child.on('close', (code) => {
        ...
        expect(output).toContain('@electron-aaos/electron-aaos-core');
        done();
      });
    }, timeout);

    it('should execute electron-aaos info command', (done) => {
      const cliPath = path.join(__dirname, '../../bin/electron-aaos.js');
  ...
  describeIntegration('Index.js init Export', () => {
    ...
    it('should maintain Electron AAOS class export in source', () => {
      const indexPath = path.join(__dirname, '../../index.js');
      const indexContent = fs.readFileSync(indexPath, 'utf8');

      // Verify Electron AAOS class exists in exports
      expect(indexContent).toContain('Electron AAOS');
      expect(indexContent).toContain('class Electron AAOS');
    });
