...
describe('MCP Setup Integration', () => {
  const cliIndexPath = path.join(__dirname, '../../.electron-aaos-core/cli/index.js');
  let tempDir;
  let originalHome;
  ...
  describe('CLI Module Loading', () => {
    it('should load mcp command module without errors', () => {
      expect(() => {
        require('../../.electron-aaos-core/cli/commands/mcp');
      }).not.toThrow();
    });

    it('should export createMcpCommand function', () => {
      const mcpModule = require('../../.electron-aaos-core/cli/commands/mcp');
      expect(typeof mcpModule.createMcpCommand).toBe('function');
    });

    it('should load all mcp subcommand modules', () => {
      expect(() => {
        require('../../.electron-aaos-core/cli/commands/mcp/setup');
        require('../../.electron-aaos-core/cli/commands/mcp/link');
        require('../../.electron-aaos-core/cli/commands/mcp/status');
        require('../../.electron-aaos-core/cli/commands/mcp/add');
      }).not.toThrow();
    });
  });
  ...
  describe('Core Module Loading', () => {
    it('should load os-detector module', () => {
      const osDetector = require('../../.electron-aaos-core/core/mcp/os-detector');
    ...
    it('should load global-config-manager module', () => {
      const configManager = require('../../.electron-aaos-core/core/mcp/global-config-manager');
    ...
    it('should load symlink-manager module', () => {
      const symlinkManager = require('../../.electron-aaos-core/core/mcp/symlink-manager');
    ...
    it('should load config-migrator module', () => {
      const configMigrator = require('../../.electron-aaos-core/core/mcp/config-migrator');
    ...
    it('should load mcp index module with all exports', () => {
      const mcp = require('../../.electron-aaos-core/core/mcp');
  ...
  describe('OS Detection', () => {
    const { detectOS, isWindows, isMacOS, isLinux, getOSInfo } = require('../../.electron-aaos-core/core/mcp/os-detector');
  ...
  describe('Server Templates', () => {
    const { getAvailableTemplates, getServerTemplate, SERVER_TEMPLATES } = require('../../.electron-aaos-core/core/mcp/global-config-manager');
  ...
  describe('Path Generation', () => {
    ...
    it('should generate correct global Electron AAOS directory path', () => {
      const electron-aaosDir = getGlobalAiosDir();

      expect(electron-aaosDir).toContain('.electron-aaos');
      expect(path.isAbsolute(electron-aaosDir)).toBe(true);
    });

    it('should generate correct MCP directory path', () => {
      const mcpDir = getGlobalMcpDir();

      expect(mcpDir).toContain('.electron-aaos');
  ...
  describe('Link Status Constants', () => {
    const { LINK_STATUS } = require('../../.electron-aaos-core/core/mcp/symlink-manager');
  ...
  describe('Migration Options Constants', () => {
    const { MIGRATION_OPTION } = require('../../.electron-aaos-core/core/mcp/config-migrator');
  ...
  describe('Default Config Structure', () => {
    const { DEFAULT_CONFIG } = require('../../.electron-aaos-core/core/mcp/global-config-manager');
  ...
  describe('CLI Help Output', () => {
    const { createMcpCommand } = require('../../.electron-aaos-core/cli/commands/mcp');
...
describe('MCP CLI Command Registration', () => {
  it('should have mcp command in main CLI', () => {
    const { createProgram } = require('../../.electron-aaos-core/cli/index');
