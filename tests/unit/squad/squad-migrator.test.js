...
const {
  ...
  SquadMigratorError,
  MigratorErrorCodes,
} = require('../../../.electron-aaos-core/development/scripts/squad');
...
describe('SquadMigrator', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should detect missing electron-aaos.type field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);

      expect(analysis.needsMigration).toBe(true);
      expect(analysis.issues.some((i) => i.type === 'MISSING_Electron AAOS_TYPE')).toBe(true);
      expect(analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'electron-aaos.type')).toBe(
        true,
      );
    });

    it('should detect missing electron-aaos.minVersion field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);
      ...
      expect(
        analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'electron-aaos.minVersion'),
      ).toBe(true);
    });
    ...
    it('should return needsMigration: false for up-to-date squad', async () => {
      ...
      const manifest = {
        electron-aaos: { type: 'squad', minVersion: '2.1.0' },
  ...
  describe('migrate()', () => {
    ...
    it('should add missing electron-aaos.type and electron-aaos.minVersion fields (AC 5)', async () => {
      // Copy legacy squad to temp dir
      const srcPath = path.join(FIXTURES_PATH, 'legacy-squad');
      ...
      expect(manifest.electron-aaos?.type).toBe('squad');
      expect(manifest.electron-aaos?.minVersion).toBe('2.1.0');
    });
