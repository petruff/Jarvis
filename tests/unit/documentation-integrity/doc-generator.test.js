...
const {
  ...
  TemplateFiles,
  OutputFiles,
} = require('../../../.electron-aaos-core/infrastructure/scripts/documentation-integrity/doc-generator');

describe('Documentation Generator', () => {
  let tempDir;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-docgen-test-'));
  });
  ...
  describe('renderTemplate', () => {
    ...
    it('should handle multiple variables', () => {
      ...
      const context = { greeting: 'Hello', name: 'User', place: 'Electron AAOS' };

      const result = renderTemplate(template, context);

      expect(result).toBe('Hello User, welcome to Electron AAOS!');
    });
