...
const {
  ...
  DeploymentWorkflow,
  DeploymentPlatform,
} = require('../../../.electron-aaos-core/infrastructure/scripts/documentation-integrity/config-generator');

describe('Config Generator', () => {
  let tempDir;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-configgen-test-'));
  });
  ...
  describe('generateConfig', () => {
    ...
    it('should create .electron-aaos-core directory', () => {
      const context = buildConfigContext('test', 'greenfield', {});

      generateConfig(tempDir, 'greenfield', context);

      expect(fs.existsSync(path.join(tempDir, '.electron-aaos-core'))).toBe(true);
      expect(fs.existsSync(path.join(tempDir, '.electron-aaos-core', 'core-config.yaml'))).toBe(true);
    });
    ...
    it('should support dry run mode', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.content).toBeTruthy();
      expect(fs.existsSync(path.join(tempDir, '.electron-aaos-core'))).toBe(false);
    });
