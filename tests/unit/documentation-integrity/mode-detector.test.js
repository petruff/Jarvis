...
const {
  ...
  InstallationMode,
  LegacyProjectType,
} = require('../../../.electron-aaos-core/infrastructure/scripts/documentation-integrity/mode-detector');
...
describe('Mode Detector', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'electron-aaos-test-'));
  });
  ...
  describe('detectInstallationMode', () => {
    ...
    it('should detect FRAMEWORK_DEV mode for electron-aaos-core repository', () => {
      // Create .electron-aaos-core directory
      fs.mkdirSync(path.join(tempDir, '.electron-aaos-core'));

      // Create package.json with electron-aaos-core name
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({
          name: '@electron-aaos/core',
          workspaces: ['packages/*'],
        }),
      );
      ...
      expect(result.legacyType).toBe(LegacyProjectType.EXISTING_Electron AAOS);
      expect(result.confidence).toBe(100);
      expect(result.markers.isAiosCoreRepo).toBe(true);
    });

    it('should detect BROWNFIELD mode for user project with existing Electron AAOS', () => {
      // Create .electron-aaos-core directory
      fs.mkdirSync(path.join(tempDir, '.electron-aaos-core'));
  ...
  describe('isAiosCoreRepository', () => {
    it('should return true for @electron-aaos/core package', () => {
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: '@electron-aaos/core' }),
      );
    ...
    it('should return false for generic monorepo with workspaces pattern only', () => {
      // Generic monorepos should NOT be detected as electron-aaos-core
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
    ...
    it('should return true for workspaces pattern with electron-aaos marker', () => {
      // Workspaces + .electron-aaos-core/infrastructure marker = electron-aaos-core repo
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );
      fs.mkdirSync(path.join(tempDir, '.electron-aaos-core', 'infrastructure'), { recursive: true });
  ...
  describe('mapLegacyTypeToMode', () => {
    it('should map EXISTING_Electron AAOS to BROWNFIELD by default (safer)', () => {
      // Without context, EXISTING_Electron AAOS defaults to BROWNFIELD (won't skip project setup)
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_Electron AAOS)).toBe(
        InstallationMode.BROWNFIELD,
      );
    });

    it('should map EXISTING_Electron AAOS to FRAMEWORK_DEV with isAiosCoreRepo context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_Electron AAOS, { isAiosCoreRepo: true })).toBe(
        InstallationMode.FRAMEWORK_DEV,
      );
    });

    it('should map EXISTING_Electron AAOS to BROWNFIELD with non-electron-aaos-core context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_Electron AAOS, { isAiosCoreRepo: false })).toBe(
        InstallationMode.BROWNFIELD,
      );
    });
  ...
  describe('validateModeSelection', () => {
    ...
    it('should warn when selecting framework-dev for non-electron-aaos-core repo', () => {
      const detected = {
        mode: InstallationMode.BROWNFIELD,
      ...
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain('electron-aaos-core repository');
    });
  ...
  describe('LegacyProjectType enum', () => {
    it('should have all legacy types', () => {
      expect(LegacyProjectType.EXISTING_Electron AAOS).toBe('EXISTING_Electron AAOS');
