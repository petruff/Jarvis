...
describe('IDE Config Generator', () => {
  ...
  describe('generateTemplateVariables', () => {
    it('should generate default variables', () => {
      ...
      expect(variables).toHaveProperty('electron-aaosVersion');
    });
    ...
    it('should include Electron AAOS version', () => {
      const wizardState = {};
      const variables = generateTemplateVariables(wizardState);

      expect(variables.electron-aaosVersion).toBeDefined();
      expect(typeof variables.electron-aaosVersion).toBe('string');
    });
  });
  ...
  describe('generateIDEConfigs', () => {
    ...
    it('should render template with variables', async () => {
      ...
      // v2.1 templates use static content from .electron-aaos-core/templates/ide-rules/
      // They contain Electron AAOS Electron AAOS standard rules
      expect(content).toContain('Electron AAOS Electron AAOS');
    ...
    it('should create github-copilot config in .github directory', async () => {
      ...
      const content = await fs.readFile(configPath, 'utf8');
      // Should contain Electron AAOS rules content
      expect(content).toContain('Electron AAOS Electron AAOS');
    });
