...
const {
  ...
  findLatestBackup,
  listBackups,
} = require('../../.electron-aaos-core/cli/commands/migrate/backup');
...
describe('Migration Backup Module', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `electron-aaos-backup-test-${testId}`);
  ...
  describe('createBackupDirName', () => {
    it('should create backup directory name with date format', () => {
      ...
      expect(name).toMatch(/^\.electron-aaos-backup-\d{4}-\d{2}-\d{2}$/);
    });

    it('should use current date', () => {
      const name = createBackupDirName();
      const today = new Date().toISOString().split('T')[0];
      expect(name).toBe(`.electron-aaos-backup-${today}`);
    });
  });
  ...
  describe('createBackup', () => {
    it('should create backup of .electron-aaos-core directory', async () => {
      // Create mock .electron-aaos-core structure
      ...
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'agents', 'test.md'), 'Agent content');
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'index.js'), 'module.exports = {}');
    ...
    it('should fail if .electron-aaos-core does not exist', async () => {
      await expect(createBackup(testDir)).rejects.toThrow(/No .electron-aaos-core directory/);
    });

    it('should include backup manifest', async () => {
      const electron-aaosCoreDir = path.join(testDir, '.electron-aaos-core');
      await fs.promises.mkdir(electron-aaosCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'test.js'), 'test');
  ...
  describe('verifyBackup', () => {
    it('should verify valid backup', async () => {
      ...
      const electron-aaosCoreDir = path.join(testDir, '.electron-aaos-core');
      await fs.promises.mkdir(electron-aaosCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'test.js'), 'test content');
    ...
    it('should detect corrupted files', async () => {
      const electron-aaosCoreDir = path.join(testDir, '.electron-aaos-core');
      await fs.promises.mkdir(electron-aaosCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'test.js'), 'original');

      const backupResult = await createBackup(testDir);

      // Corrupt a file in backup
      const backedUpFile = path.join(backupResult.backupDir, '.electron-aaos-core', 'test.js');
  ...
  describe('findLatestBackup', () => {
    it('should find the most recent backup', async () => {
      ...
      expect(latest.name).toMatch(/^\.electron-aaos-backup-/);
    });
