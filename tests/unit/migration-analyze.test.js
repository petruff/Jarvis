...
const {
  ...
  formatMigrationPlan,
  analyzeImports,
} = require('../../.electron-aaos-core/cli/commands/migrate/analyze');

describe('Migration Analyze Module', () => {
  let testDir;

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `electron-aaos-analyze-test-${Date.now()}`);
  ...
  describe('detectV2Structure', () => {
    ...
    it('should detect v2.1 modular structure', async () => {
      ...
      await fs.promises.mkdir(path.join(electron-aaosCoreDir, 'product'), { recursive: true });
      await fs.promises.mkdir(path.join(electron-aaosCoreDir, 'infrastructure'), { recursive: true });
    ...
    it('should return error if no .electron-aaos-core exists', async () => {
      const result = await detectV2Structure(testDir);
  ...
  describe('analyzeMigrationPlan', () => {
    it('should generate migration plan for v2.0 project', async () => {
      ...
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'tasks', 'build.md'), 'Task');
    ...
    it('should detect conflicts', async () => {
      ...
      const electron-aaosCoreDir = path.join(testDir, '.electron-aaos-core');
      await fs.promises.mkdir(path.join(electron-aaosCoreDir, 'agents'), { recursive: true });
      await fs.promises.mkdir(path.join(electron-aaosCoreDir, 'core'), { recursive: true }); // Conflict
  ...
  describe('analyzeImports', () => {
    it('should analyze importable files', async () => {
      const electron-aaosCoreDir = path.join(testDir, '.electron-aaos-core');
      await fs.promises.mkdir(path.join(electron-aaosCoreDir, 'cli'), { recursive: true });
      await fs.promises.writeFile(path.join(electron-aaosCoreDir, 'cli', 'index.js'), 'module.exports = {}');
