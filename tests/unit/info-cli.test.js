...
const {
  ...
  formatNotFoundError,
  wrapText,
} = require('../../.electron-aaos-core/cli/commands/workers/formatters/info-formatter');
const { findSuggestions, findRelatedWorkers } = require('../../.electron-aaos-core/cli/commands/workers/info');
const { levenshteinDistance } = require('../../.electron-aaos-core/cli/commands/workers/search-keyword');
...
describe('Info Formatter - Pretty Output', () => {
  ...
  test('formatInfoPretty includes usage example', () => {
    ...
    expect(output).toContain('electron-aaos task run json-csv-transformer');
  });
...
describe('Not Found Error Formatter', () => {
  ...
  test('formatNotFoundError includes search hint', () => {
    ...
    expect(output).toContain('electron-aaos workers search invalid');
  });
});
