...
jest.mock('../../../.electron-aaos-core/core/mcp/os-detector', () => ({
  isWindows: jest.fn(),
...
const {
  ...
  createLink,
  removeLink,
} = require('../../../.electron-aaos-core/core/mcp/symlink-manager');

const osDetector = require('../../../.electron-aaos-core/core/mcp/os-detector');

describe('Symlink Manager', () => {
  const mockProjectRoot = '/mock/project';
  const mockGlobalMcpDir = '/mock/home/.electron-aaos/mcp';
  const mockLinkPath = path.join(mockProjectRoot, '.electron-aaos-core', 'tools', 'mcp');
  ...
  describe('getProjectMcpPath', () => {
    it('should return correct path for project root', () => {
      ...
      expect(result).toBe(path.join(mockProjectRoot, '.electron-aaos-core', 'tools', 'mcp'));
    });

    it('should use current directory when no root provided', () => {
      const cwd = process.cwd();
      const result = getProjectMcpPath();
      expect(result).toBe(path.join(cwd, '.electron-aaos-core', 'tools', 'mcp'));
    });
  });
