...
describe('generate-greeting.js', () => {
  describe('generateGreeting()', () => {
    it('should generate greeting for valid agent', async () => {
      ...
      const agentPath = path.join(process.cwd(), '.electron-aaos-core', 'development', 'agents', `${agentId}.md`);
  ...
  const tests = [
    {
      ...
      fn: async () => {
        const fs = require('fs').promises;
        const agentPath = path.join(process.cwd(), '.electron-aaos-core', 'agents', 'qa.md');
        await fs.access(agentPath);
        return true;
      },
    },
