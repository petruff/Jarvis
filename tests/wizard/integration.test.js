...
const { installAiosCore, hasPackageJson } = require('../../packages/installer/src/installer/electron-aaos-core-installer');
...
jest.mock('../../packages/installer/src/installer/electron-aaos-core-installer');
...
describe('Wizard Integration - Story 1.7', () => {
  ...
  beforeEach(() => {
    ...
    // Mock Electron AAOS core installer
  ...
  describe('Full Wizard Flow (AC Integration)', () => {
    ...
    it('should install Electron AAOS core before IDE configs', async () => {
      await runWizard();

      // Verify Electron AAOS core was installed
      expect(installAiosCore).toHaveBeenCalled();

      // Verify order: Electron AAOS core before IDE configs
      const electron-aaosCoreCallOrder = installAiosCore.mock.invocationCallOrder[0];
      const ideConfigCallOrder = generateIDEConfigs.mock.invocationCallOrder[0];

      expect(electron-aaosCoreCallOrder).toBeLessThan(ideConfigCallOrder);
    });
  ...
  describe('Wizard State Flow', () => {
    it('should maintain correct state through all steps', async () => {
      ...
      expect(answers.electron-aaosCoreInstalled).toBeDefined(); // Story 1.4 - Electron AAOS core
    });
    ...
    it('should handle Electron AAOS core installation failure gracefully', async () => {
      installAiosCore.mockRejectedValue(new Error('Electron AAOS core installation failed'));

      const answers = await runWizard();

      expect(answers.electron-aaosCoreInstalled).toBe(false);
