 ...
 * integration points for Electron AAOS workflows.
 */
...
/**
 * IntegrationSuggester - Suggests Electron AAOS integration points
 */
class IntegrationSuggester {
  ...
  suggest(analysis, provider) {
    ...
    // Add Electron AAOS build step
    suggestions.push({
      type: 'add_step',
      priority: 'high',
      title: 'Add Electron AAOS Build Orchestration',
      description: 'Integrate Electron AAOS build orchestrator for intelligent parallel builds',
      code: this.getElectron AAOSBuildStep(provider),
    });
    ...
    if (!analysis.hasTests) {
      suggestions.push({
        ...
        description: 'Use Electron AAOS test discovery to automatically find and run tests',
    ...
    if (!analysis.hasSecurityScan) {
      suggestions.push({
        ...
        description: 'Add Electron AAOS PR Review AI for security analysis',
    ...
    // Add Electron AAOS status reporting
    suggestions.push({
      type: 'add_step',
      priority: 'low',
      title: 'Add Electron AAOS Status Reporting',
      description: 'Report build status to Electron AAOS dashboard',
  ...
  /**
   * Get Electron AAOS build step for provider
   */
  getElectron AAOSBuildStep(provider) {
    const steps = {
      'github-actions': `- name: Electron AAOS Build Orchestration
  run: npx electron-aaos build --parallel --smart-cache
  env:
    Electron AAOS_CI: true`,

      'gitlab-ci': `electron-aaos_build:
  stage: build
  script:
    - npx electron-aaos build --parallel --smart-cache
  variables:
    Electron AAOS_CI: "true"`,

      jenkins: `stage('Electron AAOS Build') {
  steps {
    sh 'npx electron-aaos build --parallel --smart-cache'
  }
  environment {
    Electron AAOS_CI = 'true'
  }
      ...
      circleci: `- run:
    name: Electron AAOS Build Orchestration
    command: npx electron-aaos build --parallel --smart-cache
    environment:
      Electron AAOS_CI: true`,
    };
  ...
  run: npx electron-aaos test --discover --coverage`,

      'gitlab-ci': `test:
  stage: test
  script:
    - npx electron-aaos test --discover --coverage
  coverage: '/Coverage: \\d+\\.\\d+%/'`,

      jenkins: `stage('Test') {
  steps {
    sh 'npx electron-aaos test --discover --coverage'
  }
}`,

      circleci: `- run:
    name: Run Tests
    command: npx electron-aaos test --discover --coverage`,
    };
  ...
  run: npx electron-aaos review --security-only
  continue-on-error: true`,

      'gitlab-ci': `security_scan:
  stage: test
  script:
    - npx electron-aaos review --security-only
  allow_failure: true`,

      jenkins: `stage('Security Scan') {
  steps {
    sh 'npx electron-aaos review --security-only'
  }
  post {
    failure {
      ...
      circleci: `- run:
    name: Security Scan
    command: npx electron-aaos review --security-only
    when: always`,
    };
  ...
  getStatusStep(provider) {
    const steps = {
      'github-actions': `- name: Report to Electron AAOS
  if: always()
  run: npx electron-aaos status --report-ci
  env:
    Electron AAOS_BUILD_STATUS: \${{ job.status }}`,

      'gitlab-ci': `report_status:
  stage: .post
  script:
    - npx electron-aaos status --report-ci
  when: always`,

      jenkins: `post {
  always {
    sh 'npx electron-aaos status --report-ci'
  }
}`,

      circleci: `- run:
    name: Report to Electron AAOS
    command: npx electron-aaos status --report-ci
    when: always`,
    };
