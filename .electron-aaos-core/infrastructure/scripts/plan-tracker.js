...
/**
 * Electron AAOS Plan Progress Tracker
 *
 ...
 *
 * Features:
 * - AC1: Located in `.electron-aaos-core/infrastructure/scripts/`
 ...
 * - AC7: Integrates with `.electron-aaos/dashboard/status.json`
 *
...
class PlanTracker {
  ...
  _findPlanPath() {
    const searchPaths = [
      ...
      // .electron-aaos plans location
      path.join(this.rootPath, '.electron-aaos/plans', this.storyId, CONFIG.implementationFile),
      // Epic-based location (e.g., electron-aaos-core-ade/story-4.6)
   ...
   *
   * Updates `.electron-aaos/dashboard/status.json` for dashboard integration.
   * Also updates legacy `.electron-aaos/status.json` for backwards compatibility.
   *
...
async function main() {
  ...
  if (args.length < 1 || args.includes('--help') || args.includes('-h')) {
    console.log(`
\u{1F4CA} Plan Progress Tracker - Electron AAOS Execution Engine (Story 4.6)
