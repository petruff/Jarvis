 ...
 * Used for brownfield mode installation to adapt Electron AAOS to existing projects.
 *
  ...
  if (hasDocDir) {
    ...
    if (fs.existsSync(archDir)) {
      analysis.conflicts.push(
        'docs/architecture/ already exists - Electron AAOS docs may need different location',
      );
    }
  }
}
...
function generateRecommendations(analysis) {
  ...
  if (analysis.linting !== 'none') {
    analysis.recommendations.push(
      `Preserve existing ${analysis.linting} configuration - Electron AAOS will adapt`,
    );
  } else {
  ...
  if (analysis.formatting !== 'none') {
    analysis.recommendations.push(
      `Keep existing ${analysis.formatting} settings - Electron AAOS coding-standards.md will document them`,
    );
  }

  // Workflow recommendations
  if (analysis.hasExistingWorkflows) {
    analysis.recommendations.push('Review existing CI/CD before adding Electron AAOS workflows');
    analysis.mergeStrategy = 'manual';
  } else {
    analysis.recommendations.push('Use *setup-github to add Electron AAOS standard workflows');
    analysis.mergeStrategy = 'parallel';
  }

  // TypeScript recommendations
  if (analysis.configs.tsconfig) {
    analysis.recommendations.push('Electron AAOS will use existing tsconfig.json settings');
  }
  ...
  if (analysis.frameworks.includes('NestJS')) {
    analysis.recommendations.push('NestJS detected - Electron AAOS will adapt to module structure');
  }
}
