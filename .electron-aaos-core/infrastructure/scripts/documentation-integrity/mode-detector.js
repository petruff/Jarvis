 ...
 *
 * Detects installation mode for Electron AAOS projects with three-mode support:
 * - FRAMEWORK_DEV: Contributing to electron-aaos-core itself
...
/**
 * Installation modes supported by Electron AAOS
 * @enum {string}
 */
...
const LegacyProjectType = {
  EXISTING_Electron AAOS: 'EXISTING_Electron AAOS',
  GREENFIELD: 'GREENFIELD',
  BROWNFIELD: 'BROWNFIELD',
  UNKNOWN: 'UNKNOWN',
};
...
const ModeDescriptions = {
  [InstallationMode.FRAMEWORK_DEV]: {
    ...
    hint: 'Developing electron-aaos-core itself - uses framework standards, skips project setup',
    description: 'For Electron AAOS contributors working on the framework',
  },
  [InstallationMode.GREENFIELD]: {
    label: '🆕 New Project (Greenfield)',
    hint: 'Start a fresh project with Electron AAOS - generates project docs, config, and infrastructure',
    description: 'Empty directory setup with full scaffolding',
  },
  [InstallationMode.BROWNFIELD]: {
    label: '📂 Existing Project (Brownfield)',
    hint: 'Add Electron AAOS to existing project - analyzes current structure and adapts',
 ...
 * 1. FRAMEWORK_DEV - .electron-aaos-core/ exists AND is electron-aaos-core repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  // Priority 1: Check for Electron AAOS framework development
  if (markers.hasAiosCore && markers.isAiosCoreRepo) {
    return {
      mode: InstallationMode.FRAMEWORK_DEV,
      legacyType: LegacyProjectType.EXISTING_Electron AAOS,
      confidence: 100,
      reason: 'Detected electron-aaos-core repository with .electron-aaos-core directory',
      markers,
    };
  }

  // Priority 2: Check for existing Electron AAOS installation (non-framework)
  if (markers.hasAiosCore && !markers.isAiosCoreRepo) {
    return {
      mode: InstallationMode.BROWNFIELD,
      legacyType: LegacyProjectType.EXISTING_Electron AAOS,
      confidence: 95,
      reason: 'Electron AAOS already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // Electron AAOS markers
    hasAiosCore: fs.existsSync(path.join(targetDir, '.electron-aaos-core')),
...
/**
 * Checks if the target directory is the electron-aaos-core repository itself
 *
 * @param {string} targetDir - Directory to check
 * @returns {boolean} True if this is the electron-aaos-core repository
 */
function isAiosCoreRepository(targetDir) {
  ...
  try {
    ...
    // Primary check: explicit electron-aaos-core package names
    if (packageJson.name === '@electron-aaos/core' || packageJson.name === 'electron-aaos-core') {
      return true;
    }

    // Secondary check: workspaces pattern + electron-aaos-specific marker file
    // This prevents false positives for generic monorepos
    const hasAiosMarker = fs.existsSync(path.join(targetDir, '.electron-aaos-core', 'infrastructure'));
    const hasWorkspaces =
    ...
    if (process.env.Electron AAOS_DEBUG) {
      console.warn(`[mode-detector] Error checking electron-aaos-core repository: ${error.message}`);
    }
    return false;
  }
}
 ...
 * for EXISTING_Electron AAOS legacy type (both use EXISTING_Electron AAOS but with different modes).
 * For accurate mode detection, use the mode property from detectInstallationMode().
 *
 * @param {string} legacyType - Legacy project type (EXISTING_Electron AAOS, GREENFIELD, etc.)
 * @param {Object} [context] - Optional context for disambiguation
 * @param {boolean} [context.isAiosCoreRepo] - True if this is the electron-aaos-core repository
 * @returns {string} New installation mode
 */
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_Electron AAOS is ambiguous: it can be FRAMEWORK_DEV (electron-aaos-core repo)
  // or BROWNFIELD (user project with Electron AAOS installed)
  if (legacyType === LegacyProjectType.EXISTING_Electron AAOS) {
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (selectedMode === InstallationMode.FRAMEWORK_DEV && !detected.markers.isAiosCoreRepo) {
      result.warnings.push(
        'Selected framework-dev but this does not appear to be the electron-aaos-core repository.',
      );
    }
