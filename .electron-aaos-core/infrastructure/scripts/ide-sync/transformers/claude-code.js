 ...
 * Target: .claude/commands/Electron AAOS/agents/*.md
 */
...
function transform(agentData) {
  ...
  if (agentData.raw) {
    ...
    const syncFooter = `\n---\n*Electron AAOS Agent - Synced from .electron-aaos-core/development/agents/${agentData.filename}*\n`;

    if (!agentData.raw.includes('Synced from .electron-aaos-core/development/agents/')) {
...
function generateMinimalContent(agentData) {
  ...
  const title = agent.title || 'Electron AAOS Agent';
