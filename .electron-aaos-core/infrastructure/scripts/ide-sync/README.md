...
Automatically synchronizes Electron AAOS agent definitions to IDE command files.

## Overview

IDE Sync keeps agent definitions in `.electron-aaos-core/development/agents/` synchronized with IDE-specific command files in:

- `.claude/commands/Electron AAOS/agents/` (Claude Code)
...
npm run sync:ide
# or
node .electron-aaos-core/infrastructure/scripts/ide-sync/index.js sync
```
...
# or
node .electron-aaos-core/infrastructure/scripts/ide-sync/index.js validate
```
...
# or
node .electron-aaos-core/infrastructure/scripts/ide-sync/index.js validate --strict
```
...
Configure in `.electron-aaos-core/core-config.yaml`:

```yaml
ideSync:
  enabled: true
  source: .electron-aaos-core/development/agents
  targets:
    claude-code:
      enabled: true
      path: .claude/commands/Electron AAOS/agents
    ...
    # ... other IDEs
  redirects:
    electron-aaos-developer: electron-aaos-master
...
```markdown
# Agent Redirect: electron-aaos-developer -> electron-aaos-master

This agent has been renamed. Use `electron-aaos-master` instead.
```

## File Structure

```
.electron-aaos-core/infrastructure/scripts/ide-sync/
