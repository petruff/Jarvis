...
/**
 * Electron AAOS QA Loop Orchestrator
 *
...
const CONFIG = {
  ...
  // Workflow definition
  workflowPath: '.electron-aaos-core/development/workflows/qa-loop.yaml',
  ...
  persistenceIndexPath: '.electron-aaos/qa-loops-index.json', // Track all active loops
};
...
class QALoopOrchestrator {
  ...
  _loadMaxIterations() {
    ...
    const configPaths = [
      path.join(this.rootPath, '.electron-aaos/config.yaml'),
      path.join(this.rootPath, '.electron-aaos/config.yml'),
      path.join(this.rootPath, 'electron-aaos.config.js'),
    ];
...
function printHelp() {
  console.log(`
📊 QA Loop Orchestrator - Electron AAOS QA Evolution (Story 6.5)
