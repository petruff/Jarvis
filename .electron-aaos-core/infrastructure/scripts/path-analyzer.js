...
const REFERENCE_PATTERNS = [
  // Markdown code refs to .electron-aaos-core
  { pattern: /`\.electron-aaos-core\/[^`]+`/g, type: 'code-ref' },
  // Task references
...
const ASSET_DIRS = {
  task: '.electron-aaos-core/development/tasks',
  template: '.electron-aaos-core/product/templates',
  checklist: '.electron-aaos-core/product/checklists',
  script: '.electron-aaos-core/infrastructure/scripts',
  data: '.electron-aaos-core/development/data',
  schema: '.electron-aaos-core/schemas',
  agent: '.electron-aaos-core/development/agents',
};
...
async function findFilesToAnalyze(rootPath) {
  const files = [];
  const dirsToScan = [
    { dir: '.electron-aaos-core/development/agents', type: 'agent' },
    { dir: '.electron-aaos-core/development/tasks', type: 'task' },
    { dir: '.electron-aaos-core/product/templates', type: 'template' },
    { dir: '.electron-aaos-core/product/checklists', type: 'checklist' },
  ];
...
function resolveReference(ref, rootPath) {
  ...
  if (ref.reference.startsWith('.electron-aaos-core/')) {
...
function formatConsoleOutput(report, verbose = false) {
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════════════════════════');
  lines.push('  Electron AAOS Path Analysis Report');
...
Options:
  ...
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.electron-aaos-core'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .electron-aaos-core directory. Run from project root.');
    process.exit(1);
  }
