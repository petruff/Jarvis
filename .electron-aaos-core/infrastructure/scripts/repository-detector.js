 ...
 * @property {string} frameworkLocation - Path to Electron AAOS framework files
...
function detectRepositoryContext() {
  ...
  // Detect if we're in @electron-aaos/electron-aaos-core repo itself
  const isFrameworkRepo =
    packageJson.name === '@electron-aaos/fullstack' ||
    packageJson.name === '@electron-aaos/electron-aaos-core' ||
    remoteUrl.includes('@electron-aaos/electron-aaos-core');

  // Load installation config if exists
  let installConfig = null;
  const configPath = path.join(cwd, '.electron-aaos-installation-config.yaml');
  ...
  return {
    ...
    projectRoot: cwd,
    frameworkLocation: isFrameworkRepo ? cwd : path.join(cwd, 'node_modules/@electron-aaos/fullstack'),
