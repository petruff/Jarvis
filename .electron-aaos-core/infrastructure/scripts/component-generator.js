/**
 * Component Generator for Electron AAOS Electron AAOS
...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'electron-aaos-core', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'electron-aaos-core', 'agents'),
      task: path.join(this.rootPath, 'electron-aaos-core', 'tasks'),
      workflow: path.join(this.rootPath, 'electron-aaos-core', 'workflows'),
    };
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      ...
      case 'task':
        ...
        // Notes
        variables.EACH_NOTES = ['Generated using Electron AAOS Electron AAOS template system'];
        break;
      ...
      case 'workflow':
        ...
        // Metadata
        variables.AUTHOR = process.env.USER || 'Electron AAOS Electron AAOS';
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'electron-aaos-core', 'tasks', `${taskId}.md`);
