 ...
 *
 * @module llm-routing
 * @location .electron-aaos-core/infrastructure/scripts/llm-routing/
 */
...
function updateClaudeConfig(trackingEnabled = true) {
  ...
  try {
    ...
    config.electron-aaosLLMRouting = {
...
function getInstallationSummary(result) {
  ...
  if (result.success) {
    ...
    summary.push('');
    summary.push('Usage data saved to: ~/.electron-aaos/usage-tracking/deepseek-usage.json');
    summary.push('');
  } else {
    summary.push('');
...
if (require.main === module) {
  console.log('\n🚀 Electron AAOS LLM Routing Installer\n');
