...
Base layer of the Electron AAOS modular architecture. Contains tools, integrations, scripts, and PM adapters.
...
- `ComponentGenerator` - Electron AAOS component generation
- `BatchCreator` - Batch operations

### Validation
- `AiosValidator` - Electron AAOS component validation
...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.electron-aaos-core/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.electron-aaos-core/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.electron-aaos-core/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.electron-aaos-core/infrastructure');
