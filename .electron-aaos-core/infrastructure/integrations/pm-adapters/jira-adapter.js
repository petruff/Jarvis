...
class JiraAdapter extends PMAdapter {
   ...
   * @param {string} config.project_key - Project key (e.g., "Electron AAOS")
   */
  constructor(config) {
    super(config);
  ...
  async pullStory(storyId) {
    try {
      ...
      // Map Jira status to Electron AAOS status
      const statusMapping = {
        'To Do': 'Draft',
  ...
  async updateStatus(storyId, status) {
    try {
      ...
      // Map Electron AAOS status to Jira transition
   ...
   * Get Jira issue
   * @private
   * @param {string} issueKey - Issue key (e.g., "Electron AAOS-123")
