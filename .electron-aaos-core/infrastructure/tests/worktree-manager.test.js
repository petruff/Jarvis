...
describe('WorktreeManager', () => {
  ...
  beforeEach(async () => {
    ...
    testRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'electron-aaos-worktree-test-'));
  ...
  describe('constructor', () => {
    it('should use default options when none provided', () => {
      ...
      expect(defaultManager.worktreeDir).toBe('.electron-aaos/worktrees');
  ...
  describe('getWorktreePath', () => {
    it('should return correct path for story', () => {
      ...
      expect(wtPath).toBe(path.join(testRoot, '.electron-aaos/worktrees', 'STORY-42'));
    });
  });
