#!/usr/bin/env node
/**
 * Electron AAOS Session End Hook for Gemini CLI
 * Story GEMINI-INT.6 - Electron AAOS Hooks System
 *
...
async function sessionEnd() {
  ...
  try {
    const sessionDir = path.join(projectDir, '.electron-aaos', 'sessions');
    ...
    let modifiedFiles = [];
    const sessionFilesPath = path.join(projectDir, '.electron-aaos', 'session-files.json');
    ...
    let toolCount = 0;
    const toolLogPath = path.join(projectDir, '.electron-aaos', 'logs', 'tool-results.jsonl');
