#!/usr/bin/env node
/**
 * Electron AAOS Rewind Handler
 * Story GEMINI-INT.14 - /rewind Integration
 *
 * Syncs Gemini's /rewind command with Electron AAOS state.
 */
...
async function handleRewind() {
  ...
  // Sync with Electron AAOS memory layer
  try {
    const memoryDir = path.join(projectDir, '.electron-aaos', 'memory');
    ...
    const logDir = path.join(projectDir, '.electron-aaos', 'logs');
