 ...
 */
const EVENT_MAPPING = {
  // Electron AAOS Event -> { claude: ClaudeEvent, gemini: GeminiEvent }
...
class UnifiedHook {
   ...
   * @param {string} config.event - Electron AAOS event name
  ...
  toClaudeConfig() {
    ...
    // Runners not yet implemented - path .electron-aaos-core/hooks/unified/runners/ does not exist
    return null;

    // return {
    //   event: claudeEvent,
    //   matcher: this.matcher,
    //   handler: `node .electron-aaos-core/hooks/unified/runners/claude-runner.js ${this.name}`,
    // };
  }
  ...
  toGeminiConfig() {
    ...
    //       name: this.name,
    //       type: 'command',
    //       command: `node .electron-aaos-core/hooks/unified/runners/gemini-runner.js ${this.name}`,
