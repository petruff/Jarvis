...
**Module:** `.electron-aaos-core/hooks/unified`
...
Hook Runners (runners/*.js)
         ↓
Application Logic (electron-aaos-core, electron-aaos-pro)
```
...
- **`precompact-runner.js`** (Story MIS-3)
  ...
  - Open Core architecture (delegates to electron-aaos-pro)
...
pro/memory/session-digest/extractor.js
         ↓
.electron-aaos/session-digests/{session-id}-{timestamp}.yaml
```
...
- If electron-aaos-pro not available: no-op (log and return)
...
```javascript
// .electron-aaos-core/hooks/unified/runners/my-runner.js
...
const { onMyEvent } = require('../../.electron-aaos-core/hooks/unified/runners/my-runner');
...
if (!isProAvailable()) {
  console.log('[Hook] electron-aaos-pro not available, skipping');
  return;
}
...
- **Story PRO-5:** electron-aaos-pro Repository Bootstrap (pro-detector pattern)

---

*Unified Hooks System - Electron AAOS Core*
