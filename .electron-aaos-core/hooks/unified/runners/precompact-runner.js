 ...
 * - electron-aaos-core: Hook runner (this file) + pro detection
 * - electron-aaos-pro: Digest extraction logic (memory/session-digest/)
 *
 * @module .electron-aaos-core/hooks/unified/runners/precompact-runner
 * @see Story MIS-3 - Session Digest (PreCompact Hook)
 * @see Story PRO-5 - electron-aaos-pro Repository Bootstrap
 */
 ...
 *
 * Execution flow:
 * 1. Detect electron-aaos-pro availability via pro-detector
...
async function onPreCompact(context) {
  try {
    // 1. Detect electron-aaos-pro availability
    const proAvailable = isProAvailable();

    if (!proAvailable) {
      console.log('[PreCompact] electron-aaos-pro not available, skipping session digest');
    ...
    setImmediate(async () => {
      try {
        // Dynamic import from electron-aaos-pro
        ...
        if (!digestModule || typeof digestModule.extractSessionDigest !== 'function') {
          console.error('[PreCompact] Digest extractor not found or invalid in electron-aaos-pro');
          return;
        }
