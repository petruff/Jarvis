...
Electron AAOS Presets are pre-configured bundles that provide ready-to-use configurations for specific use cases, project types, or team structures. Presets simplify the initial setup process and ensure consistent configuration across projects.
...
| ------------ | ----------------------- | ------------------ | ----------------- |
| `minimal`    | Bare minimum Electron AAOS setup | dev                | Quick prototyping |
...
\`\`\`bash
electron-aaos install --preset my-preset
\`\`\`
```
...
electron-aaos install --preset standard

# Install with multiple presets (layered)
electron-aaos install --preset standard --preset frontend

# Install custom preset from directory
electron-aaos install --preset ./presets/my-preset
```
...
electron-aaos presets list

# List with details
electron-aaos presets list --verbose
```
...
electron-aaos presets show standard

# Compare two presets
electron-aaos presets diff minimal standard
```
...
electron-aaos presets export --name my-project-preset --output ./presets/

# Export with description
electron-aaos presets export --name my-preset --description "Project template" --output ./presets/
```
...
```bash
electron-aaos install --preset standard --preset frontend --preset small-team
```
...
2. **Review before install** - Use `electron-aaos presets show` to understand what you're getting
3. **Export before modifying** - Save your config as a preset before major changes
4. **Keep presets updated** - Check for preset updates when upgrading Electron AAOS
...
_Last Updated: 2026-01-28 | Electron AAOS Framework Team_
