 ...
 *   - .electron-aaos-core/project-config.yaml (L2) — team-shared project settings
 *   - ~/.electron-aaos/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.electron-aaos/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.electron-aaos-core/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, '.electron-aaos-core', 'core-config.yaml');
  const frameworkPath = path.join(projectRoot, '.electron-aaos-core', 'framework-config.yaml');
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, '.electron-aaos-core', 'core-config.yaml');
  const backupPath = path.join(projectRoot, '.electron-aaos-core', 'core-config.backup.yaml');
...
/**
 * Ensure the ~/.electron-aaos/ directory exists.
 *
 * @returns {string} Path to ~/.electron-aaos/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), '.electron-aaos');
...
/**
 * Write user config to ~/.electron-aaos/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), '.electron-aaos', 'user-config.yaml');
...
/**
 * Write project config to .electron-aaos-core/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, '.electron-aaos-core', 'project-config.yaml');
