 ...
 */
const CONFIG_FILES = {
  framework: '.electron-aaos-core/framework-config.yaml',
  project: '.electron-aaos-core/project-config.yaml',
  pro: 'pro/pro-config.yaml',
  local: '.electron-aaos-core/local-config.yaml',
  legacy: '.electron-aaos-core/core-config.yaml',
  user: path.join(os.homedir(), '.electron-aaos', 'user-config.yaml'),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  if (options.appDir) {
    const appConfigPath = path.join(options.appDir, 'electron-aaos-app.config.yaml');
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.electron-aaos/user-config.yaml)
...
function loadLegacyConfig(projectRoot) {
  ...
  const suppressDeprecation = process.env.Electron AAOS_SUPPRESS_DEPRECATION === 'true'
    || process.env.Electron AAOS_SUPPRESS_DEPRECATION === '1';

  if (!suppressDeprecation) {
    warnings.push(
      '[DEPRECATION] Monolithic core-config.yaml detected. '
      + 'Run `electron-aaos config migrate` to split into layered config files. '
      + 'Monolithic format will be removed in v4.0.0. '
      + 'Set Electron AAOS_SUPPRESS_DEPRECATION=true to silence this warning.',
    );
  }
...
function getConfigAtLevel(projectRoot, level, options = {}) {
  ...
  switch (level) {
    ...
    case 'app': case '3': case 'L3':
      ...
      relativePath = path.join(options.appDir, 'electron-aaos-app.config.yaml');
      break;
...
/**
 * Ensure the ~/.electron-aaos/ directory exists with secure permissions.
 *
...
/**
 * Set a value in the user config file (~/.electron-aaos/user-config.yaml).
