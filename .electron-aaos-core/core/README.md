# Electron AAOS Core Module

> Central runtime module providing essential framework functionality for Electron AAOS Electron AAOS.
...
The Core module contains the foundational runtime components that all other Electron AAOS modules depend on. It provides configuration management, session handling, elicitation workflows, and essential utilities.

## Installation

The core module is automatically available within the Electron AAOS Electron AAOS framework:

```javascript
// CommonJS
const core = require('./.electron-aaos-core/core');

// ES Modules
import { ConfigCache, ElicitationEngine } from './.electron-aaos-core/core/index.esm.js';
```
...
│   ├── electron-aaos-kb.md          # Electron AAOS knowledge base
...
```javascript
const { globalConfigCache } = require('./.electron-aaos-core/core');
...
```javascript
const { loadAgentConfig } = require('./.electron-aaos-core/core');
...
```javascript
const { loadConfigSections } = require('./.electron-aaos-core/core');
...
```javascript
const { ContextDetector } = require('./.electron-aaos-core/core');
...
```javascript
const { SessionContextLoader } = require('./.electron-aaos-core/core');
...
```javascript
const { ElicitationEngine } = require('./.electron-aaos-core/core');
...
```javascript
const { ElicitationSessionManager } = require('./.electron-aaos-core/core');
...
```javascript
const { YAMLValidator, validateYAML } = require('./.electron-aaos-core/core');
...
```javascript
const { PersonalizedOutputFormatter } = require('./.electron-aaos-core/core');
...
*Electron AAOS Electron AAOS Core Module v2.0.0*
