 ...
 * Scans the .electron-aaos-core directory structure to build comprehensive manifests.
 *
...
class ManifestGenerator {
  constructor(options = {}) {
    ...
    this.electron-aaosCoreDir = path.join(this.basePath, '.electron-aaos-core');
    this.manifestDir = path.join(this.electron-aaosCoreDir, 'manifests');
    this.version = '2.1.0';
  }
  ...
  async generateAgentsManifest() {
    const agentsDir = path.join(this.electron-aaosCoreDir, 'development', 'agents');
        ...
        try {
          ...
          if (parsed && parsed.agent) {
            ...
            agents.push({
              ...
              status: 'active',
              file_path: `.electron-aaos-core/development/agents/${file}`,
  ...
  async generateWorkersManifest() {
    const registryPath = path.join(this.electron-aaosCoreDir, 'core', 'registry', 'service-registry.json');
  ...
  async generateTasksManifest() {
    const tasksDir = path.join(this.electron-aaosCoreDir, 'development', 'tasks');
      ...
      for (const file of mdFiles) {
        try {
          ...
          tasks.push({
            ...
            file_path: `.electron-aaos-core/development/tasks/${file}`,
            status: 'active',
          });
