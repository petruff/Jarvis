...
class PermissionMode {
  ...
  constructor(projectRoot = process.cwd()) {
    ...
    this.configPath = path.join(projectRoot, '.electron-aaos', 'config.yaml');
  ...
  async _saveToConfig(mode) {
    ...
    const electron-aaosDir = path.dirname(this.configPath);
    await fs.mkdir(electron-aaosDir, { recursive: true });
