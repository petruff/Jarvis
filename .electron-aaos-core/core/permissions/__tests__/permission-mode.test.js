...
describe('PermissionMode', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'electron-aaos-test-'));
    await fs.mkdir(path.join(tempDir, '.electron-aaos'), { recursive: true });
  ...
  describe('load()', () => {
    ...
    it('should load mode from config file', async () => {
      await fs.writeFile(
        path.join(tempDir, '.electron-aaos', 'config.yaml'),
        'permissions:\n  mode: auto\n',
      );
  ...
  describe('setMode()', () => {
    it('should set mode and persist to config', async () => {
      ...
      // Verify persisted
      const configContent = await fs.readFile(path.join(tempDir, '.electron-aaos', 'config.yaml'), 'utf-8');
