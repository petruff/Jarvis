...
class IdeationEngine {
  constructor(config = {}) {
    ...
    // Output paths
    this.outputDir = config.outputDir || path.join(this.rootPath, '.electron-aaos', 'ideation');
  ...
  formatMarkdown(result) {
    ...
    md += '---\n*Generated by Electron AAOS Ideation Engine*\n';
  ...
  checkSyncOperations() {
    ...
    try {
      ...
      const result = execSync(
        `grep -rn "readFileSync\\|writeFileSync\\|existsSync" --include="*.js" --include="*.ts" ${this.rootPath}/src ${this.rootPath}/.electron-aaos-core 2>/dev/null || true`,
        {
          encoding: 'utf8',
...
class CodeQualityAnalyzer {
  ...
  checkLongFunctions() {
    ...
    try {
      const result = execSync(
        `wc -l ${this.rootPath}/src/**/*.js ${this.rootPath}/.electron-aaos-core/**/*.js 2>/dev/null | sort -rn | head -10 || true`,
        {
          encoding: 'utf8',
        },
      );
