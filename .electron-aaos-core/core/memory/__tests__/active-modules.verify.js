/**
 * Verification Tests for Active Electron AAOS Memory Modules
 *
...
test('merge-rules.yaml exists in .electron-aaos', () => {
  const rulesPath = path.join(process.cwd(), '.electron-aaos', 'merge-rules.yaml');
  const exists = fs.existsSync(rulesPath);

  assertTrue(exists, 'merge-rules.yaml should exist in .electron-aaos');
});
