 ...
 *
 * @module @electron-aaos/electron-aaos-core/health-check/checks/services/gemini-cli
 * @version 1.0.0
...
class GeminiCliCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      // Check for rules.md (Electron AAOS rules)
      try {
      ...
      // Check for Electron AAOS agents
      try {
        const agentsDir = path.join(geminiDir, 'rules', 'Electron AAOS', 'agents');
        const agentFiles = await fs.readdir(agentsDir);
        details.electron-aaosAgents = agentFiles.filter((f) => f.endsWith('.md')).length;
      } catch {
        details.electron-aaosAgents = 0;
      }
    } catch {
      // No project config
    }
    ...
    if (details.projectConfig && details.electron-aaosAgents === 0) {
      warnings.push('No Electron AAOS agents installed for Gemini CLI');
    }
    ...
    if (issues.length > 0) {
      return this.warning(`Gemini CLI needs attention: ${issues.join(', ')}`, {
        recommendation: issues.includes('Not authenticated')
          ...
          : 'Run `npx electron-aaos-core install` and select Gemini CLI',
    ...
    if (warnings.length > 0) {
      return this.warning(`Gemini CLI configuration incomplete: ${warnings.join(', ')}`, {
        recommendation: 'Run `npx electron-aaos-core install` and select Gemini CLI',
    ...
    if (details.electron-aaosAgents > 0) parts.push(`${details.electron-aaosAgents} Electron AAOS agents`);
