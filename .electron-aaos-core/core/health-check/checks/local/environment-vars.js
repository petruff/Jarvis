 ...
 *
 * @module @electron-aaos/electron-aaos-core/health-check/checks/local/environment-vars
 * @version 1.0.0
...
/**
 * Electron AAOS-specific environment variables
 */
const Electron AAOS_VARS = ['Electron AAOS_DEBUG', 'Electron AAOS_LOG_LEVEL'];
...
class EnvironmentVarsCheck extends BaseCheck {
  ...
  async execute(_context) {
    ...
    // Check Electron AAOS-specific vars (info only)
    for (const varName of Electron AAOS_VARS) {
      if (process.env[varName]) {
        electron-aaosVars.push({ name: varName, value: this.maskValue(process.env[varName]) });
      }
    }
