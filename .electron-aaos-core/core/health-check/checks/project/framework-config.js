 ...
 *
 * Verifies that Electron AAOS framework configuration files are present.
 *
 * @module @electron-aaos/electron-aaos-core/health-check/checks/project/framework-config
 * @version 1.0.0
...
const REQUIRED_CONFIGS = [
  { path: '.electron-aaos-core', type: 'directory', description: 'Electron AAOS core framework' },
...
const RECOMMENDED_CONFIGS = [
  { path: '.electron-aaos', type: 'directory', description: 'Electron AAOS local configuration' },
...
class FrameworkConfigCheck extends BaseCheck {
  constructor() {
    super({
      ...
      description: 'Verifies Electron AAOS framework configuration is present',
      domain: CheckDomain.PROJECT,
      severity: CheckSeverity.HIGH,
      timeout: 3000,
      cacheable: true,
      healingTier: 0, // Cannot auto-fix framework setup
      tags: ['electron-aaos', 'config', 'framework'],
    });
  }
  ...
  async execute(context) {
    ...
    if (missingRequired.length > 0) {
      ...
      return this.fail(`Missing required framework configuration: ${missing}`, {
        recommendation: 'Run Electron AAOS setup or manually create missing directories',
        details: {
    ...
    if (missingRecommended.length > 0) {
      ...
      return this.warning(`Missing recommended configuration: ${missing}`, {
        recommendation: 'Consider adding recommended configuration for full Electron AAOS functionality',
        details: {
