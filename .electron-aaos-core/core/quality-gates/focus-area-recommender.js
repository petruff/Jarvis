...
class FocusAreaRecommender {
  ...
  analyzeChangedFiles(changedFiles = []) {
    ...
    const categoryPatterns = [
      ...
      },
      {
        ...
        category: 'electron-aaos-core',
        highlight: 'Electron AAOS framework changes',
        risk: 'high',
      },
      {
  ...
  determineSecondaryAreas(fileAnalysis, _layer2Result) {
    ...
    const secondaryCategories = ['ux', 'configuration', 'performance', 'electron-aaos-core'];
  ...
  getReviewQuestions(category) {
    const questionBank = {
      ...
      'electron-aaos-core': [
        'Does this follow Electron AAOS framework patterns?',
