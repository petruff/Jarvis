...
The Entity Registry is the central data store for the Incremental Development System (IDS). It tracks all Electron AAOS framework artifacts — tasks, templates, scripts, modules, agents, checklists, and data files — with metadata, relationships, adaptability scores, and checksums.

## Schema

The registry is stored at `.electron-aaos-core/data/entity-registry.yaml` as a single YAML file.
...
categories:
  - id: "tasks"
    ...
    basePath: ".electron-aaos-core/development/tasks"
```
...
```bash
node .electron-aaos-core/development/scripts/populate-entity-registry.js
```
