# Electron AAOS Electron AAOS Meta-Agent Troubleshooting Guide

## Overview

This guide helps diagnose and resolve common issues when using the Electron AAOS Electron AAOS meta-agent for component creation and management.
...
**Symptoms:**
```
❌ Error: Agent 'data-analyst' already exists at /electron-aaos-core/agents/data-analyst.md
```
...
3. If overwriting is intended:
   ...
   rm electron-aaos-core/agents/data-analyst.md
   *create-agent
   ```
...
1. Verify template location:
   ```bash
   ls electron-aaos-core/templates/
...
3. Check working directory:
   ```bash
   pwd
   # Should be in @electron-aaos/electron-aaos-core root
   ```
...
1. Create session directory:
   ```bash
   mkdir -p electron-aaos-core/.sessions
   ```

2. Check permissions:
   ```bash
   chmod 755 electron-aaos-core/.sessions
   ```
...
2. Check transaction directory:
   ```bash
   ls electron-aaos-core/logs/transactions/
   ```
...
1. Manual cleanup:
   ```bash
   # Check failed files
   ls -la electron-aaos-core/agents/file1.md
   
   # Remove manually if needed
   rm electron-aaos-core/agents/file1.md
   ```
...
3. Restore from backup:
   ```bash
   # Check backups
   ls electron-aaos-core/logs/transactions/txn-*/backups/
   ```
...
1. Check failure details:
   ...
   cat electron-aaos-core/logs/transactions/latest.json
   ```
...
3. Clear caches:
   ```bash
   rm -rf electron-aaos-core/.cache/
   ```
...
# Specific modules
DEBUG=electron-aaos:template,electron-aaos:elicitation *create-agent
...
```bash
# Application logs
tail -f electron-aaos-core/logs/electron-aaos-developer.log

# Transaction logs
ls -la electron-aaos-core/logs/transactions/

# Error logs
grep ERROR electron-aaos-core/logs/*.log
```
...
- Template syntax: `electron-aaos-core/docs/template-syntax.md`
- Creation guide: `electron-aaos-core/docs/component-creation-guide.md`
- API reference: `electron-aaos-core/docs/api-reference.md`
...
1. **Backup current state**:
   ```bash
   tar -czf electron-aaos-backup.tar.gz electron-aaos-core/
   ```

2. **Reset to clean state**:
   ```bash
   git checkout -- electron-aaos-core/
   npm run setup
   ```
