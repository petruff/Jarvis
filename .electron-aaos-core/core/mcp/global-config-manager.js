 ...
 *
 * Manages the global MCP configuration at ~/.electron-aaos/mcp/
...
/**
 * Check if global Electron AAOS directory exists
 * @returns {boolean}
 */
...
function addServer(serverName, serverConfig = null) {
  ...
  if (!config) {
    return { success: false, error: 'Global config not found. Run "electron-aaos mcp setup" first.' };
  }
