 ...
 * @returns {string} Path to .electron-aaos-core/tools/mcp
 */
function getProjectMcpPath(projectRoot = process.cwd()) {
  return path.join(projectRoot, '.electron-aaos-core', 'tools', 'mcp');
}
...
function createLink(projectRoot = process.cwd(), options = {}) {
  ...
  if (!fs.existsSync(globalPath)) {
    return {
      success: false,
      error: 'Global MCP directory does not exist. Run "electron-aaos mcp setup" first.',
      linkPath,
      globalPath,
    };
  }
