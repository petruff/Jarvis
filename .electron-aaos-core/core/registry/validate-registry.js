...
async function validateFields(registryPath) {
  ...
  for (const worker of registry.workers) {
    ...
    // Check path format
    if (worker.path && !worker.path.startsWith('.electron-aaos-core/')) {
      issues.push({ id: worker.id, issue: 'Invalid path prefix (must start with .electron-aaos-core/)' });
    }
  }
...
async function main() {
  ...
  const registryPath = path.join(baseDir, '.electron-aaos-core/core/registry/service-registry.json');
  const schemaPath = path.join(baseDir, '.electron-aaos-core/core/registry/registry-schema.json');
