 ...
 *
 * Provides async API for spawning Electron AAOS agents in separate terminals
...
async function createContextFile(context, outputDir = os.tmpdir()) {
  ...
  const contextPath = path.join(outputDir, `electron-aaos-context-${Date.now()}.json`);
...
async function spawnInline(agent, task, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn('bash', [scriptPath, ...args], {
      env: {
        ...process.env,
        Electron AAOS_DEBUG: opts.debug ? 'true' : 'false',
        Electron AAOS_OUTPUT_DIR: opts.outputDir,
        Electron AAOS_INLINE_MODE: 'true', // Signal to pm.sh that we're running inline
      },
...
async function cleanupOldFiles(outputDir = os.tmpdir(), maxAgeMs = 3600000) {
  ...
  try {
    ...
    const electron-aaosFiles = files.filter(
      (f) => f.startsWith('electron-aaos-output-') || f.startsWith('electron-aaos-lock-') || f.startsWith('electron-aaos-context-'),
    );

    for (const file of electron-aaosFiles) {
...
function formatCompatibilityReport(report) {
  const lines = [
    '═══════════════════════════════════════════════════════════════',
    '                 Electron AAOS Terminal Spawner Compatibility Report     ',
    '═══════════════════════════════════════════════════════════════',
    '',
