/**
 * Skill Dispatcher - Dispatches to Electron AAOS Skills instead of generic Task calls
 *
 ...
 *
 * Responsibilities:
 * - Map agent IDs to Electron AAOS Skill names
 ...
 * @property {string} skill - Full skill name (e.g., 'Electron AAOS:agents:architect')
...
/**
 * Dispatches workflow phases to Electron AAOS Skills
 */
/**
...
class SkillDispatcher {
  ...
  constructor(options = {}) {
     ...
     * These correspond to files in .claude/commands/Electron AAOS/agents/
     */
    this.skillMapping = {
      // Core development agents
      architect: 'Electron AAOS:agents:architect',
      'data-engineer': 'Electron AAOS:agents:data-engineer',
      dev: 'Electron AAOS:agents:dev',
      qa: 'Electron AAOS:agents:qa',

      // Product agents
      pm: 'Electron AAOS:agents:pm',
      po: 'Electron AAOS:agents:po',
      sm: 'Electron AAOS:agents:sm',
      analyst: 'Electron AAOS:agents:analyst',

      // Specialized agents
      'ux-design-expert': 'Electron AAOS:agents:ux-design-expert',
      'ux-expert': 'Electron AAOS:agents:ux-design-expert', // Alias
      devops: 'Electron AAOS:agents:devops',
      'github-devops': 'Electron AAOS:agents:devops', // Alias

      // Master agent
      'electron-aaos-master': 'Electron AAOS:agents:electron-aaos-master',
    };
    ...
    this.agentPersonas = {
      ...
      'electron-aaos-master': { name: 'Orion', title: 'Master Orchestrator' },
    };
  }
   ...
   * @returns {string} Full skill name (e.g., 'Electron AAOS:agents:architect')
   */
  getSkillName(agentId) {
    return this.skillMapping[agentId] || `Electron AAOS:agents:${agentId}`;
  }
  ...
  getAgentPersona(agentId) {
    return (
      this.agentPersonas[agentId] || { name: agentId, title: 'Electron AAOS Agent' }
    );
  }
  ...
  isValidAgent(agentId) {
    return agentId in this.skillMapping || agentId.startsWith('Electron AAOS:');
  }
}
