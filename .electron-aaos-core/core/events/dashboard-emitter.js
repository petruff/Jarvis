...
const MONITOR_SERVER_URL = process.env.Electron AAOS_MONITOR_URL || 'http://localhost:4001/events';
...
class DashboardEmitter {
  ...
  constructor() {
    ...
    this.fallbackPath = path.join(this.projectRoot, '.electron-aaos', 'dashboard', 'events.jsonl');
  ...
  async emit(type, data = {}) {
    ...
    const event = {
      ...
      electron-aaos_agent: this.currentAgent || undefined,
      electron-aaos_story_id: this.currentStoryId || undefined,
      data,
    };
      ...
      const response = await fetch(MONITOR_SERVER_URL, {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            electron-aaos_agent: event.electron-aaos_agent,
            electron-aaos_story_id: event.electron-aaos_story_id,
          },
        }),
