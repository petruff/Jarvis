# Electron AAOS Electron AAOS Development Rules for Gemini CLI
...
### Skills Integration
Electron AAOS agents are installed as Gemini CLI skills in `.gemini/rules/Electron AAOS/agents/`.
...
### Hooks Configuration
Electron AAOS can integrate with Gemini CLI hooks for enhanced context injection.
...
```json
{
  "hooks": {
    "BeforeAgent": [{
      "matcher": "*",
      "hooks": [{
        "name": "electron-aaos-context",
        "type": "command",
        "command": "node .electron-aaos-core/hooks/gemini/before-agent.js"
      }]
    }]
  }
}
...
- `SessionStart` - Load Electron AAOS context
...
### Extensions
Electron AAOS can be packaged as a Gemini CLI extension for easy installation:
```bash
gemini extensions install github.com/electron-aaos/electron-aaos-gemini-extension
```
...
---
*Electron AAOS Electron AAOS Gemini CLI Configuration v1.0*
