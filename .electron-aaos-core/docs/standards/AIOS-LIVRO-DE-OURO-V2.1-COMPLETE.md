# Electron AAOS Framework - Livro de Ouro v2.1 (Complete)
...
**Mantido Por:** Electron AAOS Framework Team + Community
**Repositório Principal:** `Electron AAOSAI/electron-aaos-core`

---

> **"Structure is Sacred. Tone is Flexible."**
> _— Fundamento filosófico do Electron AAOS_
...
- ✅ **npm Package Scoping** (@electron-aaos/core, @electron-aaos/squad-\*, @electron-aaos/mcp-presets)

**Referências Legadas:**

- `Electron AAOS-LIVRO-DE-OURO.md` - Base v2.0.0 (Jan 2025)
- `Electron AAOS-LIVRO-DE-OURO-V2.1.md` - Delta parcial
- `Electron AAOS-LIVRO-DE-OURO-V2.1-SUMMARY.md` - Resumo de mudanças
...
│                                                                         │
│   ┌────────────────────────────────────────────────────────────────┐   │
│   │  Electron AAOSAI/electron-aaos-core (Commons Clause)                         │   │
│   │  ─────────────────────────────────────                         │   │
...
│   ▼                          │                          ▼              │
│   ┌─────────────────────┐    │    ┌─────────────────────────────┐     │
│   │ Electron AAOSAI/         │    │    │ Electron AAOSAI/mcp-ecosystem    │     │
│   │ electron-aaos-squads (MIT)   │    │    │ (Apache 2.0)                │     │
│   │ ─────────────────   │    │    │ ──────────────────────────  │     │
│   │ • ETL Squad         │    │    │ • Docker MCP Toolkit        │     │
│   │ • Creator Squad     │    │    │ • IDE Configurations        │     │
│   │ • MMOS Squad        │    │    │ • MCP Presets               │     │
│   │ npm: @electron-aaos/squad-*  │    │    │ npm: @electron-aaos/mcp-presets      │     │
│   └─────────────────────┘    │    └─────────────────────────────┘     │
│                              │                                         │
...
│                              │                                         │
│   ┌─────────────────────┐    │    ┌─────────────────────────────┐     │
│   │ Electron AAOSAI/mmos     │    │    │ Electron AAOSAI/certified-       │     │
...
| **Electron AAOS v2.1** | ✅ **Complete**          | ✅ **Clones (DNA Mental™)** ⭐ |

**Analogia:** Linux é open source, mas Red Hat Enterprise Linux adiciona suporte e otimizações. Ambos são Linux, mas o valor agregado varia. Electron AAOS funciona igual.
...
- 📋 **Layer 3: USAGE GUIDE** - Como usar Electron AAOS v2.1 no seu contexto
...
## Bem-vindo ao Electron AAOS v2.1 - Vamos Encontrar Seu Caminho
...
## Essay 1: Por Que Electron AAOS Existe
...
Electron AAOS fornece **orquestração estruturada**:
...
| **Pax**   | `electron-aaos-master`   | Orchestrator | Framework orchestration |
...
@electron-aaos-master     # Ativa Pax (Orchestrator)
...
|---------|----------|------------|---------|
| `@electron-aaos/core` | npm public | - | Commons Clause |
| `@electron-aaos/squad-etl` | npm public | @electron-aaos/core | MIT |
| `@electron-aaos/squad-creator` | npm public | @electron-aaos/core | MIT |
| `@electron-aaos/squad-mmos` | npm public | @electron-aaos/core | MIT |
| `@electron-aaos/mcp-presets` | npm public | - | Apache 2.0 |
...
$ npx @Electron AAOSAI/electron-aaos@latest init

# Existing project (Brownfield)
$ npx @Electron AAOSAI/electron-aaos migrate v2.0-to-v2.1
```
...
$ electron-aaos stories create

# Execute a task
$ electron-aaos task develop-story --story=1.1
```
...
```bash
# Clone all repos
mkdir -p ~/Workspaces/Electron AAOS && cd ~/Workspaces/Electron AAOS
gh repo clone Electron AAOSAI/electron-aaos-core
gh repo clone Electron AAOSAI/electron-aaos-squads
gh repo clone Electron AAOSAI/mcp-ecosystem

# Link for local development
cd electron-aaos-core && npm install && npm link
cd ../electron-aaos-squads && npm install && npm link @electron-aaos/core

# VS Code workspace
code electron-aaos-workspace.code-workspace
```
...
```bash
# Search for workers
$ electron-aaos workers search "json parse"
...
# Get worker details
$ electron-aaos workers info json-parser

# Use worker in task
$ electron-aaos workers use json-parser --task my-task
```
...
| `brownfield-integration` | Add Electron AAOS to existing    | dev, architect    |
...
├── .electron-aaos-core/                        # Framework layer
│   ├── core/                          # Core module
...
**Mantido Por:** Electron AAOS Framework Team

---

_Este documento consolida Electron AAOS-LIVRO-DE-OURO.md (v2.0) + deltas v2.1 em um único documento completo._
