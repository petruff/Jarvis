 ...
 *
 * CLI command for validating Electron AAOS-Core installation integrity.
 ...
 *
 * Usage:
 *   electron-aaos validate                    # Validate current installation
 *   electron-aaos validate --repair           # Repair missing/corrupted files
 *   electron-aaos validate --repair --dry-run # Preview repairs without applying
 *   electron-aaos validate --detailed         # Show detailed file list
 *   electron-aaos validate --json             # Output as JSON
 */
...
function createValidateCommand() {
  ...
  validate
    .description('Validate Electron AAOS-Core installation integrity')
...
async function runValidation(options) {
  ...
  // Check if Electron AAOS-Core is installed
    ...
    if (options.json) {
      console.log(
        JSON.stringify(
          {
            status: 'failed',
            error: 'Electron AAOS-Core not found in current directory',
            // SECURITY: Sanitize path - only show relative indicator
            location: '.electron-aaos-core',
          },
          null,
          2
        )
      ...
      console.error(chalk.dim(`Expected at: ${electron-aaosCoreDir}`));
      console.error(chalk.dim('\nRun `npx electron-aaos-core install` to install Electron AAOS-Core'));
    }
  ...
  if (sourceDir) {
    const sourceManifest = path.join(sourceDir, '.electron-aaos-core', 'install-manifest.yaml');
  ...
  if (!sourceDir && options.repair) {
    ...
    const possibleSources = [
      ...
      path.join(projectRoot, 'node_modules/electron-aaos-core'),
      path.join(projectRoot, 'node_modules/@electron-aaos/electron-aaos-core'),
    ];

    for (const src of possibleSources) {
      if (fs.existsSync(path.join(src, '.electron-aaos-core', 'install-manifest.yaml'))) {
        sourceDir = src;
        break;
      }
    }
