 ...
 *
 * Subcommands:
 *   electron-aaos config show [--level <n>] [--app <name>] [--debug]
 *   electron-aaos config diff --levels <a>,<b>
 *   electron-aaos config migrate [--dry-run] [--force]
 *   electron-aaos config validate [--level <n>]
...
// Resolve core config modules (relative from .electron-aaos-core/cli/commands/config/)
...
function migrateAction(options) {
  ...
  try {
    ...
    if (originalStr === resolvedStr) {
      ...
      console.log('This may be expected due to key normalization. Run `electron-aaos config diff` to inspect.');
    }
...
function splitL1(config) {
  ...
  l1.metadata = { name: 'Electron AAOS Electron AAOS', framework_version: '3.12.0' };
...
function initLocalAction() {
  const root = getProjectRoot();
  const templatePath = path.join(root, '.electron-aaos-core', 'local-config.yaml.template');
  const targetPath = path.join(root, '.electron-aaos-core', 'local-config.yaml');

  if (!fs.existsSync(templatePath)) {
    console.error('Template not found: .electron-aaos-core/local-config.yaml.template');
    process.exit(1);
  }
  ...
  console.log('Created: .electron-aaos-core/local-config.yaml (from template)');
...
/**
 * Create the `electron-aaos config` command with all subcommands.
 * @returns {Command}
 */
