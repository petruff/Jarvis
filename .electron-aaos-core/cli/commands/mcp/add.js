...
async function executeAdd(server, options) {
  ...
  if (options.listTemplates) {
    ...
    console.log('Usage: electron-aaos mcp add <server-name>');
    console.log('Example: electron-aaos mcp add context7');
    return;
  }
  ...
  if (!globalConfigExists()) {
    ...
    console.log('Run "electron-aaos mcp setup" first to create global configuration.');
    process.exit(1);
  }
  ...
  if (!server) {
    ...
    console.log('  electron-aaos mcp add myserver --type sse --url https://example.com/mcp');
    console.log('  electron-aaos mcp add myserver --command npx --args "-y,@scope/mcp-server"');
    process.exit(1);
  }
  ...
  if (result.success) {
    ...
    if (result.error.includes('No template available')) {
      ...
      console.log(`  electron-aaos mcp add ${server} --type sse --url <url>`);
      console.log(`  electron-aaos mcp add ${server} --command npx --args "-y,@scope/server"`);
    }
