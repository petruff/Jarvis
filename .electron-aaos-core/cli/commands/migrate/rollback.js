 ...
 */
async function removeV21Structure(electron-aaosCoreDir, options = {}) {
...
async function restoreFromBackup(backup, projectRoot, options = {}) {
  ...
  const backupAiosCoreDir = path.join(backup.path, '.electron-aaos-core');

  // Restore .electron-aaos-core files
  ...
  for (const file of backup.manifest.files) {
    try {
      ...
      const targetPath = file.isConfig
        ...
        : path.join(electron-aaosCoreDir, file.relativePath);
...
async function executeRollback(projectRoot, options = {}) {
  ...
  const electron-aaosCoreDir = path.join(projectRoot, '.electron-aaos-core');
  result.removal = await removeV21Structure(electron-aaosCoreDir, { onProgress });
  ...
  for (const dir of expectedDirs) {
    if (fs.existsSync(path.join(electron-aaosCoreDir, dir))) {
      validCount++;
    }
  }
...
function formatRollbackSummary(result) {
  ...
  lines.push('');
  lines.push('🔙 Electron AAOS Migration Rollback');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
