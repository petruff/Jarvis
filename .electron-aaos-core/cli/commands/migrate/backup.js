...
function createBackupDirName() {
  ...
  return `.electron-aaos-backup-${dateStr}`;
}
...
/**
 * Create backup of .electron-aaos-core directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  if (!fs.existsSync(electron-aaosCoreDir)) {
    throw new Error('No .electron-aaos-core directory found. Is this an Electron AAOS v2.0 project?');
  }
  ...
  const files = await getAllFiles(electron-aaosCoreDir);
  const manifest = {
    version: '2.0',
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(electron-aaosCoreDir, srcFile);
    const destFile = path.join(backupDir, '.electron-aaos-core', relativePath);
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Backup manifest not found. Is this a valid Electron AAOS backup?');
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, '.electron-aaos-core', file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter(entry => entry.isDirectory() && entry.name.startsWith('.electron-aaos-backup-'))
    .map(entry => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace('.electron-aaos-backup-', ''),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('.electron-aaos-backup-')) {
