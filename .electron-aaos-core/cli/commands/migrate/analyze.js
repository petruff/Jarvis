...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(electron-aaosCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: 'No .electron-aaos-core directory found',
    };
  }
  ...
  const hasV21Structure = v21Modules.every(module =>
    fs.existsSync(path.join(electron-aaosCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some(dir =>
    fs.existsSync(path.join(electron-aaosCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: 'Unable to detect Electron AAOS version structure',
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .electron-aaos-core
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(electron-aaosCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(electron-aaosCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(electron-aaosCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(electron-aaosCoreDir, moduleName);
