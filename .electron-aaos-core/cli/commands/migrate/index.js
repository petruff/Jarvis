/**
 * Electron AAOS Migration Command
 *
...
function printHeader(fromVersion, toVersion) {
  console.log('');
  console.log(`🔄 Electron AAOS Migration v${fromVersion} → v${toVersion}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
}
...
async function runMigration(options) {
  ...
  if (!versionInfo.isV2) {
    ...
    console.error(`❌ ${versionInfo.error || 'Cannot detect Electron AAOS version'}`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (!migrationResult.success) {
      ...
      console.log('  Run: electron-aaos migrate --rollback');
      process.exit(1);
    }
  ...
  try {
    ...
    const importResult = await updateAllImports(electron-aaosCoreDir, plan, { verbose, onProgress });
  ...
  const backupDir = `.electron-aaos-backup-${new Date().toISOString().split('T')[0]}`;
...
async function runRollbackCommand(options) {
  ...
  console.log('');
  console.log('🔙 Electron AAOS Migration Rollback');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
...
async function showBackupStatus(_options) {
  ...
  console.log('');
  console.log('📦 Electron AAOS Migration Backups');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
function createMigrateCommand() {
  ...
  migrate
    .description('Migrate Electron AAOS from v2.0 to v2.1 modular structure')
...
Migration Flow:
  Phase 1: Backup
    - Creates .electron-aaos-backup-{date}/ with all files
...
Examples:
  $ electron-aaos migrate                          # Interactive migration
  $ electron-aaos migrate --dry-run                # Preview changes
  $ electron-aaos migrate --verbose                # Detailed output
  $ electron-aaos migrate -y --skip-tests          # Non-interactive, skip tests
  $ electron-aaos migrate --rollback               # Restore from backup
  $ electron-aaos migrate --status                 # Show backups

Rollback:
  If migration fails or you want to revert:
  $ electron-aaos migrate --rollback

  Manual rollback:
  $ cp -r .electron-aaos-backup-{date}/* .electron-aaos-core/
`);
