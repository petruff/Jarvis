# Guia do Usuário Electron AAOS Electron AAOS

## Visão Geral

O Electron AAOS Electron AAOS é um framework de desenvolvimento full stack orientado por agentes de IA que implementa uma metodologia ágil única. Este guia explica como usar o Electron AAOS Electron AAOS de forma eficaz através de suas duas fases principais: **Planejamento** e **Desenvolvimento**.
...
### Instalando Electron AAOS-FullStack

O Electron AAOS-FullStack oferece instalação simplificada através de um instalador interativo completo.
...
npx @electron-aaos/electron-aaos-core@rc install

# OU usando NPM direto (produção - quando disponível)
npx @electron-aaos/fullstack install
```

**O que acontece durante a instalação**:

```
🚀 Electron AAOS-FullStack Installation Wizard
...
📦 Select Expansion Packs to Install:
  ...
  ◯ electron-aaos-infrastructure-devops (DevOps utilities)
...
✓ Installing .electron-aaos-core/ framework...
...
Se você está contribuindo para o Electron AAOS-FullStack em si:

```bash
# Clone o repositório
git clone https://github.com/Electron AAOSAI/electron-aaos-core
cd @electron-aaos/electron-aaos-core
...
```
your-project/
├── .electron-aaos-core/                    # ✅ Framework core instalado
...
Se você já tem Electron AAOS instalado e quer fazer upgrade:

```bash
# Upgrade automático (RC.9+)
npx @electron-aaos/electron-aaos-core@rc install --force-upgrade

# OU interativo (todos os RCs)
npx @electron-aaos/electron-aaos-core@rc install
# → Menu aparece:
#   1. Keep current version (v1.0.0-rc.8)
#   2. Upgrade Electron AAOS core (v1.0.0-rc.8 → v1.0.0-rc.10)
...
npm view @electron-aaos/electron-aaos-core@rc version
# Output: 1.0.0-rc.10

# Ver agentes disponíveis
npx @electron-aaos/electron-aaos-core@rc list:agents

# Ver expansion packs
npx @electron-aaos/electron-aaos-core@rc list:expansions

# Ajuda do instalador
npx @electron-aaos/electron-aaos-core@rc install --help
```
...
O futuro comando `electron-aaos init` permitirá escolher explicitamente entre dois modos:

#### Framework Development Mode
- Para desenvolvedores contribuindo ao Electron AAOS-FullStack
- `.electron-aaos-core/` é código fonte (commitado)
- Mudanças afetam o framework
- Quality gates testam integridade do framework

#### Project Development Mode
- Para desenvolvedores usando Electron AAOS em seus projetos
- `.electron-aaos-core/` é dependência (gitignored)
...
npx electron-aaos init

# Cria .electron-aaos-installation-config.yaml
...
npx @electron-aaos/electron-aaos-core@1.0.0-rc.10 install --force-upgrade
```
...
O Electron AAOS Electron AAOS usa uma abordagem de duas fases que separa planejamento estratégico de implementação tática:
...
O agente **qa** do Electron AAOS Electron AAOS inclui capacidades avançadas de arquitetura de testes:
...
2. **Recursos especiais**:
   - Reconhecimento automático de comandos Electron AAOS
   - Integração com workflows e tasks
   - Rastreamento automático de checkboxes

## Sistema de Preferências Técnicas

O Electron AAOS Electron AAOS permite configurar preferências técnicas que influenciam decisões dos agentes:
...
O Electron AAOS-FullStack impõe integridade de repositório através de um agente centralizado de DevOps.
...
```bash
# No repositório @electron-aaos/electron-aaos-core
@github-devops
*detect-repo
# Output:
# Repository: github.com/Electron AAOSAI/electron-aaos-core
# Mode: framework-development
# Quality Gates: Testes do framework Electron AAOS
```
...
O Electron AAOS-FullStack oferece três modos de desenvolvimento para acomodar diferentes níveis de habilidade e restrições de tempo.
...
Ao integrar Electron AAOS Electron AAOS em projetos existentes:
...
O Electron AAOS Electron AAOS inclui meta-agentes para orquestração:

### electron-aaos-master

O **electron-aaos-master** é o agente de orquestração principal:
...
### electron-aaos-orchestrator

O **electron-aaos-orchestrator** gerencia fluxos de trabalho:
...
### electron-aaos-developer

O **electron-aaos-developer** é o meta-agente para o próprio Electron AAOS:
...
[electron-aaos-developer guiará você na criação de um novo agente]
```
...
O Electron AAOS Electron AAOS suporta expansion packs para domínios específicos:
...
- **electron-aaos-infrastructure-devops** - Utilities de DevOps e infraestrutura
...
#   ◉ hybrid-ops
#   ◯ expansion-creator
#   ◯ electron-aaos-infrastructure-devops
#   ◯ meeting-notes
```
...
Use o **electron-aaos-developer** para criar agentes customizados:
...
Adicione templates próprios em `electron-aaos-core/templates/`:
...
- [ ] IDE configurado com regras Electron AAOS
...
- [Arquitetura do Electron AAOS](../docs/architecture.md)
- [Guia de Expansion Packs](../docs/Squads.md)
- [Trabalhando em Brownfield](./working-in-the-brownfield.md)

### Suporte
- [GitHub Issues](https://github.com/Electron AAOSAI/electron-aaos-core/issues)
...
*Electron AAOS Electron AAOS User Guide v2.0*
