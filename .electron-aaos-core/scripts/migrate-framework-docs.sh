#!/bin/bash
#
# Electron AAOS Framework Documentation Migration Script
#
# Purpose: Migrate framework docs from electron-aaos-fullstack to electron-aaos-core (REPO 1)
...
#   --target-repo    Path to electron-aaos-core repository (default: ../electron-aaos-core)
...
# Default values
DRY_RUN=false
TARGET_REPO="../electron-aaos-core"
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Electron AAOS Framework Documentation Migration Script             ║${NC}"
...
if [ ! -d "$SOURCE_DOCS" ]; then
  ...
  echo "  Please run this script from the electron-aaos-fullstack root directory"
  exit 1
fi
...
if [ ! -d "$TARGET_REPO" ]; then
  ...
  echo "  Please clone electron-aaos-core first or specify correct path with --target-repo"
  exit 1
fi
...
**Source:** electron-aaos-fullstack/docs/framework/
**Target:** electron-aaos-core/docs/framework/
**Story:** 6.1.2.6 - Framework Configuration System

## Migration Status

✅ Framework documentation successfully migrated from electron-aaos-fullstack to electron-aaos-core (REPO 1)

## Files Migrated

- coding-standards.md - Official Electron AAOS coding standards
...
- electron-aaos-fullstack maintains copies in `docs/framework/` (will be removed Q3 2026)
- electron-aaos-fullstack maintains legacy copies in `docs/architecture/` (will be removed Q3 2026)
...
```bash
# Restore from electron-aaos-fullstack
cd electron-aaos-fullstack
...
if [ "$DRY_RUN" = false ] && [ -d "$TARGET_REPO/.git" ]; then
  ...
  echo "    git commit -m \"docs: migrate framework documentation from electron-aaos-fullstack\""
  echo ""
  echo "    Story: 6.1.2.6 - Framework Configuration System"
  echo "    Migrated: coding-standards.md, tech-stack.md, source-tree.md"
  echo "    Source: electron-aaos-fullstack/docs/framework/"
  echo ""
...
echo "    4. Commit changes to electron-aaos-core repository"
echo "    5. Update electron-aaos-fullstack deprecation warnings (Q3 2026)"
echo ""
