#!/bin/bash
# ============================================
# Electron AAOS PM Orchestration Script (pm.sh)
...
#   --output    - Custom output file path (default: /tmp/electron-aaos-output-{timestamp}.md)
#
# Environment Variables:
#   Electron AAOS_OUTPUT_DIR   - Directory for output files (default: /tmp)
#   Electron AAOS_DEBUG        - Enable debug logging (default: false)
#   Electron AAOS_TIMEOUT      - Timeout in seconds (default: 300)
...
# Configuration
OUTPUT_DIR="${Electron AAOS_OUTPUT_DIR:-/tmp}"
DEBUG="${Electron AAOS_DEBUG:-false}"
TIMEOUT="${Electron AAOS_TIMEOUT:-300}"
CLAUDE_CMD="${CLAUDE_CMD:-claude}"
INLINE_MODE="${Electron AAOS_INLINE_MODE:-false}"
...
show_help() {
  cat << EOF
Electron AAOS Multi-Modal Orchestration Script v${VERSION}
...
Environment Variables:
  Electron AAOS_OUTPUT_DIR    Output directory (default: /tmp)
  Electron AAOS_DEBUG         Enable debug mode (default: false)
  Electron AAOS_TIMEOUT       Timeout in seconds (default: 300)
  Electron AAOS_INLINE_MODE   Run without a visual terminal (default: false)
  CLAUDE_CMD         Claude CLI command (default: claude)
...
setup_paths() {
  ...
  if [[ -n "$CUSTOM_OUTPUT" ]]; then
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/electron-aaos-output-${timestamp}.md"
  fi

  LOCK_FILE="${OUTPUT_DIR}/electron-aaos-lock-${timestamp}.lock"
...
spawn_inline() {
  ...
  # Build the command
  local output=""
  output+="=== Electron AAOS Agent Session (Inline) ===$(printf '\n')"
...
spawn_terminal() {
  ...
  local full_cmd="echo '=== Electron AAOS Agent Session ===' && "
...
main() {
  ...
  log_info "Electron AAOS Multi-Modal Orchestration Script v${VERSION}"
