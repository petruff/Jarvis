...
- campo: workflow_state
  tipo: object
  destino: File system (.electron-aaos/{instance-id}-engine-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] For action=continue/status/skip/abort, an active engine state file must exist
    ...
    blocker: true
    validação: |
      Check .electron-aaos/{instance-id}-engine-state.yaml exists with status=active
...
post-conditions:
  ...
  - [ ] State file created with all step outputs
    ...
    blocker: true
    validação: |
      Verify .electron-aaos/{instance-id}-engine-state.yaml exists and contains outputs
...
- **Tool:** workflow-state-manager
  ...
  - **Source:** .electron-aaos-core/development/scripts/workflow-state-manager.js

- **Tool:** workflow-validator
  - **Purpose:** Validate workflow before starting
  - **Source:** .electron-aaos-core/development/scripts/workflow-validator.js
...
- Template: `subagent-step-prompt.md` available at `.electron-aaos-core/development/templates/`
...
- `core` → `.electron-aaos-core/development/workflows/{workflow_name}.yaml`
...
State preserved at: .electron-aaos/{instance_id}-engine-state.yaml
```
...
State saved to: .electron-aaos/{instance_id}-engine-state.yaml
```
...
1. **Load template** from `.electron-aaos-core/development/templates/subagent-step-prompt.md`
...
resolve_agent_path(agent_ref, target_context, squad_name):
  ...
  IF agent_ref starts with "core:":
    RETURN ".electron-aaos-core/development/agents/{agent_ref without prefix}.md"
  IF agent_ref starts with "squad:":
    RETURN "squads/{squad_name}/agents/{agent_ref without prefix}.md"

  # Context-based resolution
  IF target_context == "core":
    RETURN ".electron-aaos-core/development/agents/{agent_ref}.md"
  ...
  IF target_context == "hybrid":
    ...
    core_path = ".electron-aaos-core/development/agents/{agent_ref}.md"
...
resolve_task_path(uses_ref, target_context, squad_name):
  IF target_context == "core":
    RETURN ".electron-aaos-core/development/tasks/{uses_ref}.md"
  IF target_context == "squad":
    RETURN "squads/{squad_name}/tasks/{uses_ref}.md"
  IF target_context == "hybrid":
    squad_path = "squads/{squad_name}/tasks/{uses_ref}.md"
    core_path = ".electron-aaos-core/development/tasks/{uses_ref}.md"
    IF squad_path exists → RETURN squad_path
    IF core_path exists → RETURN core_path
...
resolve_data_path(data_ref, target_context, squad_name):
  IF target_context == "core":
    RETURN ".electron-aaos-core/data/{data_ref}"
  IF target_context == "squad":
    RETURN "squads/{squad_name}/data/{data_ref}"
  IF target_context == "hybrid":
    squad_path = "squads/{squad_name}/data/{data_ref}"
    core_path = ".electron-aaos-core/data/{data_ref}"
    IF squad_path exists → RETURN squad_path
    IF core_path exists → RETURN core_path
...
```yaml
# .electron-aaos/{instance-id}-engine-state.yaml
engine_state:
  workflow_id: {id}
  workflow_name: {name}
