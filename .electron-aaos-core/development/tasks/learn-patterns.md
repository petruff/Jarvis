...
# Learn Patterns - Electron AAOS Developer Task
...
*learn-patterns --component electron-aaos-core/agents/developer.md --analyze
...
class LearnPatternsTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🧠 Electron AAOS Pattern Learning'));
  ...
  async loadModification(modificationId) {
    ...
    const sources = [
      path.join(this.rootPath, '.electron-aaos', 'modifications', `${modificationId}.json`),
      path.join(this.rootPath, '.electron-aaos', 'history', `${modificationId}.json`),
      path.join(this.rootPath, '.electron-aaos', 'proposals', `${modificationId}.json`)
    ];
