---
task: Sync Squad to Electron AAOS
...
Entrada: |
  ...
  - official: Flag para marcar como oficial (--official, apenas Electron AAOSAI)
...
Checklist:
  ...
  - "[x] Calcular checksum"
  - "[x] Enviar para Electron AAOS API"
  - "[x] Exibir URL do marketplace"
---

# *sync-squad-electron-aaos

Sincroniza um squad local para o Electron AAOS API marketplace.
...
*sync-squad-electron-aaos ./squads/meu-squad

# Sync público (visível para todos)
*sync-squad-electron-aaos ./squads/meu-squad --public

# Preview sem sincronizar
*sync-squad-electron-aaos ./squads/meu-squad --dry-run

# Sync com verbosidade
*sync-squad-electron-aaos ./squads/meu-squad --verbose
```

## Autenticação

Requer autenticação com Electron AAOS API:
...
```env
SYNKRA_API_URL=https://api.electron-aaos.dev/api
SYNKRA_API_TOKEN=seu-token
```

Para obter um token:
1. Acesse https://electron-aaos.dev/settings/api-keys
...
Step 3: Sync to Electron AAOS API
  ✓ Visibility: private
  ✓ API URL: https://api.electron-aaos.dev/api

Syncing to Electron AAOS API...
...
Next steps:
  ...
  - Make public: *sync-squad-electron-aaos ./squads/meu-squad --public
```
...
| `--official` | Marca como squad oficial (apenas Electron AAOSAI team) | false |
...
// 3. Validate with squad-validator.js
const { validateSquad } = await import('.electron-aaos-core/development/scripts/squad/squad-validator.js');
...
const apiUrl = process.env.SYNKRA_API_URL || 'https://api.electron-aaos.dev/api';
...
✅ Squad synced successfully!
  ...
  if (flags.public) {
    output(`  URL: https://electron-aaos.dev/squads/${result.data.squad_id}`);
  }
} else {
...
| `401 Unauthorized` | Token inválido ou expirado | Gere novo token em electron-aaos.dev |
...
- **SQS-5:** SquadSyncService for Electron AAOS API (Sprint 8)
