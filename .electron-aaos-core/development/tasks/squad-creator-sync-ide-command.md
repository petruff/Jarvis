...
Checklist:
  - '[x] Carregar .electron-aaos-sync.yaml'
...
O sistema usa `.electron-aaos-sync.yaml` na raiz do projeto para configuração:
...
│  1. Parse type + name                             │
│     ↓                                             │
│  2. Load .electron-aaos-sync.yaml                          │
...
│     ↓                                             │
│  8. Log to .electron-aaos-sync.log                         │
│     ↓                                             │
│  9. Display summary                               │
│                                                   │
└──────────────────────────────────────────────────┘
...
| `Pack alias not found` | Squad não está em pack_aliases  | Adicione ao .electron-aaos-sync.yaml |
| `File exists`          | Destino já existe               | Use --force ou escolha ação |
| `IDE not active`       | IDE não está em active_ides     | Ative no .electron-aaos-sync.yaml    |
...
const syncConfig = loadYaml('.electron-aaos-sync.yaml');
...
if (syncConfig.behavior?.log_sync_operations) {
  appendLog('.electron-aaos-sync.log', {
...
- `*sync-squad-electron-aaos` - Sincronizar para Electron AAOS marketplace
