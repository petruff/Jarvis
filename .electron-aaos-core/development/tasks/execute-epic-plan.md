...
- campo: epic_state
  tipo: object
  destino: File system (.electron-aaos/epic-{epicId}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Template workflow must exist
    ...
    blocker: true
    validação: |
      execution.template must resolve to .electron-aaos-core/development/workflows/{template}.yaml
  ...
  - [ ] For action=continue, state file must exist
    ...
    blocker: true
    validação: |
      .electron-aaos/epic-{epicId}-state.yaml must exist with status=active
...
post-conditions:
  - [ ] State file updated with current wave progress
    ...
    blocker: true
    validação: |
      .electron-aaos/epic-{epicId}-state.yaml exists and reflects completed waves/stories
...
4. **Error:** State file corrupted
   ...
   - **Recovery:** Restore from .electron-aaos/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in `.electron-aaos-core/development/workflows/`
- Inner loop `development-cycle.yaml` exists in `.electron-aaos-core/development/workflows/`
...
```yaml
# .electron-aaos/epic-{epicId}-state.yaml
epic_state:
  epicId: {epicId}
...
**1. Load state** from `.electron-aaos/epic-{epicId}-state.yaml`
...
State preserved at: .electron-aaos/epic-{epicId}-state.yaml
...
PROCEDURE execute_wave(wave, stories, state):
  ...
  IF wave.parallel == true:
    ...
    FOR EACH story_id IN wave.stories (IN PARALLEL):
      ...
      Task tool call:
        ...
        subagent_type: "electron-aaos-dev"
        prompt: |
  ...
  Task tool call:
    ...
    subagent_type: "electron-aaos-architect"  # or whatever the gate agent is
    prompt: |
  ...
  IF gate verdict == APPROVED:
    ...
    Task tool call:
      ...
      subagent_type: "electron-aaos-devops"
      prompt: |
...
PROCEDURE final_gate(execution, state):
  ...
  Task tool call:
    ...
    subagent_type: "electron-aaos-architect"
    prompt: |
