...
- campo: newMode
  tipo: object
  destino: .electron-aaos/config.yaml
  persistido: true
```
...
Read the current permission mode from `.electron-aaos/config.yaml` under `permissions.mode`.
...
Update `.electron-aaos/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.electron-aaos/` directory if needed.
...
```javascript
const { cycleMode } = require('./.electron-aaos-core/core/permissions');
...
- If `.electron-aaos/config.yaml` cannot be read, start from default `ask` mode
...
version: 1.0.0
dependencies:
  - .electron-aaos-core/core/permissions/permission-mode.js
  - .electron-aaos-core/core/permissions/index.js
tags:
  - permissions
  - mode-toggle
  - universal-command
