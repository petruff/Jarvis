...
if (!context) {
  throw new Error('Unable to detect repository. Run "electron-aaos init" first.');
}
```
...
function loadPRConfig() {
  const configPath = path.join(process.cwd(), '.electron-aaos-core', 'core-config.yaml');
...
### NPM Package with Semantic-Release (electron-aaos-core)
```yaml
github:
  pr:
