...
pre-conditions:
  ...
  - [ ] Session state exists (optional)
    tipo: pre-condition
    blocker: false
    validação: Check .electron-aaos/session-state.json exists
...
```javascript
const SuggestionEngine = require('.electron-aaos-core/workflow-intelligence/engine/suggestion-engine');
...
```javascript
const formatter = require('.electron-aaos-core/workflow-intelligence/engine/output-formatter');
