...
autoClaude:
  ...
  inputs:
    ...
    - name: projectContext
      type: file
      path: .electron-aaos/project-context.yaml
      required: false
...
step_11:
  ...
  actions:
    ...
    - action: update_dashboard_status
      script: |
        const { PlanTracker } = require('.electron-aaos-core/infrastructure/scripts/plan-tracker.js');
...
recovery:
  ...
  attempt_tracking:
    storage: .electron-aaos/recovery/{storyId}/{subtaskId}.json
    schema:
      subtaskId: string
      attempts: array
