...
# Propose Modification - Electron AAOS Developer Task

## Purpose
Create and submit modification proposals for collaborative review and approval within the Electron AAOS Electron AAOS framework.
...
*propose-modification electron-aaos-core/agents/weather-agent.md enhance --title "Add caching support" --description "Implement response caching to reduce API calls" --priority medium

# Propose critical refactoring with impact analysis
*propose-modification electron-aaos-core/scripts/core-utility.js refactor --title "Optimize performance" --priority critical --impact-analysis --assignees "alice,bob"

# Create draft proposal for workflow deprecation
*propose-modification electron-aaos-core/workflows/legacy-workflow.yaml deprecate --draft --title "Deprecate legacy workflow" --link-issues "123,456"
```
...
class ProposeModificationTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('📝 Electron AAOS Modification Proposal'));
  ...
  async createProposal(component, config) {
    const proposal = {
      ...
      metadata: {
        createdBy: process.env.USER || 'electron-aaos-developer',
  ...
  async storeProposal(proposal, result) {
    const proposalsDir = path.join(this.rootPath, '.electron-aaos', 'proposals');
...
[Describe validation approach]`
  ...
  generateProposalUrl(proposalId) {
    ...
    return `http://electron-aaos-framework.local/proposals/${proposalId}`;
  }
}
