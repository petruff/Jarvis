...
```bash
node bin/electron-aaos-ids.js ids:health
```
...
```bash
node bin/electron-aaos-ids.js ids:health --fix
```
...
- Log all healing actions to `.electron-aaos-core/data/registry-healing-log.jsonl`

### Step 3: JSON Output (Machine-Readable)

```bash
node bin/electron-aaos-ids.js ids:health --json
node bin/electron-aaos-ids.js ids:health --fix --json
```
...
```javascript
const { RegistryHealer } = require('.electron-aaos-core/core/ids/registry-healer');
