...
### 7. Update Electron AAOS Documentation (REQUIRED)
...
post-conditions:
  ...
  - [ ] Electron AAOS documentation updated
    tipo: post-condition
    blocker: true
...
**CRITICAL NOTE:** Tools added to Docker MCP Toolkit are NOT immediately available to Electron AAOS agents. The docker-gateway caches tools at Claude Code startup. User MUST restart their Claude Code session for new tools to appear.
...
🔗 Status: Running
📋 Preset: Added to 'electron-aaos-full'

Next steps:
1. Test tools: docker mcp tools call {name}.tool1 --param value
2. Use in workflow: *mcp-workflow with {name} tools
3. Add to other presets: docker mcp preset update electron-aaos-dev --add-server {name}
```
...
changelog:
  ...
  1.2.0:
    - Added: Steps 7-9 for Electron AAOS documentation and session restart
    ...
    - Fixed: Tools not appearing in Electron AAOS agents after MCP addition
  1.1.0:
