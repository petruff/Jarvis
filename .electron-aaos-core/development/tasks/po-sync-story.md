...
- PM tool configured in `.electron-aaos-pm-config.yaml` (or will use local-only mode)
...
```javascript
const { getPMAdapter } = require('../.electron-aaos-core/scripts/pm-adapter-factory');
...
- **Configuration missing**: Inform user to run `electron-aaos init`
...
```javascript
const { syncStoryToPM } = require('../.electron-aaos-core/scripts/story-manager');
