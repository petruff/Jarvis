...
- **Tool:** code-analyzer
  ...
  - **Source:** .electron-aaos-core/utils/code-analyzer.js
...
- **Script:** analyze-codebase.js
  ...
  - **Language:** JavaScript
  - **Location:** .electron-aaos-core/scripts/analyze-codebase.js
...
Systematically audit all utilities in `.electron-aaos-core/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .electron-aaos-core/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .electron-aaos-core/agents .electron-aaos-core/tasks Squads 2>/dev/null | wc -l)
