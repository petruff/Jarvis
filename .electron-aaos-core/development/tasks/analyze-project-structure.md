...
**Purpose:** Analyze an existing Electron AAOS project to understand its structure, services, patterns, and provide recommendations for implementing new features. This is Phase 1 of the Incremental Feature Workflow.
...
- field: project_path
  ...
  required: false
  validation: Valid directory path with .electron-aaos-core/
...
```yaml
pre-conditions:
  - [ ] Project has .electron-aaos-core/ directory
    type: pre-condition
    blocker: true
    validation: |
      Check .electron-aaos-core/ directory exists in project root
    error_message: "Pre-condition failed: Not an Electron AAOS project (.electron-aaos-core/ not found)"
...
acceptance-criteria:
  - [ ] Project structure scanned
    ...
    blocker: true
    validation: |
      Assert .electron-aaos-core/ configuration analyzed
...
1. **Error:** No .electron-aaos-core/ Directory
   - **Cause:** Not an Electron AAOS project
   - **Resolution:** Initialize Electron AAOS first or check directory
...
```javascript
// Core Electron AAOS structure
const scanLocations = {
  electron-aaosCore: '.electron-aaos-core/',
  services: '.electron-aaos-core/infrastructure/services/',
  squads: '.electron-aaos-core/squads/',
  agents: '.electron-aaos-core/development/agents/',
  tasks: '.electron-aaos-core/development/tasks/',
  data: '.electron-aaos-core/data/'
};
```
...
const configPatterns = {
  ...
  configFile: exists('electron-aaos.config.js') || exists('.electron-aaos-core/core-config.yaml'),
...
```
.electron-aaos-core/infrastructure/services/{service-name}/
...
| Aspect | Value |
|--------|-------|
| Framework | Electron AAOS-FullStack |
...
```
.electron-aaos-core/infrastructure/services/{service_name}/
├── README.md
├── index.ts
