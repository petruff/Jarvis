...
- campo: gotchas_file
  tipo: string
  destino: .electron-aaos/gotchas.md
  persistido: true

- campo: gotchas_json
  tipo: string
  destino: .electron-aaos/gotchas.json
  persistido: true
...
```yaml
pre-conditions:
  - [ ] Project has .electron-aaos directory initialized
    tipo: pre-condition
    blocker: false
    validação: |
      Check if .electron-aaos/ directory exists, create if not
    error_message: "Creating .electron-aaos/ directory"
...
post-conditions:
  - [ ] gotchas.md file generated/updated
    ...
    blocker: true
    validação: |
      Check .electron-aaos/gotchas.md exists and has content
  ...
  - [ ] gotchas.json schema valid
    ...
    blocker: false
    validação: |
      Validate .electron-aaos/gotchas.json against schema
...
1. **update** (default)
   ...
   - Generates `.electron-aaos/gotchas.md` and `.electron-aaos/gotchas.json`
...
```javascript
const { GotchasDocumenter } = require('.electron-aaos-core/infrastructure/scripts/gotchas-documenter');

const documenter = new GotchasDocumenter(rootPath, {
  outputPath: '.electron-aaos/gotchas.md',
  quiet: false
});
```
...
documenter.mergeWithExisting('.electron-aaos/gotchas.json');
```
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require('.electron-aaos-core/infrastructure/scripts/gotchas-documenter');
...
const { getGotchasForSelfCritique } = require('.electron-aaos-core/infrastructure/scripts/gotchas-documenter');
...
```json
{
  "schema": "electron-aaos-gotchas-v1",
  "version": "1.0.0",
...
node .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js update

# List all gotchas
node .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js list

# List high severity only
node .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js list --severity high

# Search for specific gotchas
node .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js search "zustand"

# List by category
node .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js category TypeScript

# Output as JSON
node .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js list --format json
```
...
3. **Error:** Write permission denied
   - **Cause:** Cannot write to .electron-aaos directory
...
**Script:** gotchas-documenter.js
  ...
  - **Language:** JavaScript
  - **Location:** .electron-aaos-core/infrastructure/scripts/gotchas-documenter.js

---

## Dependencies

- `.electron-aaos-core/development/tasks/capture-session-insights.md` - Provides input insights
- `.electron-aaos-core/product/checklists/self-critique-checklist.md` - Consumes gotchas (AC5)
- `.electron-aaos-core/development/tasks/spec-write-spec.md` - May reference gotchas
