...
- campo: workflow_state
  tipo: object
  destino: File system (.electron-aaos/{instance-id}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] For action=continue/status/skip/abort, an active state file must exist
    ...
    blocker: true
    validação: |
      Check .electron-aaos/{instance-id}-state.yaml exists with status=active
...
- **Tool:** workflow-state-manager
  ...
  - **Implementation:** AI agent reads/writes `.electron-aaos/{instance-id}-state.yaml` files directly
...
- For engine mode: `run-workflow-engine.md` task must exist at `.electron-aaos-core/development/tasks/run-workflow-engine.md`
- State directory `.electron-aaos/` must be writable
...
3. **Create state file** using WorkflowStateManager.createState():
   ...
   - Writes state to `.electron-aaos/{instance-id}-state.yaml`
...
3. **Generate cleanup notes:**
   ...
   State file preserved at: .electron-aaos/{instance-id}-state.yaml
