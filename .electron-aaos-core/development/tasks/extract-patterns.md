...
outputs:
  - name: patterns_markdown
    type: string
    destino: File (.electron-aaos/patterns.md)
    persistido: true
...
pre-conditions:
  - [ ] Project root is a valid codebase
    tipo: pre-condition
    blocker: true
    validação: Check package.json or .electron-aaos-core exists
...
```javascript
const PatternExtractor = require('.electron-aaos-core/infrastructure/scripts/pattern-extractor');
...
Subcommands:
  ...
  save        Save to .electron-aaos/patterns.md
...
Examples:
  ...
  *extract-patterns save                      # Save to .electron-aaos/patterns.md
...
Output File:
  Default location: .electron-aaos/patterns.md
...
Patterns saved to: .electron-aaos/patterns.md
...
post-conditions:
  - [ ] Patterns file created/updated
    tipo: post-condition
    blocker: false
    validação: Check .electron-aaos/patterns.md exists and is valid markdown
...
references:
  - path: .electron-aaos/patterns.md
...
node .electron-aaos-core/infrastructure/scripts/pattern-extractor.js [command] [options]

# Via Electron AAOS command
...
dependencies:
  modules:
    - .electron-aaos-core/infrastructure/scripts/pattern-extractor.js
  tasks: []
  referenced_by:
    - spec-write.md
tags:
