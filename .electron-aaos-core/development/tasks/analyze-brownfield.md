...
pre-conditions:
  ...
  - [ ] Brownfield Analyzer module is available
    ...
    blocker: true
    validation: |
      Verify .electron-aaos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js exists
...
- **Tool:** brownfield-analyzer
  ...
  - **Source:** .electron-aaos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js

- **Tool:** mode-detector
  - **Purpose:** Collect project markers for analysis
  - **Source:** .electron-aaos-core/infrastructure/scripts/documentation-integrity/mode-detector.js
...
- **Script:** brownfield-analyzer.js
  ...
  - **Language:** JavaScript
  - **Location:** .electron-aaos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js
...
Analyze an existing project to understand its structure, tech stack, coding standards, and CI/CD workflows before Electron AAOS integration. This task provides recommendations for safe integration and identifies potential conflicts.
...
```javascript
const { analyzeProject, formatMigrationReport } = require('./.electron-aaos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer');
...
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  • Preserve existing ESLint configuration - Electron AAOS will adapt          ║
║  • Keep existing Prettier settings - Electron AAOS coding-standards.md will d ║
║  • Review existing CI/CD before adding Electron AAOS workflows                ║
║  • Electron AAOS will use existing tsconfig.json settings                     ║
...
|----------|---------|---------|
| `parallel` | Safe to proceed with standard Electron AAOS setup | Use `*setup-project-docs` directly |
...
1. **Review GitHub Workflows:**
   ...
   # Check for potential conflicts with Electron AAOS workflows
...
1. **docs/architecture/ exists:**
   - Decide: Keep existing or merge with Electron AAOS docs
   - Option A: Rename existing to `docs/legacy-architecture/`
   - Option B: Configure Electron AAOS to use alternate path
...
- Run this task BEFORE any Electron AAOS integration
