...
Cria um novo squad seguindo a arquitetura task-first do Electron AAOS.
...
? Config inheritance:
  > extend (adiciona as regras do core)
    ...
    none (sem heranca)
? Minimum Electron AAOS version: (2.1.0)
```
...
📚 Documentation:
   ...
   - Task Format: .electron-aaos-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md

🚀 When ready to share:
   - Local only: Keep in ./squads/ (private)
   - Public: @squad-creator *publish-squad meu-dominio-squad
   - API: @squad-creator *sync-squad-electron-aaos meu-dominio-squad
```
...
```javascript
const { SquadGenerator } = require('./.electron-aaos-core/development/scripts/squad');
