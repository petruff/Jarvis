...
Configure Docker MCP Toolkit as the primary MCP infrastructure for Electron AAOS, using **HTTP transport** instead of stdio to avoid timeout issues during gateway initialization.
...
Configure Docker MCP Toolkit as the primary MCP infrastructure for Electron AAOS, replacing 1MCP with the containerized gateway approach. This enables:
...
### 3. Enable Electron AAOS Default MCPs
