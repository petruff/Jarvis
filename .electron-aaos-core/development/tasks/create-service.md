...
outputs:
  - name: service_directory
    type: directory
    location: ".electron-aaos-core/infrastructure/services/{service_name}/"
    persistido: true
...
```yaml
pre-conditions:
  - [ ] WIS-10 templates exist at .electron-aaos-core/development/templates/service-template/
    tipo: pre-condition
    blocker: true
  ...
  - [ ] Service name is unique (no existing service with same name)
    tipo: pre-condition
    blocker: true
    validação: Check .electron-aaos-core/infrastructure/services/{name}/ does not exist
...
3. agent-tool - Tool for Electron AAOS agents
...
// Check uniqueness
const targetDir = `.electron-aaos-core/infrastructure/services/${serviceName}/`;
...
```javascript
const templateDir = '.electron-aaos-core/development/templates/service-template/';
const templates = [
  'README.md.hbs',
  'index.ts.hbs',
  'types.ts.hbs',
...
cd .electron-aaos-core/infrastructure/services/{service_name}/
...
============================================
 ...
 Next Steps:
   1. cd .electron-aaos-core/infrastructure/services/{service_name}
