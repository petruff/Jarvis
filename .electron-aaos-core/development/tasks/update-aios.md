# Task: Update Electron AAOS Framework

> **Version:** 4.0.0
> **Created:** 2026-01-29
> **Updated:** 2026-01-31
> **Type:** SYNC (git-native framework synchronization)
> **Agent:** @devops (Gage) or @electron-aaos (Orion)
> **Execution:** Simple bash script (~15 lines)

## Purpose

Git-native sync of Electron AAOS framework from upstream repository. Uses sparse clone + file comparison for safe review before applying changes. All local customizations preserved automatically by backup/restore.
...
bash .electron-aaos-core/scripts/update-electron-aaos.sh

# Review changes shown by the script, then:
git add .electron-aaos-core && git commit -m "chore: sync Electron AAOS framework"   # Apply changes
# OR
git checkout -- .electron-aaos-core/                                         # Cancel changes
```
...
1. **Clone upstream** - Sparse shallow clone of Electron AAOSAI/electron-aaos-core (only `.electron-aaos-core/`)
...
| Path | Reason |
|------|--------|
| `.electron-aaos-core/expansion-packs/` | Custom copywriters, data, ralph |
| `.electron-aaos-core/marketing/` | Marketing-specific agents/tasks |
| `source/` | Business context YAML |
| `Knowledge/` | Knowledge bases |
| `.electron-aaos-core/context/` | Compiled contexts |
...
```yaml
task: updateElectron AAOSFramework
agent: devops
mode: simple
timeout: 60  # 1 minute max

execution:
  script: .electron-aaos-core/scripts/update-electron-aaos.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .electron-aaos-core/scripts/update-electron-aaos.sh
  3. Review changes displayed
  4. git add .electron-aaos-core && git commit -m "chore: sync Electron AAOS framework"  # to apply
  5. git checkout -- .electron-aaos-core/                                        # to cancel
...
ls -la .electron-aaos-core/expansion-packs/  # if exists
