...
```yaml
pre-conditions:
  - [ ] Entity Registry exists at .electron-aaos-core/data/entity-registry.yaml
    tipo: pre-condition
    blocker: false
    validação: |
...
```bash
# Basic query
node bin/electron-aaos-ids.js ids:query "validate story drafts"

# With JSON output
node bin/electron-aaos-ids.js ids:query "template rendering engine" --json

# Filter by type
node bin/electron-aaos-ids.js ids:query "database migration" --type script

# Filter by category
node bin/electron-aaos-ids.js ids:query "agent persona" --category agents
```
...
const { RegistryLoader } = require(path.resolve('.electron-aaos-core/core/ids/registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve('.electron-aaos-core/core/ids/incremental-decision-engine'));
...
- `electron-aaos ids:create-review` — Review CREATE decisions for 30-day assessment
