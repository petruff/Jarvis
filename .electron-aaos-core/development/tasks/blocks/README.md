# Electron AAOS Task Blocks System
...
Blocks are small, focused units of functionality that can be included in multiple tasks. They follow the DRY (Don't Repeat Yourself) principle and provide consistent behavior across the Electron AAOS framework.

## Directory Structure

```
.electron-aaos-core/development/tasks/blocks/
├── README.md                  # This file
...
```javascript
const { loadBlock, executeBlock } = require('.electron-aaos-core/utils/block-loader');
...
| `agent-prompt-template` | Standardized template for spawning Electron AAOS agents | Any skill/task that invokes agents via Task tool |
...
*Electron AAOS Task Blocks System v1.1.0*
