...
Load Electron AAOS project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Gotchas
    ...
    actions:
      - read: .electron-aaos/gotchas.json
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .electron-aaos-core/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .electron-aaos-core/core-config.yaml
...
```javascript
const { loadContext } = require('.electron-aaos-core/utils/context-loader');
...
| File | Purpose |
|------|---------|
| `.electron-aaos/gotchas.json` | Known issues and workarounds |
| `.electron-aaos-core/data/technical-preferences.md` | User-defined patterns |
| `.electron-aaos-core/core-config.yaml` | Project configuration |
