...
Standardized template for spawning Electron AAOS agents with consistent structure. Ensures all agent invocations follow the same persona loading, context, mission, and output pattern.
...
```markdown
You are {agent_name}, the Electron AAOS {agent_role}. Read your complete agent file at:
{agent_file_path}
...
<!-- Parameters:
  ...
  agent_file_path=.claude/commands/Electron AAOS/agents/architect.md,
...
```javascript
const { loadBlock, renderTemplate } = require('.electron-aaos-core/utils/block-loader');
