...
# Suggest Refactoring - Electron AAOS Developer Task
...
```bash
# Analyze single file
*suggest-refactoring electron-aaos-core/scripts/complex-utility.js

# Analyze directory with specific patterns
*suggest-refactoring electron-aaos-core/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
*suggest-refactoring electron-aaos-core/utils --threshold 7 --apply-all

# Export suggestions for review
*suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
*suggest-refactoring electron-aaos-core/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 Electron AAOS Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.electron-aaos', 'refactoring', `${suggestionId}.json`);
