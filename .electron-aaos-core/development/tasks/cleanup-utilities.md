...
const configPath = path.join(__dirname, '../../.electron-aaos-core/core-config.yaml');
...
mkdir -p .backups
cp -r .electron-aaos-core/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .electron-aaos-core/agents .electron-aaos-core/tasks .electron-aaos-core/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .electron-aaos-core/agents/ .electron-aaos-core/tasks/ .electron-aaos-core/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .electron-aaos-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .electron-aaos-core/utils-archive
```

**3.2 Create Archive README**

Create `.electron-aaos-core/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back**:
   ```bash
   cp .electron-aaos-core/utils-archive/utility-name.js .electron-aaos-core/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .electron-aaos-core/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .electron-aaos-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .electron-aaos-core/utils
```

**Selective Restoration**:
```bash
cp .electron-aaos-core/utils-archive/specific-utility.js .electron-aaos-core/scripts/
```
...
- **Tool:** task-runner
  ...
  - **Source:** .electron-aaos-core/core/task-runner.js

- **Tool:** logger
  - **Purpose:** Execution logging and error tracking
  - **Source:** .electron-aaos-core/utils/logger.js
...
- **Script:** execute-task.js
  ...
  - **Language:** JavaScript
  - **Location:** .electron-aaos-core/scripts/execute-task.js
...
git mv .electron-aaos-core/scripts/utility-name.js .electron-aaos-core/utils-archive/

# Or for multiple files:
git mv .electron-aaos-core/scripts/{electron-aaos-validator-fixed.js,electron-aaos-validator-refactored.js} .electron-aaos-core/utils-archive/
```
...
git mv .electron-aaos-core/scripts/electron-aaos-validator.test.js tests/utils/
```
...
1. electron-aaos-validator-fixed.js - Duplicate of electron-aaos-validator.js
2. electron-aaos-validator-refactored.js - Duplicate of electron-aaos-validator.js
...
...
Update utility count in `.electron-aaos-core/core-config.yaml`:
...
framework:
  entities:
    utils:
      ...
      location: .electron-aaos-core/scripts/
```
...
```bash
node .electron-aaos-core/scripts/electron-aaos-validator.js
```
...
for util in $(ls .electron-aaos-core/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .electron-aaos-core/agents .electron-aaos-core/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
rm -rf .electron-aaos-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .electron-aaos-core/utils
git checkout .electron-aaos-core/core-config.yaml
```
...
1. `.electron-aaos-core/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.electron-aaos-core/utils-archive/ARCHIVE-README.md` - Archive documentation
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.electron-aaos-core/core-config.yaml` - Corrected utility count
...
- ✅ Framework validation (electron-aaos-validator.js) passes
- ✅ All agents (@dev, @po, @qa) activate successfully
