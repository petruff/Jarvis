...
Complete environment bootstrap for new Electron AAOS projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
## Task Definition (Electron AAOS Task Format V1.0)
...
- campo: environment_report
  tipo: object
  destino: File system (.electron-aaos/environment-report.yaml)
  persistido: true
...
post-conditions:
  ...
  - [ ] Environment report generated
    ...
    blocker: false
    validação: |
      Check .electron-aaos/environment-report.yaml exists
...
acceptance-criteria:
  ...
  - [ ] Project structure follows Electron AAOS conventions
    tipo: acceptance-criterion
    blocker: false
    validação: |
      Check docs/, .electron-aaos/, and package.json exist
...
- **Tool:** cli-checker
  ...
  - **Source:** .electron-aaos-core/infrastructure/scripts/cli-checker.js

- **Tool:** github-cli
  - **Purpose:** Repository creation and authentication
  - **Source:** .electron-aaos-core/infrastructure/tools/cli/github-cli.yaml
...
- Cache detection results in .electron-aaos/environment-report.yaml
...
```
╔════════════════════════════════════════════════════════════════════════╗
║                     Electron AAOS ENVIRONMENT AUDIT                              ║
╠════════════════════════════════════════════════════════════════════════╣
...
ESSENTIAL (required for Electron AAOS):
...
GitHub Organization/Username:
  Found organizations: Electron AAOSAI
...
# Electron AAOS generated files
.electron-aaos/project-status.yaml
.electron-aaos/environment-report.yaml
...
> Created with Electron AAOS Electron AAOS
...
---
*Generated by Electron AAOS Environment Bootstrap*
...
- Initialize Electron AAOS Electron AAOS project structure
...
**Action:** Create Electron AAOS-compliant project structure
...
# Create .electron-aaos/config.yaml
@"
# Electron AAOS Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".electron-aaos/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with Electron AAOS Electron AAOS",
  "scripts": {
...
**Action:** Ask user for their profile preference and persist to `~/.electron-aaos/user-config.yaml`
...
```bash
# Create ~/.electron-aaos/ with secure permissions
mkdir -p ~/.electron-aaos
chmod 700 ~/.electron-aaos

# Write user-config.yaml with selected profile
cat > ~/.electron-aaos/user-config.yaml << EOF
# Electron AAOS User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require('.electron-aaos-core/core/config/config-resolver');
...
- `~/.electron-aaos/` directory must have permissions 700
- `~/.electron-aaos/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".electron-aaos/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .electron-aaos/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ Electron AAOS ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
╠═══════════════════════════════════════════════════════════════════════════╣
║  my-awesome-project/                                                       ║
║  ├── .electron-aaos/                    # Electron AAOS configuration                        ║
...
║                                                                            ║
║  Full workflow: .electron-aaos-core/development/workflows/greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  • View environment report: cat .electron-aaos/environment-report.json              ║
║  • Check GitHub repo: gh repo view --web                                   ║
║  • Electron AAOS help: @electron-aaos-master *help                                           ║
...
- [ ] .electron-aaos/config.yaml created
- [ ] Environment report generated
...
- [Electron AAOS Greenfield Workflow](.electron-aaos-core/development/workflows/greenfield-fullstack.yaml)
