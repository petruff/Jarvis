...
To help users contribute to the Electron AAOS open-source project (`@electron-aaos/electron-aaos-core`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

**Target Repository**: `@electron-aaos/electron-aaos-core` (open-source framework)
...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `"electron-aaos-core/agents/improved-agent.md"`
...
- **pr_url**: `string`
  ...
  - **Example**: `"https://github.com/Electron AAOSAI/electron-aaos-core/pull/123"`
...
2. **Validate Repository State**
   - Check if `@electron-aaos/electron-aaos-core` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `electron-aaos-core/agents/*` → "agent"
     - `electron-aaos-core/tasks/*` → "task"
     - `electron-aaos-core/tools/*` → "tool"
...
14. **Generate PR Title & Description**
    ...
    - **Description**: Use PR template:
      ...
      **First-time contributor?** Welcome! 🎉 This PR was created using Electron AAOS PR Automation.
      ```
...
15. **Create Pull Request**
    - Use GitHub CLI:
      ```bash
      gh pr create \
        --repo Electron AAOSAI/electron-aaos-core \
        --title "{title}" \
        --body "{description}" \
        --base main \
...
18. **Provide Next Steps**
    - Display to user:
      ...
      Thank you for contributing to Electron AAOS! 🚀
      ```
...
- [ ] Fork of @electron-aaos/electron-aaos-core exists
  - **Validation**: `gh repo view {user}/@electron-aaos/electron-aaos-core` succeeds
...
```bash
electron-aaos activate Otto  # github-devops agent
electron-aaos pr create \
  --type="Squad" \
...
electron-aaos pr create \
  --type="agent" \
  --path="electron-aaos-core/agents/improved-po.md" \
...
electron-aaos pr create \
  --type="bug-fix" \
  --path="electron-aaos-core/tasks/create-next-story.md" \
