...
Configure complete GitHub DevOps infrastructure for user projects created with Electron AAOS. This task copies GitHub Actions workflows, configures CodeRabbit, sets up branch protection, and manages secrets.
...
- campo: devops_setup_report
  tipo: object
  destino: File system (.electron-aaos/devops-setup-report.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Not already configured (idempotency check)
    ...
    blocker: false
    validação: |
      Check .electron-aaos/devops-setup-report.yaml existence
...
post-conditions:
  ...
  - [ ] DevOps setup report generated
    ...
    blocker: false
    validação: |
      Test-Path ".electron-aaos/devops-setup-report.yaml"
...
acceptance-criteria:
  ...
  - [ ] Setup report documents all configurations
    ...
    blocker: true
    validação: |
      .electron-aaos/devops-setup-report.yaml contains all setup details
...
# Check idempotency
if (Test-Path ".electron-aaos/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content ".electron-aaos-core/infrastructure/templates/github-workflows/ci.yml.template" -Raw
...
Copy-Item ".electron-aaos-core/infrastructure/templates/github-workflows/pr-automation.yml.template" `
  -Destination ".github/workflows/pr-automation.yml"
Write-Host "✅ Installed pr-automation.yml"

# Copy release.yml
Copy-Item ".electron-aaos-core/infrastructure/templates/github-workflows/release.yml.template" `
...
$coderabbitConfig = Get-Content ".electron-aaos-core/infrastructure/templates/coderabbit.yaml.template" -Raw
...
$report = @"
# Electron AAOS DevOps Setup Report
...
# Ensure .electron-aaos directory exists
New-Item -ItemType Directory -Path ".electron-aaos" -Force | Out-Null

$report | Out-File -FilePath ".electron-aaos/devops-setup-report.yaml" -Encoding utf8

Write-Host "✅ Setup report saved to .electron-aaos/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .electron-aaos/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .electron-aaos/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝
