...
- **Tool:** validation-engine
  ...
  - **Source:** .electron-aaos-core/utils/validation-engine.js
...
- **Script:** run-validation.js
  ...
  - **Language:** JavaScript
  - **Location:** .electron-aaos-core/scripts/run-validation.js
...
# Review Proposal - Electron AAOS Developer Task
...
class ReviewProposalTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('📋 Electron AAOS Proposal Review'));
  ...
  async loadProposal(proposalId) {
    try {
      const proposalFile = path.join(this.rootPath, '.electron-aaos', 'proposals', `${proposalId}.json`);
  ...
  async checkForConflicts(proposal) {
    ...
    const indexFile = path.join(this.rootPath, '.electron-aaos', 'proposals', 'index.json');
  ...
  async getReviewDetails(proposal, analysis, config) {
    ...
    // Set reviewer info
    details.reviewer = process.env.USER || 'electron-aaos-reviewer';
  ...
  async storeReview(proposal, review) {
    const reviewsDir = path.join(this.rootPath, '.electron-aaos', 'proposals', 'reviews');
  ...
  async updateProposalStatus(proposal, review) {
    ...
    const proposalFile = path.join(this.rootPath, '.electron-aaos', 'proposals', `${proposal.proposalId}.json`);
