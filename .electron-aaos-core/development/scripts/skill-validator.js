/**
 * Electron AAOS Skill Validator
 * Story GEMINI-INT.5 - Skills Cross-CLI Compatibility
 *
 * Validates that Electron AAOS agent skills work correctly in both
...
class SkillValidator {
  constructor(config = {}) {
    this.agentsDir = config.agentsDir || path.join(process.cwd(), '.electron-aaos-core', 'development', 'agents');
    this.tasksDir = config.tasksDir || path.join(process.cwd(), '.electron-aaos-core', 'development', 'tasks');
    this.errors = [];
    this.warnings = [];
  }
  ...
  validateGeminiCompatibility(agentDef, result) {
    // Gemini CLI uses .gemini/rules/Electron AAOS/agents/ structure
  ...
  generateReport(results) {
    let report = '# Electron AAOS Skills Cross-CLI Compatibility Report\n\n';
