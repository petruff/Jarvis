...
async function loadAgentRequirements() {
  ...
  const requirementsPath = path.join(process.cwd(), '.electron-aaos-core', 'data', 'agent-config-requirements.yaml');
...
class AgentConfigLoader {
  ...
  async loadAgentDefinition(options = {}) {
    ...
    // Load from file
    const agentPath = path.join(process.cwd(), '.electron-aaos-core', 'development', 'agents', `${this.agentId}.md`);
      ...
      switch (command) {
        ...
        case 'preload': {
          const agents = agentId ? [agentId] : [
            'electron-aaos-master', 'dev', 'qa', 'architect', 'po', 'pm', 'sm',
