...
/**
 * Default Electron AAOS minimum version
 * @constant {string}
 */
const DEFAULT_Electron AAOS_MIN_VERSION = '2.1.0';
...
class SquadGeneratorError extends Error {
  ...
  static schemaNotFound(schemaPath) {
    return new SquadGeneratorError(
      ...
      'Ensure Electron AAOS is properly installed',
    );
  }
}
...
electron-aaos:
  minVersion: "${config.electron-aaosMinVersion || DEFAULT_Electron AAOS_MIN_VERSION}"
  type: squad
...
${config.description || 'Custom Electron AAOS squad.'}
...
This squad ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'does not inherit'} the core Electron AAOS configuration.
...
> This file ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'is independent of'} the core Electron AAOS coding standards.
...
- Node.js >= 18.x
- Electron AAOS >= ${config.electron-aaosMinVersion || DEFAULT_Electron AAOS_MIN_VERSION}
...
class SquadGenerator {
   ...
   * @param {string} [config.electron-aaosMinVersion] - Minimum Electron AAOS version
  ...
  async generate(config) {
    ...
    const fullConfig = {
      ...
      electron-aaosMinVersion: config.electron-aaosMinVersion || DEFAULT_Electron AAOS_MIN_VERSION,
...
module.exports = {
  ...
  DEFAULT_Electron AAOS_MIN_VERSION,
