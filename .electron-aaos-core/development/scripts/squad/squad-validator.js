      ...
      try {
        ...
        if (!hasYamlFrontmatter && !hasMarkdownHeading) {
          result.warnings.push({
            ...
            file: agentFile,
            message: 'Agent file may not follow Electron AAOS agent definition format',
            suggestion:
  ...
  async validateWorkflows(squadPath) {
    ...
    try {
      ...
      result.warnings.push({
        ...
        suggestion: 'Ensure workflow-validator.js exists in .electron-aaos-core/development/scripts/',
      });
      return result;
    }

    const coreAgentsPath = path.join(process.cwd(), '.electron-aaos-core', 'development', 'agents');
