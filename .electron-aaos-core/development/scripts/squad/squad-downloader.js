 ...
 *
 * Downloads squads from the electron-aaos-squads GitHub repository.
...
/**
 * Default registry URL for electron-aaos-squads
 * @constant {string}
 */
const REGISTRY_URL =
  'https://raw.githubusercontent.com/Electron AAOSAI/electron-aaos-squads/main/registry.json';

/**
 * GitHub API base URL for electron-aaos-squads contents
 * @constant {string}
 */
const GITHUB_API_BASE =
  'https://api.github.com/repos/Electron AAOSAI/electron-aaos-squads/contents/packages';
...
/**
 * Squad Downloader class for downloading squads from electron-aaos-squads repository
 */
class SquadDownloader {
    ...
    if (registry.squads && registry.squads.official) {
      for (const squad of registry.squads.official) {
        squads.push({
          ...
          type: 'official',
          author: squad.author || 'Electron AAOSAI',
        });
      }
    }
  ...
  /**
   * Fetch registry from electron-aaos-squads repository
   *
  ...
  async _downloadSquadFiles(squadInfo, targetPath) {
    ...
    if (!Array.isArray(contents)) {
      throw new SquadDownloaderError(
        ...
        'Check if squad exists in electron-aaos-squads repository',
      );
    }
  ...
  _fetch(url, useApi = false) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'Electron AAOS-SquadDownloader/1.0',
        },
      };
