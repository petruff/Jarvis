...
class SquadMigrator {
  ...
  async analyze(squadPath) {
    ...
    try {
      ...
      // Check for missing electron-aaos.type
      if (!manifest.electron-aaos?.type) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_Electron AAOS_TYPE',
          message: 'Missing required field: electron-aaos.type',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'electron-aaos.type',
          value: 'squad',
        });
      }

      // Check for missing electron-aaos.minVersion
      if (!manifest.electron-aaos?.minVersion) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_MIN_VERSION',
          message: 'Missing required field: electron-aaos.minVersion',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'electron-aaos.minVersion',
          value: '2.1.0',
        });
      }
