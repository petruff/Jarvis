 ...
 * Can be overridden via core-config.yaml pipeline.timeout_ms or Electron AAOS_PIPELINE_TIMEOUT env var.
 * @type {number}
 */
...
const ALL_AGENT_IDS = [
  ...
  'devops', 'electron-aaos-master', 'squad-creator',
];
...
class UnifiedActivationPipeline {
  ...
  async _loadCoreConfig() {
    try {
      const configPath = path.join(this.projectRoot, '.electron-aaos-core', 'core-config.yaml');
   ...
   * Priority: Electron AAOS_PIPELINE_TIMEOUT env > core-config.yaml pipeline.timeout_ms > default
   * @private
  ...
  _resolvePipelineTimeout(coreConfig) {
    ...
    const envTimeout = process.env.Electron AAOS_PIPELINE_TIMEOUT;
    if (envTimeout) {
