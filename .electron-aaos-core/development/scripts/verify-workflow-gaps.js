 ...
 * Usage: node .electron-aaos-core/development/scripts/verify-workflow-gaps.js
 *
 * @version 1.0.0
 */
...
function verifyGap1() {
  ...
  const createWf = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/development/tasks/create-workflow.md'), 'utf-8');
  assert(
  ...
  const modifyWf = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/development/tasks/modify-workflow.md'), 'utf-8');
  assert(
  ...
  const steps = require('../../../.electron-aaos-core/core/elicitation/workflow-elicitation.js');
  ...
  const template = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/product/templates/workflow-template.yaml'), 'utf-8');
  assert(
...
async function verifyGap2() {
  ...
  try {
    ({ WorkflowValidator } = require('../../../.electron-aaos-core/development/scripts/workflow-validator'));
  ...
  const goodResult = await validator.validate(path.join(ROOT, '.electron-aaos-core/development/workflows/greenfield-service.yaml'));
  assert(
  ...
  const strictResult = await strictValidator.validate(path.join(ROOT, '.electron-aaos-core/development/workflows/greenfield-service.yaml'));
  assert(
  ...
  const { SquadValidator } = require('../../../.electron-aaos-core/development/scripts/squad/squad-validator');
  ...
  const FrameworkAnalyzer = require('../../../.electron-aaos-core/infrastructure/scripts/framework-analyzer');
  ...
  assert(
    fs.existsSync(path.join(ROOT, '.electron-aaos-core/development/tasks/validate-workflow.md')),
    '2.12 validate-workflow.md task file exists',
    'File not found',
  );

  // 2.13 electron-aaos-master has validate-workflow command
  const masterMd = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/development/agents/electron-aaos-master.md'), 'utf-8');
  assert(
    masterMd.includes('name: validate-workflow'),
    '2.13a electron-aaos-master has validate-workflow command',
    'Command not found in electron-aaos-master.md',
  );
  assert(
    masterMd.includes('validate-workflow.md'),
    '2.13b electron-aaos-master has validate-workflow.md in dependencies',
...
async function verifyGap3() {
  ...
  try {
    ({ WorkflowStateManager } = require('../../../.electron-aaos-core/development/scripts/workflow-state-manager'));
  ...
  try {
    ...
    const wfContent = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/development/workflows/greenfield-service.yaml'), 'utf-8');
    ...
    const patterns = yaml.load(fs.readFileSync(path.join(ROOT, '.electron-aaos-core/data/workflow-patterns.yaml'), 'utf-8'));
    assert(
    ...
    const schemaContent = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/data/workflow-state-schema.yaml'), 'utf-8');
    ...
    assert(
      fs.existsSync(path.join(ROOT, '.electron-aaos-core/development/tasks/run-workflow.md')),
      '3.21 run-workflow.md task file exists',
      'File not found',
    );

    // 3.22 electron-aaos-master has run-workflow command
    const masterMd = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/development/agents/electron-aaos-master.md'), 'utf-8');
    assert(
      masterMd.includes('name: run-workflow'),
      '3.22a electron-aaos-master has run-workflow command',
      'Command not found',
    );
    assert(
      masterMd.includes('run-workflow.md'),
      '3.22b electron-aaos-master has run-workflow.md in dependencies',
      'Dependency not found',
    );
...
async function verifyGap4() {
  ...
  const { WorkflowValidator, WorkflowValidationErrorCodes } = require('../../../.electron-aaos-core/development/scripts/workflow-validator');
  ...
  try {
    ...
    const { WorkflowStateManager } = require('../../../.electron-aaos-core/development/scripts/workflow-state-manager');
    ...
    for (const taskFile of taskFiles) {
      const content = fs.readFileSync(path.join(ROOT, '.electron-aaos-core/development/tasks', taskFile), 'utf-8');
      assert(
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('  Electron AAOS Workflow Gaps — Verification Suite');
