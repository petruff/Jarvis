 ...
 *
 * Analyzes all 13 Electron AAOS agents to determine which core-config.yaml sections
...
/**
 * List of all Electron AAOS agents
 */
const AGENTS = [
...
async function extractAgentDependencies(agentId) {
  const agentPath = path.join('.electron-aaos-core', 'agents', `${agentId}.md`);
...
function analyzeConfigNeeds(agentData) {
  ...
  if (agentData.agentId === 'electron-aaos-master' ||
...
*Auto-generated by Electron AAOS Agent Config Audit (Story 6.1.2.6)*
`;
...
async function auditAllAgents() {
  console.log('🔍 Auditing all 13 Electron AAOS agents...\n');
