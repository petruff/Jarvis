...
activation-instructions:
  ...
  - STEP 2.5: |
      ...
      Check user_profile using config-resolver's resolveConfig():
        ...
        - If user_profile === 'bob':
          → Load bob-orchestrator.js module from .electron-aaos-core/core/orchestration/bob-orchestrator.js
      ...
      Module: .electron-aaos-core/core/config/config-resolver.js
  ...
  - STEP 3.5: |
      ...
      1. Run data lifecycle cleanup first:
         - const { runStartupCleanup } = require('.electron-aaos-core/core/orchestration/data-lifecycle-manager')
         - await runStartupCleanup(projectRoot) // Cleanup locks, sessions >30d, snapshots >90d

      2. Check for existing session state:
         - const { BobOrchestrator } = require('.electron-aaos-core/core/orchestration/bob-orchestrator')
      ...
      Module: .electron-aaos-core/core/orchestration/bob-orchestrator.js (Story 12.5)
      Module: .electron-aaos-core/core/orchestration/data-lifecycle-manager.js (Story 12.5)
      Task: .electron-aaos-core/development/tasks/session-resume.md
...
persona:
  ...
  orchestration_constraints:
    ...
    integration:
      module: .electron-aaos-core/core/orchestration/terminal-spawner.js
      script: .electron-aaos-core/scripts/pm.sh
      executor_assignment: .electron-aaos-core/core/orchestration/executor-assignment.js
...
commands:
  ...
  # NOTE: correct-course removed - delegated to @electron-aaos-master
  # See: docs/architecture/command-authority-matrix.md
  # For course corrections → Escalate to @electron-aaos-master using *correct-course
...
- Course corrections → Escalate to @electron-aaos-master using `*correct-course`
...
| Course correction | @electron-aaos-master | `*correct-course` |
...
| @electron-aaos-master | Framework modification | `*create-brownfield-prd` |
...
2. PRD templates in `.electron-aaos-core/product/templates/`
...
6. **Course correction** → Escalate to `@electron-aaos-master *correct-course` if deviations detected
