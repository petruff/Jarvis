...
Electron AAOS Pro operates as an open-core CLI product: `electron-aaos-core` is MIT-licensed and free, while `electron-aaos-pro` is a proprietary git submodule with premium squads, persistent memory, usage metrics, and enterprise integrations. The existing infrastructure (PRO-1 through PRO-8) provides feature gating, license key validation, encrypted local cache, machine fingerprinting, and graceful degradation.
...
6. **Open source** (Apache 2.0) -- aligns with Electron AAOS open-core philosophy
...
| Feature | Free (electron-aaos-core) | Pro Individual | Pro Team | Enterprise |
|---------|-------------------|----------------|----------|------------|
...
| Command | Purpose |
|---------|---------|
| `electron-aaos pro devices` | List registered devices with seat usage |
| `electron-aaos pro device-reset --slot N` | Remote deactivation of a device slot |
...
| Docker/CI | Not designed for ephemeral environments; use electron-aaos-core (free) for CI |
...
```
User clicks "Buy Pro" on electron-aaos.ai/pro
  --> Redirect to Polar.sh hosted checkout
    --> Payment succeeds
      --> Polar.sh fires webhook POST to api.electron-aaos.ai/v1/webhook/polar
        ...
        --> Send email via Resend with key + activation command
          --> User runs: electron-aaos pro activate --key PRO-XXXX
```
...
The checkout success page (`electron-aaos.ai/pro/thank-you?order_id={id}`) displays the license key immediately, providing a safety net if email delivery fails.
...
```
User visits electron-aaos.ai/pro/recover (or runs: electron-aaos pro recover --email user@example.com)
  --> Enters email
...
| Pattern | Source | Electron AAOS Pro Application |
|---------|--------|---------------------|
| Credential delivery via email | Sidekiq Pro | Webhook -> generate key -> email |
| Environment variable for CI | Nx Powerpack, Cal.com | `Electron AAOS_PRO_LICENSE=<key>` |
...
| CLI activation command | Nx Powerpack | `electron-aaos pro activate --key <KEY>` (exists) |
...
- `electron-aaos pro devices` and `electron-aaos pro device-reset` CLI commands
...
- `electron-aaos pro recover --email` CLI command
...
| `.electron-aaos-core/cli/commands/pro/index.js` | ADD `devices`, `device-reset`, `recover` subcommands | PRO-12/13 |
...
- Electron AAOS Pro runs locally with 30-day offline support
