# Electron AAOS Source Tree Structure
...
**Repository:** Electron AAOSAI/electron-aaos-core
...
- [Framework Core (.electron-aaos-core/)](#framework-core-electron-aaos-core)
...
Electron AAOS uses a **modular architecture** with clear separation of concerns:

1. **Framework Core** (`.electron-aaos-core/`) - Portable framework components organized by domain
...
```
electron-aaos-core/                             # Root project
├── .electron-aaos-core/                        # Framework core (modular)
...
│
├── bin/                               # CLI executables
│   └── electron-aaos.js                        # Main CLI entry point
│
...
## Framework Core (.electron-aaos-core/)
...
│
├── data/                              # Shared Data
│   ├── electron-aaos-kb.md                     # Electron AAOS knowledge base (@electron-aaos-master, lazy-loaded)
...
│   │   ├── electron-aaos-master.md             # Master orchestrator
...
```yaml
Agents:
  Location: .electron-aaos-core/development/agents/
  Format: Markdown with YAML frontmatter
  Naming: {agent-name}.md (kebab-case)
  Example: dev.md, qa.md, architect.md

Tasks:
  Location: .electron-aaos-core/development/tasks/
...
Workflows:
  Location: .electron-aaos-core/development/workflows/
  Format: YAML
...
### Shared Data Files (.electron-aaos-core/data/)

| File | Owner | Fill Rule | Update Trigger | Used By |
|------|-------|-----------|----------------|---------|
| `electron-aaos-kb.md` | @electron-aaos-master | Updated on major framework changes | Manual edit | @electron-aaos-master (lazy) |
...
### Product Data Files (.electron-aaos-core/product/data/)
...
│   ├── electron-aaos migration/                # Electron AAOS migration stories
│   │   ├── story-6.1.2.1.md
│   │   ├── story-6.1.2.2.md
│   │   ├── story-6.1.2.3.md
│   │   ├── story-6.1.2.4.md
...
Squads are modular extensions that add specialized capabilities to Electron AAOS. Unlike the deprecated Squads, Squads follow a standardized template structure.
...
```bash
# Future CLI (planned):
npx create-electron-aaos-squad my-squad-name
...
### REPO 1: Electron AAOSAI/electron-aaos-core (Commons Clause)

```
electron-aaos-core/
├── .electron-aaos-core/                        # Framework assets (modular v2.0)
│   ├── cli/                           # CLI commands and utilities
│   ├── core/                          # Framework essentials
│   │   ├── config/                    # Configuration system
...
### REPO 2: Electron AAOSAI/squads (MIT)

```
squads/
├── verified/                          # Electron AAOS-curated squads
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
└── tools/                             # Squad development tools
    └── create-electron-aaos-squad/
```

### REPO 3: Electron AAOSAI/mcp-ecosystem (Apache 2.0)

```
mcp-ecosystem/
├── presets/                           # MCP presets (Docker MCP Toolkit)
│   ├── electron-aaos-dev/
│   ├── electron-aaos-research/
│   └── electron-aaos-docker/
│
...
### REPO 4: Electron AAOSAI/certified-partners (Private)
...
### REPO 5: Electron AAOSAI/mmos (Private + NDA)
...
Location: .electron-aaos-core/development/agents/{agent-name}.md
Example: .electron-aaos-core/development/agents/security-expert.md

# I'm creating a new task:
Location: .electron-aaos-core/development/tasks/{task-name}.md
Example: .electron-aaos-core/development/tasks/deploy-to-production.md

# I'm creating a new workflow:
Location: .electron-aaos-core/development/workflows/{workflow-name}.yaml
Example: .electron-aaos-core/development/workflows/continuous-deployment.yaml

# I'm creating a new template:
Location: .electron-aaos-core/product/templates/{template-name}-tmpl.{yaml|md}
Example: .electron-aaos-core/product/templates/deployment-plan-tmpl.yaml

# I'm creating a new checklist:
Location: .electron-aaos-core/product/checklists/{checklist-name}-checklist.md
Example: .electron-aaos-core/product/checklists/security-review-checklist.md

# I'm creating a CLI command:
Location: .electron-aaos-core/cli/commands/{category}/{command-name}.js
Example: .electron-aaos-core/cli/commands/generate/workflow.js

# I'm creating a core utility:
Location: .electron-aaos-core/core/utils/{utility-name}.js
Example: .electron-aaos-core/core/utils/performance-monitor.js

# I'm creating an infrastructure script:
Location: .electron-aaos-core/infrastructure/scripts/{category}/{script-name}.js
Example: .electron-aaos-core/infrastructure/scripts/llm-routing/router.js

# I'm adding a PM tool adapter:
Location: .electron-aaos-core/infrastructure/integrations/pm-adapters/{adapter-name}.js
Example: .electron-aaos-core/infrastructure/integrations/pm-adapters/monday-adapter.js
...
The ADE is configured via `.electron-aaos-core/core-config.yaml`:
...
ade:
  ...
  memoryLayer:
    enabled: true
    patternStore: .electron-aaos/patterns/
```

### ADE Runtime State

Runtime state is persisted in `.electron-aaos/`:
...
| 1.1     | 2025-12-14 | Updated org to Electron AAOSAI, replaced Squads with Squads system [Story 6.10]                                                                                           | Dex (dev)        |
...
| 3.1     | 2026-02-06 | Added Data File Governance section: documented 7 missing data files with owner, fill rule, and update trigger. Expanded .electron-aaos-core/data/ and product/data/ tree listings. [Story ACT-8] | Dex (dev) |

---

_This is an official Electron AAOS framework standard. All file placement must follow this structure._
