...
# Estructura del Árbol de Código Fuente Electron AAOS
...
**Repositorio:** Electron AAOSAI/electron-aaos-core
...
- [Core del Framework (.electron-aaos-core/)](#core-del-framework-electron-aaos-core)
...
Electron AAOS utiliza una **arquitectura modular** con clara separación de responsabilidades:

1. **Core del Framework** (`.electron-aaos-core/`) - Componentes portables del framework organizados por dominio
...
```
electron-aaos-core/                             # Proyecto raíz
├── .electron-aaos-core/                        # Core del framework (modular)
...
│
├── bin/                               # Ejecutables CLI
│   └── electron-aaos.js                        # Punto de entrada principal CLI
│
...
## Core del Framework (.electron-aaos-core/)
...
│   └── electron-aaos-kb.md                     # Base de conocimiento Electron AAOS
│
├── development/                       # Assets de Desarrollo
│   ├── agents/                        # Definiciones de agentes (11 agentes core)
│   │   ├── electron-aaos-master.md             # Orquestador maestro
...
│   ├── electron-aaos migration/                # Stories de migración Electron AAOS
│   │   ├── story-6.1.2.1.md
│   │   ├── story-6.1.2.2.md
│   │   ├── story-6.1.2.3.md
│   │   ├── story-6.1.2.4.md
...
Los Squads son extensiones modulares que agregan capacidades especializadas a Electron AAOS. A diferencia de los Squads deprecados, los Squads siguen una estructura de template estandarizada.
...
```
electron-aaos-core/
├── .electron-aaos-core/                        # Assets del framework (modular v2.0)
│   ├── cli/                           # Comandos CLI y utilidades
│   ├── core/                          # Esenciales del framework
│   │   ├── config/                    # Sistema de configuración
...
│   └── electron-aaos.js                        # CLI principal
│
├── tools/                             # Herramientas de build y utilidad
│   ├── cli.js                         # Constructor CLI
│   └── installer/                     # Scripts de instalación
...
```
squads/
├── verified/                          # Squads curados por Electron AAOS
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
### REPO 4: Electron AAOSAI/certified-partners (Privado)
...
### REPO 5: Electron AAOSAI/mmos (Privado + NDA)
...
Ubicación: .electron-aaos-core/development/agents/{agent-name}.md
Ejemplo: .electron-aaos-core/development/agents/security-expert.md

# Estoy creando una nueva tarea:
Ubicación: .electron-aaos-core/development/tasks/{task-name}.md
Ejemplo: .electron-aaos-core/development/tasks/deploy-to-production.md

# Estoy creando un nuevo workflow:
Ubicación: .electron-aaos-core/development/workflows/{workflow-name}.yaml
Ejemplo: .electron-aaos-core/development/workflows/continuous-deployment.yaml

# Estoy creando un nuevo template:
Ubicación: .electron-aaos-core/product/templates/{template-name}-tmpl.{yaml|md}
Ejemplo: .electron-aaos-core/product/templates/deployment-plan-tmpl.yaml

# Estoy creando un nuevo checklist:
Ubicación: .electron-aaos-core/product/checklists/{checklist-name}-checklist.md
Ejemplo: .electron-aaos-core/product/checklists/security-review-checklist.md

# Estoy creando un comando CLI:
Ubicación: .electron-aaos-core/cli/commands/{category}/{command-name}.js
Ejemplo: .electron-aaos-core/cli/commands/generate/workflow.js

# Estoy creando una utilidad core:
Ubicación: .electron-aaos-core/core/utils/{utility-name}.js
Ejemplo: .electron-aaos-core/core/utils/performance-monitor.js

# Estoy creando un script de infraestructura:
Ubicación: .electron-aaos-core/infrastructure/scripts/{category}/{script-name}.js
Ejemplo: .electron-aaos-core/infrastructure/scripts/llm-routing/router.js

# Estoy agregando un adaptador de herramienta PM:
Ubicación: .electron-aaos-core/infrastructure/integrations/pm-adapters/{adapter-name}.js
Ejemplo: .electron-aaos-core/infrastructure/integrations/pm-adapters/monday-adapter.js
...
| 1.1     | 2025-12-14 | Actualizada org a Electron AAOSAI, reemplazado Squads con sistema de Squads [Story 6.10]                                          | Dex (dev)        |
...
_Este es un estándar oficial del framework Electron AAOS. Toda colocación de archivos debe seguir esta estructura._
