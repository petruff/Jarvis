...
# Guía de Solución de Problemas de Electron AAOS Electron AAOS
...
### Problema 1: "npx @electron-aaos/electron-aaos-core no se reconoce"
...
Electron AAOS Electron AAOS should be installed in your project directory,
...
```
Error: Electron AAOS Electron AAOS requires Node.js 18.0.0 or higher
...
```
npm ERR! code E404
npm ERR! 404 Not Found - GET https://registry.npmjs.org/@electron-aaos/electron-aaos-core
```
...
npm ERR! network request to https://registry.npmjs.org/@electron-aaos/electron-aaos-core failed
```
...
npm install @electron-aaos/electron-aaos-core --verbose
```
...
```
Error: ENOTEMPTY: directory not empty, rmdir '.electron-aaos-core'
```
...
mv .electron-aaos-core .electron-aaos-core.backup
...
cp .electron-aaos-core.backup/custom-files/* .electron-aaos-core/
```
...
2. Verifique que los archivos fueron creados:
   ...
   ```bash
   # Claude Code
   ls .claude/commands/Electron AAOS/agents/
...
```
Error: Agent 'dev' not found in .electron-aaos-core/agents/
```
...
npx yaml-lint .electron-aaos-core/agents/dev.md
...
mv .electron-aaos-core/agents/dev.md .electron-aaos-core/agents/dev.md.backup
npx @electron-aaos/electron-aaos-core install --full
```
...
```bash
# Verificar estado de instalación de Electron AAOS
npx @electron-aaos/electron-aaos-core status
...
npx @electron-aaos/electron-aaos-core install --verbose
```
...
```bash
# Verificar estructura de .electron-aaos-core
find .electron-aaos-core -type f | wc -l
# Esperado: 200+ archivos

# Verificar YAML corrupto
for f in .electron-aaos-core/**/*.yaml; do npx yaml-lint "$f"; done
...
1. Ejecute `npx @electron-aaos/electron-aaos-core status` y anote la salida
2. Revise esta guía de solución de problemas
3. Busque [Issues de GitHub](https://github.com/Electron AAOSAI/electron-aaos-core/issues) existentes
...
- **Issues de GitHub**: [@electron-aaos/electron-aaos-core/issues](https://github.com/Electron AAOSAI/electron-aaos-core/issues)
