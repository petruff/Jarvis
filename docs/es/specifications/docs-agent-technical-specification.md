...
**@docs (Ajax)** es un agente de IA especializado en crear **documentacion educativa** para Electron AAOS. A diferencia de la documentacion tecnica (JSDoc/Typedoc), Ajax se enfoca en **tutoriales, guias de incorporacion, explicadores visuales y materiales de capacitacion para socios**.
...
whenToUse: |
  ...
  - Tutoriales explicando como usar funciones de Electron AAOS
...
Entrada:
  ...
  - campo: agent_definition_path
    ...
    obligatorio: false
    example: "expansion-packs/electron-aaos/agents/dev/agent.yaml"
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Usa estilo consistente (paleta de colores Electron AAOS)"
      blocker: false
...
whenToUse: |
  ...
  - Nuevo socio se une al ecosistema Electron AAOS
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Enlaces a documentacion Electron AAOS relevante"
      blocker: false
...
- Email [support@electron-aaos.dev]
...
[Nombre del Agente] es el agente [rol] de Electron AAOS, especializado en [experiencia principal].
...
- Email: partners@electron-aaos.dev
...
**Notas de Plantilla:**
- Usar paleta de colores Electron AAOS (Azul primario, Purpura secundario, Rosa terciario)
...
```bash
#!/bin/bash
# Hook de Auto-Actualizacion Electron AAOS @docs
...
if echo "$changed_files" | grep -qE "^(src/|electron-aaos-core/)"; then
  echo "Cambios de codigo detectados. Analizando impacto en documentacion..."

  # Invocar tarea de actualizacion de @docs
  electron-aaos agent invoke @docs --task update-educational-docs \
    --files "$changed_files" \
    --mode interactive
...
jobs:
  check-docs:
    ...
    steps:
      ...
      - name: Instalar Electron AAOS CLI
        run: npm install -g @electron-aaos/core
  ...
  qa-docs:
    ...
    steps:
      ...
      - name: Ejecutar QA de Documentacion
        id: qa
        run: |
          electron-aaos agent invoke @docs --task qa-documentation \
            --paths "docs/**/*.md" \
...
- [ ] Crear `expansion-packs/electron-aaos/agents/docs/agent.yaml`
...
- [ ] **Reconocimiento de Industria:** Docs de Electron AAOS citados como "mejor en espacio de agentes IA"
...
Con la adicion de @docs (Ajax), Electron AAOS ahora tiene **13 agentes nombrados**:
...
| 13 | @electron-aaos-master | Orion | Comandante | Aries | Cian |
