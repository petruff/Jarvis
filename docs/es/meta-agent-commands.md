...
Guia de referencia completa para todos los comandos del meta-agente de Electron AAOS Electron AAOS.
...
Actualiza Electron AAOS Electron AAOS.
...
Electron AAOS> create-agent helper
Electron AAOS> activate helper
Electron AAOS> helper translate "Hello" --to spanish
Electron AAOS> exit
```
...
```bash
Electron AAOS_AI_PROVIDER=openai          # Proveedor de IA
Electron AAOS_AI_MODEL=gpt-4             # Modelo de IA
Electron AAOS_LOG_LEVEL=debug            # Nivel de log
Electron AAOS_TELEMETRY=disabled         # Configuracion de telemetria
Electron AAOS_TIMEOUT=30000             # Timeout de comando (ms)
Electron AAOS_MEMORY_CACHE=true         # Cache de memoria
```
