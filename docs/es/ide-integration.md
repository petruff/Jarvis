...
Guía para integrar Electron AAOS con IDEs compatibles y plataformas de desarrollo con IA.
...
Electron AAOS es compatible con 9 plataformas de desarrollo potenciadas por IA. Elige la que mejor se adapte a tu flujo de trabajo.
...
**Nivel de Recomendación:** Mejor integración con Electron AAOS
...
1. Electron AAOS crea automáticamente el directorio `.claude/` al inicializar
...
1. Electron AAOS crea el directorio `.cursor/` al inicializar
...
1. Electron AAOS crea el directorio `.windsurf/` y el archivo `.windsurfrules`
...
2. Electron AAOS crea el directorio `.cline/` al inicializar
...
2. Electron AAOS crea `.github/copilot-instructions.md`
...
1. Electron AAOS crea el directorio `.antigravity/`
...
Electron AAOS mantiene una única fuente de verdad para las definiciones de agentes y las sincroniza con todos los IDEs configurados:

```
┌─────────────────────────────────────────────────────┐
│                    Electron AAOS Core                         │
│  .electron-aaos-core/development/agents/  (Fuente de Verdad) │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
...
Electron AAOS puede configurarse para sincronizar automáticamente cuando hay cambios en los agentes:
...
├── Sí --> Claude Code (Mejor integración con Electron AAOS)
└── No
    └── ¿Prefieres VS Code?
...
_Guía de Integración con IDEs de Electron AAOS Electron AAOS v2.1.0_
