...
Este documento especifica las capacidades de auto-reparación del Sistema de Verificación de Salud de Electron AAOS (HCS). La auto-reparación permite que el sistema corrija automáticamente ciertos problemas mientras mantiene la seguridad y el control del usuario.
...
| ---------------------- | ----------------------------------------------------- | ------------ |
| `recreate_config`      | Recrear `.electron-aaos/config.yaml` faltante desde plantilla  | Sí           |
| `clear_cache`          | Limpiar archivos de caché obsoletos en `.electron-aaos/cache/` | Sí           |
...
- id: PC-001
  description: '.electron-aaos/config.yaml faltante'
  severity: CRITICAL
  tier: 1
...
✅ Auto-corregidos 3 problemas:
   • Recreado .electron-aaos/config.yaml (respaldo: .electron-aaos/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Agregado .electron-aaos/cache/ a .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      - 'Abrir .electron-aaos-core/development/tasks/deploy.md'
      - 'Encontrar línea: agent: legacy-dev'
      - 'Reemplazar con: agent: devops'
      - 'Verificar con: npx electron-aaos task validate deploy'
...
Pasos para resolver:
  1. Abrir .electron-aaos-core/development/tasks/deploy.md
  2. Encontrar línea: agent: legacy-dev
  3. Reemplazar con: agent: devops
  4. Verificar con: npx electron-aaos task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config de usuario (fuera de .electron-aaos/)
...
safeToAutoFix:
  paths:
    - '.electron-aaos/**' # Archivos del workspace Electron AAOS
    - '.electron-aaos-core/**/*.yaml' # YAML del framework (con cuidado)
...
```json
{
  ...
  "files": [
    {
      "original": ".electron-aaos/config.yaml",
      "backup": "config.yaml",
      "action": "recreate_config",
  ...
  "rollbackCommand": "npx electron-aaos health-check --rollback health-check-2025-12-30T10-30-00"
}
```
...
```yaml
# .electron-aaos/backups/.retention
retention:
...
async function createBackup(action) {
  ...
  const backupDir = path.join('.electron-aaos', 'backups', backupId);
...
npx electron-aaos health-check --rollback health-check-2025-12-30T10-30-00

# Listar respaldos disponibles
npx electron-aaos health-check --list-backups

# Revertir último respaldo
npx electron-aaos health-check --rollback-last
```
...
```
.electron-aaos-core/core/health-check/
├── healers/
...
class RecreateConfigHealer extends BaseHealer {
  ...
  async canHeal(issue) {
    return issue.id === 'PC-001' && !(await fs.pathExists('.electron-aaos/config.yaml'));
  }

  async heal(issue, context) {
    const template = await fs.readFile('.electron-aaos-core/templates/config-template.yaml');
    await fs.writeFile('.electron-aaos/config.yaml', template);
    return { success: true, message: 'Config recreado desde plantilla' };
  }

  async verify(issue) {
    return await fs.pathExists('.electron-aaos/config.yaml');
  }
}
```
...
```javascript
// .electron-aaos/logs/self-healing.log
const healingLog = {
  append: async (entry) => {
    const logPath = '.electron-aaos/logs/self-healing.log';
    const logEntry = {
