...
sequenceDiagram
    ...
    Detector-->>CLI: ~home/.electron-aaos/mcp
    CLI->>Mgr: createGlobalStructure()
    Mgr->>FS: mkdir ~/.electron-aaos/mcp/servers
    Mgr->>FS: mkdir ~/.electron-aaos/mcp/cache
...
sequenceDiagram
    ...
    Symlink->>FS: create symlink/junction ./mcp → ~/.electron-aaos/mcp
    FS-->>Symlink: linked
    Symlink-->>CLI: success
