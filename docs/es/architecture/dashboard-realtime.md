# 🔴 Electron AAOS Dashboard - Arquitectura de Observabilidad en Tiempo Real
...
Este documento describe la arquitectura para **observabilidad en tiempo real** del Electron AAOS Dashboard, permitiendo que los usuarios monitoreen comandos ejecutados en la CLI con máximo detalle visual.
...
│                                   ▼                                      │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │               .electron-aaos/dashboard/events.jsonl (append-only)         │    │
...
```
.electron-aaos-core/core/events/dashboard-emitter.ts
```

### Interfaz

```typescript
// .electron-aaos-core/core/events/types.ts
...
const EVENTS_DIR = '.electron-aaos/dashboard';
...
```typescript
// .electron-aaos-core/integrations/claude-code/hooks.ts
...
```
.electron-aaos/dashboard/events.jsonl
```
...
const Electron AAOS_DIR = process.env.Electron AAOS_PROJECT_ROOT || process.cwd();
const STATUS_FILE = join(Electron AAOS_DIR, '.electron-aaos/dashboard/status.json');
const EVENTS_FILE = join(Electron AAOS_DIR, '.electron-aaos/dashboard/events.jsonl');
...
│                                                                              │
│  .electron-aaos/dashboard/events.jsonl (SOLO ALTO NIVEL)                             │
│  ─────────────────────────────────────────────────                          │
