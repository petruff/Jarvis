...
# Stack Tecnologico Electron AAOS
...
Electron AAOS esta construido sobre JavaScript/TypeScript moderno con runtime Node.js, optimizado para desarrollo CLI multiplataforma con UX interactivo y capacidades de orquestacion de agentes.
...
Electron AAOS se integra con servidores Model Context Protocol (MCP):
...
```yaml
Base: 30MB (Node.js + Electron AAOS core)
...
```bash
# Configuracion Electron AAOS
Electron AAOS_DEBUG=true                    # Habilitar logging de debug
Electron AAOS_CONFIG_PATH=/ruta/custom      # Ubicacion de config personalizada
Electron AAOS_YOLO_MODE=true               # Forzar modo yolo
...
| 1.1     | 2025-12-14 | Actualizado aviso de migracion a Electron AAOSAI/electron-aaos-core, semantic-release a v25.0.2 [Story 6.10] | Dex (dev)        |

---

_Este es un estandar oficial del framework Electron AAOS. Todas las elecciones de tecnologia deben alinearse con este stack._
