...
Este documento define los modos de ejecución para el Sistema de Verificación de Salud de Electron AAOS (HCS), basado en investigación de mejores prácticas de la industria de Kubernetes, VS Code, Terraform, npm/yarn, y patrones CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Al instalar/bootstrap**    | `npx electron-aaos install` | 60-120s  | Esperado                 | Validación de setup         | ✅ **Requerido**  |
...
jobs:
  health-check:
    ...
    steps:
      ...
      - run: npx electron-aaos health-check --mode=full --report
      - uses: actions/upload-artifact@v4
        with:
          name: health-report
          path: .electron-aaos/reports/health-check-*.md
```
...
**Veredicto:** Requerido. Parte de `npx electron-aaos install` y `*bootstrap-setup`.
...
name: Electron AAOS Health Check
  ...
  health-check:
    ...
    steps:
      ...
      - name: Run Health Check
        run: |
          npx electron-aaos health-check \
      ...
      - name: Post to Slack (on failure)
        ...
        with:
          payload: |
            {
              "text": "⚠️ Electron AAOS Health Check Falló",
              "blocks": [
                {
                  "type": "section",
                  "text": {
...
jobs:
  post-merge-health:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npx electron-aaos health-check --mode=quick
```
...
{
  "electron-aaos.healthCheck.enabled": true,
  "electron-aaos.healthCheck.interval": 300,
  "electron-aaos.healthCheck.mode": "quick",
  "electron-aaos.healthCheck.showNotifications": true
}
```
