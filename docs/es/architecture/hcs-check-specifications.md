...
**Propósito:** Verificar que los archivos del framework Electron AAOS están correctamente configurados y son consistentes.

### Verificaciones

| ID     | Nombre                      | Severidad | Nivel | Modo  | Timeout | Descripción                                         |
| ------ | --------------------------- | --------- | ----- | ----- | ------- | --------------------------------------------------- |
| PC-001 | Config existe               | CRITICAL  | 1     | quick | 100ms   | `.electron-aaos/config.yaml` existe y es YAML válido         |
...
| PC-006 | Sin archivos huérfanos      | LOW       | 3     | full  | 5s      | Todos archivos en `.electron-aaos-core/` están referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    ...
    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.electron-aaos/config.yaml no encontrado',
...
const requiredSections = [
  'Project Overview', // o 'Electron AAOS-FULLSTACK Development Rules'
...
Los usuarios pueden definir verificaciones específicas del proyecto en `.electron-aaos/custom-checks.yaml`:
