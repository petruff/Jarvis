# 🏛️ Electron AAOS Dashboard - Arquitectura Completa
...
El Electron AAOS Dashboard es una aplicación Next.js que proporciona una interfaz visual para monitorear y gestionar el sistema Electron AAOS. Se comunica con el CLI/Electron AAOS a través de archivos de estado en el sistema de archivos y Server-Sent Events (SSE).

### Principios Arquitectónicos

1. **CLI-First**: Dashboard es complementario al CLI, no sustituto
2. **File-Based Communication**: Estado a través de `.electron-aaos/dashboard/status.json`
...
│                              FILESYSTEM                                  │
│  ┌──────────────────────────┐  ┌──────────────────────────────────────┐ │
│  │ .electron-aaos/dashboard/         │  │ docs/stories/                        │ │
...
│   │   │   ├── status/route.ts       # Polling de estado Electron AAOS
...
│   │   ├── use-electron-aaos-status.ts        # Estado con SWR
...
| ---------------- | ----------------------------- | -------------------------------- |
| `story-store`    | `electron-aaos-stories`            | `storyOrder` (orden de las columnas) |
| `ui-store`       | `electron-aaos-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `electron-aaos-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `electron-aaos-dashboard-settings` | Todo el objeto `settings`         |
...
```typescript
// Retorna estado actual de Electron AAOS
// Lee de: .electron-aaos/dashboard/status.json
...
```
┌─────────────┐                              ┌──────────────────┐
│   CLI/Electron AAOS  │                              │    Dashboard     │
│   (Claude)  │                              │    (Next.js)     │
└──────┬──────┘                              └────────┬─────────┘
       │                                              │
       │  1. Agente activado                         │
       │  ─────────────────────────────────────────▶  │
       │     Escribe .electron-aaos/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
_Documentación generada por @architect (Aria) - Electron AAOS Core v2.0_
