...
El **Motor de Desarrollo Autónomo (ADE)** es la infraestructura de Electron AAOS para flujos de trabajo de desarrollo autónomo. Permite que los agentes de IA trabajen de forma independiente a través de pipelines inteligentes, bucles autorreparables y aprendizaje persistente.
...
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────────────────────────────────────────────────────────┐  │ │
│  │  │                     Estado en Tiempo de Ejecución .electron-aaos/        │  │ │
│  │  └─────────────────────────────────────────────────────────────────┘  │ │
│  │         │                │                │                │          │ │
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐   │ │
...
Todo el estado de ADE se persiste en `.electron-aaos/` para:
...
Archivo de Estado: .electron-aaos/project-status.yaml
```
...
| ----------------- | ------------------------------- | --------------------------------- |
| Patrones de Código | Patrones reutilizables de la base de código | .electron-aaos/patterns/code-patterns.json |
| Gotchas           | Trampas conocidas y soluciones  | .electron-aaos/patterns/gotchas.json       |
| Insights de Sesión | Descubrimientos durante sesiones | .electron-aaos/sessions/                   |
| Mapa de Base de Código | Análisis de estructura del proyecto | .electron-aaos/codebase-map.json           |
...
ADE se integra con agentes Electron AAOS a través de:
...
Todos los componentes de ADE actualizan `.electron-aaos/status.json`:
...
Ubicado en `.electron-aaos-core/core-config.yaml`:
...
ade:
  ...
  capaMemoria:
    habilitado: true
    almacenPatrones: .electron-aaos/patterns/
...
```
.electron-aaos-core/workflow-intelligence/
├── engine/
...
_Este es un estándar oficial del framework Electron AAOS que documenta el Motor de Desarrollo Autónomo._
