...
El Sistema de Saludo Contextual es una mejora de UX que hace que los saludos de los agentes Electron AAOS sean inteligentes y adaptativos, mostrando información y comandos relevantes basados en el contexto de la sesión.

## Lo Que Se Ha Implementado

### Componentes Core (Story 6.1.2.5)

1. **ContextDetector** (`.electron-aaos-core/core/session/context-detector.js`)
...
2. **GitConfigDetector** (`.electron-aaos-core/infrastructure/scripts/git-config-detector.js`)
   - Detecta configuración de git del proyecto
   - Caché con TTL de 5 minutos
   - Protección de timeout de 1000ms

3. **GreetingBuilder** (`.electron-aaos-core/development/scripts/greeting-builder.js`)
...
4. **WorkflowNavigator** (`.electron-aaos-core/development/scripts/workflow-navigator.js`)
...
5. **Workflow Patterns** (`.electron-aaos-core/data/workflow-patterns.yaml`)
...
```bash
node .electron-aaos-core/development/scripts/test-greeting-system.js
```
...
### Scripts Core
- `.electron-aaos-core/core/session/context-detector.js` - Detección de tipo de sesión
- `.electron-aaos-core/infrastructure/scripts/git-config-detector.js` - Detección de git config
- `.electron-aaos-core/development/scripts/greeting-builder.js` - Construcción del saludo
- `.electron-aaos-core/development/scripts/workflow-navigator.js` - Navegación de workflow
- `.electron-aaos-core/development/scripts/agent-exit-hooks.js` - Hooks de salida (para persistencia)

### Archivos de Datos
- `.electron-aaos-core/data/workflow-patterns.yaml` - Definiciones de workflows
...
### Configuración
- `.electron-aaos-core/core-config.yaml` - Configuración global (secciones git + agentIdentity)

### Agentes (Actualizados)
- `.electron-aaos-core/agents/dev.md` - Metadatos de visibilidad de comandos
- `.electron-aaos-core/agents/po.md` - Metadatos de visibilidad de comandos
- `.electron-aaos-core/agents/*.md` - 9 agentes restantes (actualización pendiente)
...
R: Usa el script de prueba: `node .electron-aaos-core/development/scripts/test-greeting-system.js`
