...
Sincroniza agentes, tasks, workflows y checklists de Electron AAOS a traves de multiples configuraciones de IDE.

## Vision General

La task `*command` automatiza la sincronizacion de componentes Electron AAOS a todos los directorios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operaciones de copia manuales.
...
Editar `.electron-aaos-sync.yaml` para habilitar tus IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Leer configuracion .electron-aaos-sync.yaml               │
...
```
tu-proyecto/
├── .electron-aaos-sync.yaml           # Configuracion de sincronizacion
...
**Solucion**: Agregar el alias a `.electron-aaos-sync.yaml`:
...
- [Arquitectura Electron AAOS](../core-architecture.md)
