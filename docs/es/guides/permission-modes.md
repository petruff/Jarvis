...
> Controla cuanta autonomia tienen los agentes Electron AAOS sobre tu sistema.

---

## Vision General

Los Modos de Permisos te permiten controlar el nivel de autonomia que tienen los agentes Electron AAOS. Ya sea que estes explorando un nuevo codebase o ejecutando builds completamente autonomos, hay un modo para tu flujo de trabajo.
...
El modo se persiste en `.electron-aaos/config.yaml`:
...
```yaml
# .github/workflows/electron-aaos.yml
- name: Run Electron AAOS
  run: |
    echo "permissions:\n  mode: auto" > .electron-aaos/config.yaml
    electron-aaos run build
```
...
Verificar que `.electron-aaos/config.yaml` existe y es escribible:

```bash
ls -la .electron-aaos/config.yaml
```
...
```javascript
const { PermissionMode, OperationGuard } = require('./.electron-aaos-core/core/permissions');
