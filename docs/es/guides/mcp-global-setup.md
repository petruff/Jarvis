...
# Guía de Configuración Global de MCP en Electron AAOS

> 🌐 [EN](../../guides/mcp-global-setup.md) | [PT](../../pt/guides/mcp-global-setup.md) | **ES**

---

> Configura servidores MCP (Model Context Protocol) globales para Electron AAOS Electron AAOS.
...
El Sistema Global de MCP te permite configurar servidores MCP una vez y compartirlos en todos los proyectos Electron AAOS. Esto elimina la necesidad de configurar los mismos servidores en cada proyecto.
...
- `~/.electron-aaos/` - Directorio global de Electron AAOS
- `~/.electron-aaos/mcp/` - Directorio de configuración MCP
- `~/.electron-aaos/mcp/global-config.json` - Archivo de configuración principal
- `~/.electron-aaos/mcp/servers/` - Configuraciones individuales de servidor
- `~/.electron-aaos/mcp/cache/` - Caché de respuestas
- `~/.electron-aaos/credentials/` - Almacenamiento seguro de credenciales
...
Electron AAOS incluye plantillas para servidores MCP populares:
...
Las credenciales se almacenan en `~/.electron-aaos/credentials/` con un `.gitignore` para prevenir commits accidentales.
...
| Directorio existe  | Usar `electron-aaos mcp setup --force` para recrear                               |
...
_Electron AAOS Electron AAOS v2.1 Guía de Configuración Global de MCP_
