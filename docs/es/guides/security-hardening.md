# Guía de Hardening de Seguridad de Electron AAOS

> [EN](../../guides/security-hardening.md) | [PT](../../pt/guides/security-hardening.md) | **ES**

---

> Guía completa para fortalecer la seguridad en despliegues de Electron AAOS Electron AAOS - desde desarrollo hasta producción.
...
Electron AAOS Electron AAOS opera en una capa privilegiada entre los modelos de IA y tu sistema. Esta guía cubre estrategias de hardening específicas para entornos de desarrollo orquestados por IA.
...
### Preocupaciones de Seguridad Específicas de Electron AAOS
...
Electron AAOS implementa múltiples capas de protección:
...
Las claves API son los secretos más críticos en Electron AAOS. Las claves comprometidas pueden llevar a uso no autorizado, brechas de datos e impacto financiero significativo.
...
const secrets = await SecretManager.loadSecrets({
  ...
  secretName: 'electron-aaos/production/api-keys',
...
```javascript
// .electron-aaos-core/core/security/key-validator.js
const requiredKeys = [
...
```bash
# ============================================================
# CONFIGURACIÓN DE ENTORNO Electron AAOS
# ============================================================
...
AUDIT_LOG_PATH=/var/log/electron-aaos/audit.log
...
mkdir -p ~/.electron-aaos/secrets
chmod 700 ~/.electron-aaos/secrets

# Crear archivo de secretos cifrado
# Nunca almacenar secretos en texto plano
openssl enc -aes-256-cbc -salt -pbkdf2 \
  -in secrets.txt \
  -out ~/.electron-aaos/secrets/encrypted.dat

# Establecer permisos apropiados
chmod 600 ~/.electron-aaos/secrets/*
...
function validateEnvironment() {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (process.env.Electron AAOS_DEBUG === 'true') {
...
### Permisos de Estructura de Directorios Electron AAOS
...
# Directorios de configuración Electron AAOS
chmod 700 .electron-aaos/              # Solo el propietario puede acceder
chmod 700 .electron-aaos-core/         # Fuente del framework
chmod 700 .claude/            # Configuración de Claude

# Archivos de configuración sensibles
chmod 600 .env                # Variables de entorno
chmod 600 .electron-aaos/config.yaml   # Configuración principal
chmod 600 .electron-aaos/users.json    # Base de datos de usuarios
chmod 600 .electron-aaos/sessions.json # Sesiones activas
...
# Archivos temporales
chmod 700 .electron-aaos/temp/
chmod 600 .electron-aaos/temp/*
```

### Control de Acceso a Directorios

```yaml
# .electron-aaos/config.yaml - Configuración de directorios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
echo "Verificación de Permisos de Seguridad Electron AAOS"
echo "==========================================="
...
check_permission ".electron-aaos" "700"
check_permission ".electron-aaos/config.yaml" "600"
...
Electron AAOS usa contenedores Docker para aislar los servidores MCP del sistema host:
...
const { OperationGuard } = require('./.electron-aaos-core/core/permissions');
...
```javascript
// .electron-aaos-core/core/security/input-sanitizer.js
...
PROTECTED_FILES = [
    '.claude/CLAUDE.md',
    '.claude/rules/*.md',
    '.electron-aaos-core/development/agents/*.md',
    'package.json',
    'tsconfig.json'
]
...
```yaml
# .electron-aaos/config.yaml - Configuración de auditoría
audit:
  enabled: true
  ...
  output:
    file:
      enabled: true
      path: .electron-aaos/logs/audit.log
      maxSize: 10M
      maxFiles: 10
      compress: true
    console:
  ...
  retention:
    days: 90
    archivePath: .electron-aaos/logs/archive
```
...
```javascript
// .electron-aaos-core/core/security/audit-logger.js
...
| ---------------------- | ----------------- | ---------------- |
| **Electron AAOS_DEBUG**         | `true`            | `false`          |
...
```yaml
# .electron-aaos/config.development.yaml
security:
  debug: true
...
```yaml
# .electron-aaos/config.production.yaml
security:
  debug: false
...
function validateProductionSecurity() {
  ...
  if (process.env.Electron AAOS_DEBUG === 'true') {
    errors.push('Electron AAOS_DEBUG debe ser false en producción');
  }
...
Si descubres una vulnerabilidad de seguridad en Electron AAOS Electron AAOS, por favor sigue las prácticas de divulgación responsable:

### Proceso de Reporte

1. **NO** crees un issue público en GitHub para vulnerabilidades de seguridad
2. Envía las preocupaciones de seguridad por email a: **security@electron-aaos.ai**
...
**Versión de Electron AAOS:** [ej., 2.1.0]
...
Actualmente, Electron AAOS Electron AAOS no tiene un programa formal de bug bounty. Sin embargo, las contribuciones significativas de seguridad son reconocidas y pueden recibir licencias de Electron AAOS Pro u otro reconocimiento.
...
_Guía de Hardening de Seguridad de Electron AAOS Electron AAOS v2.1.0_
