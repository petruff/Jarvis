...
# Guía del Sistema de Quality Gates de Electron AAOS

> 🌐 [EN](../../guides/quality-gates.md) | [PT](../../pt/guides/quality-gates.md) | **ES**

---

> Guía completa del sistema de quality gates de 3 capas para Electron AAOS Electron AAOS.
...
El Sistema de Quality Gates de Electron AAOS proporciona aseguramiento de calidad automatizado a través de tres capas progresivas de validación. Cada capa detecta diferentes tipos de problemas en la etapa apropiada del desarrollo.
...
```yaml
# .electron-aaos-core/core/quality-gates/quality-gate-config.yaml
layer1:
  enabled: true
...
electron-aaos qa run --layer=1

# Ejecutar verificación específica
electron-aaos qa run --layer=1 --check=lint
electron-aaos qa run --layer=1 --check=test
electron-aaos qa run --layer=1 --check=typecheck

# Ejecutar con salida detallada
electron-aaos qa run --layer=1 --verbose
```
...
layer2:
  ...
  quinn:
    enabled: true
    autoReview: true
    agentPath: '.claude/commands/Electron AAOS/agents/qa.md'
    severity:
      block: ['CRITICAL']
      warn: ['HIGH', 'MEDIUM']
```
...
electron-aaos qa run --layer=2

# Ejecutar solo CodeRabbit
electron-aaos qa run --layer=2 --tool=coderabbit

# Ejecutar revisión de Quinn (@qa)
electron-aaos qa run --layer=2 --tool=quinn
```
...
const QualityGateManager = require('./.electron-aaos-core/core/quality-gates/quality-gate-manager');
...
electron-aaos qa request-review --pr=123

# Aprobar la revisión
electron-aaos qa signoff --pr=123 --reviewer="@architect"

# Verificar estado de aprobación
electron-aaos qa signoff-status --pr=123
```
...
electron-aaos qa run --layer=1
electron-aaos qa run --layer=2
electron-aaos qa run --layer=3

# Ejecutar con opciones
electron-aaos qa run --verbose          # Salida detallada
electron-aaos qa run --fail-fast        # Detener en primera falla
electron-aaos qa run --continue-on-fail # Continuar a pesar de fallas
```
...
electron-aaos qa status --layer=1

# Obtener estado para PR
electron-aaos qa status --pr=123
```
...
# Exportar a archivo
electron-aaos qa report --output=qa-report.json
electron-aaos qa report --format=markdown --output=qa-report.md
```

### `electron-aaos qa configure`
...
electron-aaos qa configure --layer1.coverage.minimum=90
electron-aaos qa configure --layer2.coderabbit.enabled=false
electron-aaos qa configure --layer3.requireSignoff=true
```
...
jobs:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '18'
      - run: npm ci
      - run: electron-aaos qa run --layer=1
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '18'
      - run: npm ci
      - run: electron-aaos qa run --layer=2
        env:
  ...
  layer3:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: electron-aaos qa request-review --pr=${{ github.event.pull_request.number }}
```
...
layer3:
  stage: layer3
  script:
    - electron-aaos qa request-review
  needs:
    - layer2
  when: manual
```
...
electron-aaos qa run --layer=1 --fail-fast
```
...
# Reportes
reports:
  location: '.electron-aaos/qa-reports'
  format: 'json'
  retention: 30
  includeMetrics: true
...
status:
  location: '.electron-aaos/qa-status.json'
  updateOnChange: true
...
_Guía del Sistema de Quality Gates de Electron AAOS Electron AAOS v2.1_
