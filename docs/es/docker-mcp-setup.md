...
Guía para configurar servidores MCP (Model Context Protocol) basados en Docker con Electron AAOS.
...
- Proyecto **Electron AAOS** inicializado
...
- `~/.electron-aaos/mcp/servers/` - Configuraciones de servidores individuales
...
Electron AAOS usa Docker MCP Toolkit como infraestructura MCP principal:
...
electron-aaos mcp add <server-name>

# Agregar con configuración personalizada
electron-aaos mcp add custom-server --config='{"command":"npx","args":["-y","package"]}'

# Eliminar servidor
electron-aaos mcp remove <server-name>

# Habilitar/deshabilitar servidores
electron-aaos mcp enable <server-name>
electron-aaos mcp disable <server-name>
```
...
- [Agente DevOps](../.electron-aaos-core/development/agents/devops.md)

---

_Guía de Configuración de Docker MCP de Electron AAOS Electron AAOS v2.1.0_
