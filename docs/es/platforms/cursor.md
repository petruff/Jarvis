...
# Guía de Electron AAOS para Cursor
...
### ¿Por qué usar Electron AAOS con Cursor?

Cursor es uno de los **IDEs de IA más populares** con excelente integración de Electron AAOS:
...
Verificar que se crearon los archivos de Electron AAOS:
...
- Integración del marco Electron AAOS
...
### Iniciando Cursor con Electron AAOS
...
| `@docs`       | Documentación                  |
| `@web`        | Búsqueda web                   |
| `@agent-name` | Agente Electron AAOS                    |
...
# Reinicializar Electron AAOS
npx @anthropic/electron-aaos init --ide cursor --force
```
...
R: Ejecutar `npm run sync:agents` después de actualizaciones de Electron AAOS.
...
2. Instalar Electron AAOS:
   ```bash
   npx @anthropic/electron-aaos init --ide cursor
   ```
...
2. Inicializar Electron AAOS para Claude Code:
...
2. Inicializar Electron AAOS para Windsurf:

   ```bash
   npx @anthropic/electron-aaos init --ide windsurf
   ```
...
- [Guía de Integración de Electron AAOS](../README.md)

---

_Electron AAOS Electron AAOS - Guía de Plataforma Cursor v1.0_
