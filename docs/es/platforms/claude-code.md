...
# Guía Electron AAOS para Claude Code

> 🌐 [EN](../../platforms/claude-code.md) | [PT](../../pt/platforms/claude-code.md) | **ES**

---

> **CLI Oficial de Anthropic** - Plataforma Recomendada para Electron AAOS
...
### ¿Por qué usar Electron AAOS con Claude Code?

Claude Code ofrece la **mejor integración** con Electron AAOS debido a:
...
### Paso 3: Instalar Electron AAOS

```bash
# Navega a tu proyecto
cd tu-proyecto

# Inicializa Electron AAOS
...
# Verifica que los archivos Electron AAOS fueron creados
ls -la .claude/
```
...
- Comprensión del marco Electron AAOS
...
export Electron AAOS_DEBUG="true"                        # Habilitar registro de depuración
```
...
### Activando Agentes Electron AAOS
...
```mermaid
flowchart LR
    Source["Electron AAOS Core<br/>.electron-aaos-core/development/agents"] --> Parser["Analizador de Agentes"]
    Parser --> Transform["Transformador Claude"]
    Transform --> Output[".claude/commands/Electron AAOS/agents"]
```
...
1. Los cambios locales se respaldan en `.claude/commands/Electron AAOS/agents/.backup/`
...
# Verificar registros de Electron AAOS
cat .electron-aaos-core/logs/sync.log
```
...
### Preguntas Específicas de Electron AAOS
...
R: En `.claude/commands/Electron AAOS/agents/` después de la sincronización.

**P: ¿Con qué frecuencia debo sincronizar agentes?**
R: Después de actualizar Electron AAOS o cuando los agentes se modifiquen en el núcleo.
...
2. Inicializa Electron AAOS con Claude Code:
   ```bash
   npx @anthropic/electron-aaos init --ide claude-code
   ```
...
1. Tus agentes Electron AAOS se almacenan en `.electron-aaos-core/development/agents/`
2. Inicializa Electron AAOS para el IDE objetivo
...
*Electron AAOS Electron AAOS - Guía de Plataforma Claude Code v1.0*
