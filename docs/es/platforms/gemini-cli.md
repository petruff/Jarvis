...
# Guía de Electron AAOS para Gemini CLI
...
### ¿Por qué usar Electron AAOS con Gemini CLI?
...
```markdown
# Reglas de Electron AAOS Electron AAOS para Gemini CLI
...
## Activación
Incluye "Como el agente Electron AAOS dev" en tu prompt.
...
### Activando Agentes de Electron AAOS

Incluye contexto de agentes en los prompts:

```bash
# Agente de desarrollador
gemini "Como el agente Electron AAOS dev, implementa la característica de login"

# Agente de QA
gemini "Como el agente Electron AAOS qa, revisa este código por seguridad"

# Agente de arquitecto
gemini "Como el agente Electron AAOS architect, diseña el esquema de base de datos"
```
...
cat src/auth.ts | gemini "Como Electron AAOS qa, revisa este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como Electron AAOS qa, revisa este archivo" >> reviews.md
done
```
...
**Frase de Activación:** "Como el agente Electron AAOS dev"

## Rol
Eres un Desarrollador Full Stack Senior trabajando con Electron AAOS Electron AAOS.
...
1. Instalar Electron AAOS para Gemini CLI:
   ```bash
   npx @anthropic/electron-aaos init --ide gemini-cli
   ```
2. Los agentes se transforman a formato basado en prompts

### De Gemini CLI a Otros IDEs

1. Los agentes de Electron AAOS en `.electron-aaos-core/` son portátiles
...
*Electron AAOS Electron AAOS - Guía de Plataforma Gemini CLI v1.0*
