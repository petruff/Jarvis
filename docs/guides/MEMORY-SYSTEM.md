...
4. [Camada 2: Electron AAOS Framework](#camada-2-electron-aaos-framework)
...
O sistema de memoria do Electron AAOS opera em **duas camadas independentes** que coexistem mas **nao se comunicam entre si**:

| Camada | Gerenciado Por | Escopo |
|--------|---------------|--------|
| **Camada 1: Claude Code Nativo** | Claude Code CLI | Auto Memory, CLAUDE.md, Session Transcripts |
| **Camada 2: Electron AAOS Framework** | Scripts JS em `.electron-aaos-core/` | Gotchas, Session State, Context Snapshots, Timeline |
...
- **Cada camada persiste de forma independente** — `.claude/` vs `.electron-aaos/`
...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        ...
        CLAUDE_MD_P["CLAUDE.md Projeto<br/>electron-aaos-core/.claude/CLAUDE.md"]
    ...
    subgraph Electron AAOS_FRAMEWORK["Camada 2: Electron AAOS Framework"]
        direction TB
        GOTCHAS["Gotchas Memory<br/>.electron-aaos-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["Context Snapshot<br/>.electron-aaos-core/core/memory/context-snapshot.js"]
        FILE_EVO["File Evolution Tracker<br/>.electron-aaos-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.electron-aaos-core/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader<br/>.electron-aaos-core/core/session/context-loader.js"]
        CTX_DETECT["Context Detector<br/>.electron-aaos-core/core/session/context-detector.js"]
        SESS_STATE["Session State<br/>.electron-aaos-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.electron-aaos-core/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks<br/>.electron-aaos-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_Electron AAOS["Storage Electron AAOS (.electron-aaos/)"]
        S_GOTCHAS[".electron-aaos/gotchas.json + .md"]
        S_ERRORS[".electron-aaos/error-tracking.json"]
        S_SNAPSHOTS[".electron-aaos/snapshots/"]
        S_TIMELINE[".electron-aaos/timeline/"]
        S_FILE_EVO[".electron-aaos/file-evolution/"]
        S_SESSION[".electron-aaos/session-state.json"]
    ...
    style Electron AAOS_FRAMEWORK fill:#FFF3E0,stroke:#E65100
    style STORAGE_CLAUDE fill:#E8F5E9,stroke:#2E7D32
    style STORAGE_Electron AAOS fill:#FCE4EC,stroke:#C62828
```
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["Memory Layer (.electron-aaos-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["Session Layer (.electron-aaos-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["Orchestration (.electron-aaos-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["Activation Pipeline (.electron-aaos-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks System (.electron-aaos-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        ...
        LOAD_PROJECT["3. electron-aaos-core/.claude/CLAUDE.md<br/>Instrucoes nivel projeto"]
        LOAD_RULES["4. electron-aaos-core/.claude/rules/*.md<br/>5 arquivos de regras detalhadas"]
...
flowchart LR
    subgraph AGENT_MEM[".claude/agent-memory/"]
        AM1["electron-aaos-architect/MEMORY.md"]
        AM2["electron-aaos-dev/MEMORY.md"]
    ...
    NOTE["NOTA: Apenas agentes<br/>definidos em .claude/agents/<br/>usam esta memoria.<br/>Agentes Electron AAOS (.electron-aaos-core/<br/>development/agents/)<br/>NAO usam."]
...
## Camada 2: Electron AAOS Framework
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em:<br/>.electron-aaos/gotchas.json<br/>.electron-aaos/gotchas.md<br/>.electron-aaos/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em:<br/>.electron-aaos/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em:<br/>.electron-aaos/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em:<br/>.electron-aaos/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD4["Le .electron-aaos/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("Persiste em:<br/>.electron-aaos/session-state.json<br/>TTL: 1 hora")]
...
sequenceDiagram
    ...
    Note over CC: NENHUMA memoria Electron AAOS<br/>e carregada automaticamente<br/>no system prompt.<br/>Apenas contexto de sessao<br/>aparece no greeting.
```
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".electron-aaos/session-state.json<br/>Agente atual, comandos, workflow"]
        S2[".electron-aaos/error-tracking.json<br/>Ocorrencia do erro"]
        S3[".electron-aaos/gotchas.json<br/>Nova gotcha auto-capturada"]
        S4[".electron-aaos/gotchas.json<br/>Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml<br/>Phase atualizada"]
        S6["docs/stories/.session-state.yaml<br/>Story marcada done"]
        S7[".electron-aaos/snapshots/{id}.json<br/>Contexto capturado"]
        S8[".electron-aaos/file-evolution/<br/>Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    CL->>FS: Le .electron-aaos/session-state.json
    ...
    CL->>FS: Escreve .electron-aaos/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        TRACK --> READ_ET["Le .electron-aaos/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["Salva .electron-aaos/gotchas.json"]
        SAVE_JSON --> SAVE_MD["Gera .electron-aaos/gotchas.md"]
...
classDiagram
    class GotchasStore {
        +string schema = "electron-aaos-gotchas-memory-v1"
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker<br/>.electron-aaos/file-evolution/"]
        SRC2["ContextSnapshot<br/>.electron-aaos/snapshots/"]
    ...
    subgraph STORAGE["Storage"]
        ST1[(".electron-aaos/timeline/<br/>unified-timeline.json")]
    end
...
```mermaid
flowchart LR
    subgraph Electron AAOS_EVENTS["Eventos Electron AAOS"]
        E1["sessionStart"]
        E2["beforeAgent"]
        E3["beforeTool"]
        E4["afterTool"]
...
|---------|--------|------------|--------|
| `.electron-aaos-core/core/memory/gotchas-memory.js` | Memory | Epic 9, Story 9.4 | Auto-capture de erros repetidos, gotchas manuais, injecao em tasks |
| `.electron-aaos-core/core/memory/context-snapshot.js` | Memory | Story 12.6 | Captura e restaura contexto de desenvolvimento |
| `.electron-aaos-core/core/memory/file-evolution-tracker.js` | Memory | Gap impl | Rastreia evolucao de arquivos, detecta drift |
| `.electron-aaos-core/core/memory/timeline-manager.js` | Memory | Gap impl | Facade unificada para timeline cross-session |
| `.electron-aaos-core/core/session/context-loader.js` | Session | Story 2.2, 6.1.2.5 | Continuidade inter-agente, handoff de contexto |
| `.electron-aaos-core/core/session/context-detector.js` | Session | Story 2.2 | Deteccao hibrida de tipo de sessao (new/existing/workflow) |
| `.electron-aaos-core/core/orchestration/session-state.js` | Orchestration | Story 11.5 | Estado persistente de epic/story, crash recovery |
| `.electron-aaos-core/core/orchestration/context-manager.js` | Orchestration | Legacy | Estado de workflow entre fases (migrado para session-state) |
| `.electron-aaos-core/core/elicitation/session-manager.js` | Elicitation | — | Sessions de elicitacao save/load |

### Scripts de Ativacao (consomem memoria)

| Arquivo | Funcao |
|---------|--------|
| `.electron-aaos-core/development/scripts/unified-activation-pipeline.js` | Orchestrador principal — carrega sessao no Tier 3 |
| `.electron-aaos-core/development/scripts/greeting-builder.js` | Monta greeting com contexto de sessao/memoria |

### Hooks (persistencia de sessao)

| Arquivo | CLI | Funcao |
|---------|-----|--------|
| `.electron-aaos-core/hooks/gemini/session-start.js` | Gemini | Carrega contexto Electron AAOS no inicio da sessao |
| `.electron-aaos-core/hooks/gemini/session-end.js` | Gemini | Persiste sumario da sessao em `.electron-aaos/sessions/` |
| `.electron-aaos-core/hooks/gemini/before-agent.js` | Gemini | Pre-processamento antes de agente |
| `.electron-aaos-core/hooks/gemini/before-tool.js` | Ambos | Pre-processamento antes de tool |
| `.electron-aaos-core/hooks/gemini/after-tool.js` | Ambos | Pos-processamento apos tool |
| `.electron-aaos-core/hooks/unified/hook-interface.js` | Ambos | Classe base UnifiedHook + EVENT_MAPPING |
| `.electron-aaos-core/hooks/unified/hook-registry.js` | Ambos | Registro central de hooks |
| `.electron-aaos-core/hooks/unified/index.js` | Ambos | Entry point do sistema unificado |
...
| `electron-aaos-core/.claude/CLAUDE.md` | Instrucoes projeto (carregado sempre) |
| `electron-aaos-core/.claude/rules/*.md` | 5 arquivos de regras (carregados sempre) |
...
```
.electron-aaos/                                        # Runtime state (gitignored)
...
├── electron-aaos-architect/MEMORY.md
├── electron-aaos-dev/MEMORY.md
...
Os modulos Electron AAOS (gotchas, snapshots, timeline) **nao fazem flush** automatico ao final da sessao. Dados em memoria podem se perder se o processo terminar abruptamente.
...
Claude auto-memory (`MEMORY.md`) e Electron AAOS memory (`.electron-aaos/`) nunca se sincronizam. Gotchas capturadas pelo Electron AAOS nao aparecem no MEMORY.md e vice-versa.
...
| Gotchas Memory Script | `.electron-aaos-core/core/memory/gotchas-memory.js` |
| Context Snapshot Script | `.electron-aaos-core/core/memory/context-snapshot.js` |
| File Evolution Tracker | `.electron-aaos-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.electron-aaos-core/core/memory/timeline-manager.js` |
| Session Context Loader | `.electron-aaos-core/core/session/context-loader.js` |
| Context Detector | `.electron-aaos-core/core/session/context-detector.js` |
| Session State | `.electron-aaos-core/core/orchestration/session-state.js` |
| Unified Hook Interface | `.electron-aaos-core/hooks/unified/hook-interface.js` |
| Hook Registry | `.electron-aaos-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.electron-aaos-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.electron-aaos-core/hooks/gemini/session-end.js` |
| Core Config | `.electron-aaos-core/core-config.yaml` |
| Claude Settings | `~/.claude/settings.json` |
| Story Development Cycle | `docs/guides/workflows/STORY-DEVELOPMENT-CYCLE-WORKFLOW.md` |

---

*Electron AAOS Memory System Architecture Guide v1.0*
