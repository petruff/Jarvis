...
> **ARQUITETURA:** O MIS segue o modelo **Open Core** do Electron AAOS. Toda a inteligencia de memoria reside no repositorio privado `electron-aaos-pro` (submodule `pro/`). O `electron-aaos-core` (open source) fornece apenas extension points e funciona 100% sem memoria inteligente — exatamente como hoje.
...
O sistema de memoria atual opera em **duas camadas desconectadas** (Claude Code nativo + Electron AAOS Framework) com gaps criticos:
...
|--------|--------|-------------|-------|
| **Cleanup** | Remover dead code, preparar base | **electron-aaos-core** | MIS-2 |
| **Capture** | Captura conhecimento de sessao via hooks | **electron-aaos-pro** | MIS-3 |
| **Storage** | Armazena em Markdown com frontmatter YAML | **electron-aaos-pro** | MIS-3, MIS-4 |
| **Retrieval** | Recupera por relevancia com progressive disclosure | **electron-aaos-pro** + extension points em **electron-aaos-core** | MIS-4, MIS-6 |
| **Evolution** | Aprende e evolui regras automaticamente | **electron-aaos-pro** | MIS-5, MIS-7 |

### Principios Chave (vs Estado Atual)

| Aspecto | Estado Atual | Com MIS (electron-aaos-pro ativo) | Sem MIS (electron-aaos-core only) |
|---------|-------------|--------------------------|--------------------------|
...
> **electron-aaos-core funciona 100% sem o MIS** — exatamente como funciona hoje. Quando `electron-aaos-pro` esta presente (submodule `pro/`), o MIS se conecta automaticamente via extension points. Zero configuracao manual.

### Modelo Open Core

```mermaid
flowchart LR
    subgraph CORE["electron-aaos-core (Open Source, MIT)"]
        direction TB
    ...
    subgraph PRO["electron-aaos-pro (Privado, Submodule pro/)"]
        direction TB
...
O electron-aaos-core ja possui o modulo `bin/utils/pro-detector.js` (Story PRO-5):
...
#### electron-aaos-core (open source) — Extension Points Only

| Arquivo | Tipo | Descricao |
|---------|------|-----------|
| `bin/utils/pro-detector.js` | Existente | `isProAvailable()`, `loadProModule()` |
| `.electron-aaos-core/development/scripts/unified-activation-pipeline.js` | Modificacao | Adiciona `if (isProAvailable())` no Tier 2 para memory loading |
| `.electron-aaos-core/development/scripts/greeting-builder.js` | Modificacao | Adiciona memory stats no greeting se pro disponivel |
| `.electron-aaos-core/core/memory/gotchas-memory.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.electron-aaos-core/core/session/context-loader.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.claude/settings.json` | Modificacao | Hook stubs que chamam scripts em `pro/` (graceful fail se ausente) |

#### electron-aaos-pro (privado) — Toda a Inteligencia
...
Os hooks do Claude Code sao configurados em `.claude/settings.json` (electron-aaos-core). O script apontado reside em `pro/`:
...
```json
{
  "hooks": {
    "PreCompact": [
      {
        "type": "command",
        "command": "node .electron-aaos-core/hooks/pro-hook-runner.js PreCompact",
        "timeout": 10000
      }
    ]
  }
...
```javascript
// .electron-aaos-core/hooks/pro-hook-runner.js (no core, ~20 linhas)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
### 8. Open Core (Electron AAOS PRO pattern)
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["electron-aaos-core (Open Source)"]
        direction TB
        EXT["Extension Points<br/>in Pipeline"]
        HOOK_RUNNER[".electron-aaos-core/hooks/<br/>pro-hook-runner.js"]
        GOTCHAS["gotchas-memory.js<br/>(standalone)"]
        CTX_LOADER["context-loader.js<br/>(standalone)"]
    end

    subgraph PRO_BOUNDARY["electron-aaos-pro (Privado — pro/)"]
        ...
        subgraph STORAGE["CAMADA 2: STORAGE"]
            ST1[".electron-aaos/memories/shared/"]
            ST2[".electron-aaos/memories/{agent}/"]
            ST3[".electron-aaos/memories/index.json"]
        end
...
### Fluxo Sem Pro (electron-aaos-core standalone)
...
```mermaid
flowchart LR
    subgraph CORE["electron-aaos-core"]
        HR[".electron-aaos-core/hooks/<br/>pro-hook-runner.js"]
        UAP["unified-activation-<br/>pipeline.js"]
        GM["gotchas-memory.js"]
        PD["bin/utils/<br/>pro-detector.js"]
    end

    subgraph PRO["electron-aaos-pro (pro/)"]
...
> **Repositorio:** `electron-aaos-pro` (pro/memory/)
> **Extension point no core:** `.electron-aaos-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (electron-aaos-core — committed)
{
  "hooks": {
    "PreCompact": [
      {
    ...
    "Stop": [
      {
        "type": "command",
        "command": "node .electron-aaos-core/hooks/pro-hook-runner.js Stop",
        "timeout": 5000
      }
    ],
    "PostToolUseFailure": [
      {
        "type": "command",
        "command": "node .electron-aaos-core/hooks/pro-hook-runner.js PostToolUseFailure",
        "timeout": 3000
      }
    ]
  }
...
```mermaid
flowchart TD
    subgraph CORE_HOOKS["electron-aaos-core"]
        HR["pro-hook-runner.js"]
    end

    subgraph PRO_CAPTURE["electron-aaos-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("Persist to<br/>.electron-aaos/memories/")]
    end
...
> **Repositorio:** `electron-aaos-pro` (pro/memory/) gerencia o storage
> **Storage location:** `.electron-aaos/memories/` (gitignored, criado pelo pro)
...
```
.electron-aaos/memories/                          # Criado e gerenciado por pro/memory/
...
> **Repositorio:** `electron-aaos-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **Repositorio:** `electron-aaos-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
sequenceDiagram
    ...
    participant HR as pro-hook-runner.js<br/>(electron-aaos-core)
    participant PD as pro-detector.js<br/>(electron-aaos-core)
    participant SD as session-digest.js<br/>(electron-aaos-pro)
    participant FS as .electron-aaos/memories/
...
> **Repositorio:** `electron-aaos-pro` (pro/memory/agent-memory-api.js)
...
Cada agente acessa:
1. Suas proprias memorias privadas (`.electron-aaos/memories/{agent}/`)
2. Memorias compartilhadas (`.electron-aaos/memories/shared/`)
...
```mermaid
sequenceDiagram
    participant UAP as UnifiedActivationPipeline<br/>(electron-aaos-core)
    participant PD as pro-detector.js<br/>(electron-aaos-core)
    participant ML as memory-loader.js<br/>(electron-aaos-pro)
    participant MR as memory-retriever.js<br/>(electron-aaos-pro)
    participant GB as GreetingBuilder<br/>(electron-aaos-core)
...
### Especificacao (arquivos em .electron-aaos/memories/)
...
# Always Use Absolute Imports in Electron AAOS
...
> **100% em electron-aaos-pro** — nenhum componente de self-learning no core
...
→ PROPOSTA: Adicionar a rules/: "Warning: .electron-aaos/ permissions"
...
**Sem pro:** Apenas `[CORE]` e `[NATIVO]` existem. `[PRO]` (.electron-aaos/memories/) nunca e criado.

---

## Mapa de Arquivos do Sistema

### electron-aaos-core (Open Source) — Extension Points

| Arquivo | Tipo | Story | Descricao |
|---------|------|-------|-----------|
| `bin/utils/pro-detector.js` | Existente (PRO-5) | — | `isProAvailable()`, `loadProModule()` |
| `.electron-aaos-core/hooks/pro-hook-runner.js` | **Novo** | MIS-3 | Wrapper: verifica pro, delega hook (~20 linhas) |
| `.electron-aaos-core/development/scripts/unified-activation-pipeline.js` | **Modificacao** | MIS-6 | Adiciona `if (isProAvailable())` no Tier 2 |
| `.electron-aaos-core/development/scripts/greeting-builder.js` | **Modificacao** | MIS-6 | Memory stats no greeting se pro disponivel |
| `.electron-aaos-core/core/memory/gotchas-memory.js` | **Path fix** | MIS-2 | Corrige `.electron-aaos/error-tracking.json` |
| `.electron-aaos-core/core/session/context-loader.js` | **Path fix** | MIS-2 | Corrige `.electron-aaos/session-state.json` |
...
### Arquivos Removidos (MIS-2, electron-aaos-core)

| Arquivo | Linhas | Razao |
|---------|--------|-------|
| `.electron-aaos-core/core/memory/timeline-manager.js` | 746 | 0 consumers (orphan) |
| `.electron-aaos-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 consumers (orphan) |
| `.electron-aaos-core/core/memory/context-snapshot.js` | 648 | 0 consumers (orphan) |
| `.electron-aaos-core/elicitation/session-manager.js` | — | Dead code |
| `.electron-aaos/compound-analysis/` | ~14K | 0 readers (orphan data) |

---

## Diferencas: Estado Atual vs MIS

| Aspecto | electron-aaos-core hoje | electron-aaos-core + electron-aaos-pro (MIS) | electron-aaos-core sem pro |
|---------|---------------|---------------------------|------------------|
...
|-------|--------|-------------|------------|-------|
| MIS-1 | Investigation & Architecture Design | docs (electron-aaos-core) | — | 12h (Done) |
| MIS-2 | Dead Code Cleanup & Path Repair | **electron-aaos-core** | MIS-1 | 4h |
| MIS-3 | Session Digest (PreCompact Hook) | **electron-aaos-core** (hook runner) + **electron-aaos-pro** (digest) | MIS-1 | 14h |
| MIS-4 | Progressive Memory Retrieval | **electron-aaos-pro** | MIS-3 | 16h |
| MIS-5 | Self-Learning Engine | **electron-aaos-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | Pipeline Integration & Agent Memory API | **electron-aaos-core** (ext points) + **electron-aaos-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md & Rules Auto-Evolution | **electron-aaos-pro** | MIS-5 | 8h |
| **Total** | | | | **~78h** |
...
| Epic PRO Architecture | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-electron-aaos-pro-architecture/EPIC-PRO-INDEX.md) |
...
*Electron AAOS Memory Intelligence System — Architecture Vision v2.1 (Core/Pro Split)*
*Target state apos Epic MIS completo (7 stories, ~78 horas)*
*electron-aaos-core: extension points + dead code cleanup*
*electron-aaos-pro: toda a inteligencia de memoria (Capture, Storage, Retrieval, Evolution)*
