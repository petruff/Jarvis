...
The integration follows the Electron AAOS Open Core model:
- **electron-aaos-core:** Extension point in UnifiedActivationPipeline (this guide)
- **electron-aaos-pro:** Memory intelligence implementation (retrieval, scoring, learning)
...
```yaml
# .electron-aaos-core/development/agents/dev.md
agent:
  id: dev
  config:
...
```javascript
isProAvailable() === true
// but .electron-aaos/session-digests/ is empty
```
...
```javascript
isProAvailable() === true
// and .electron-aaos/session-digests/ contains memory digests
```
...
const fs = require('fs');
const digests = fs.readdirSync('.electron-aaos/session-digests');
...
**Solutions:**
1. Reduce digest count (archive old digests to `.electron-aaos/session-digests/archive/`)
