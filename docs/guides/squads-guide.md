...
Complete guide for creating, validating, publishing, and managing Squads in Electron AAOS.
...
Squads are modular teams of AI agents that extend Electron AAOS functionality. Each squad is a self-contained package containing:
...
│  Level 2: Electron AAOS-SQUADS  → github.com/Electron AAOSAI (Public/Free)  │
│  Level 3: SYNKRA API   → api.electron-aaos.dev      (Marketplace)  │
└─────────────────────────────────────────────────────────────┘
```
...
|-------|---------|-------------|
| [etl-squad](https://github.com/Electron AAOSAI/electron-aaos-squads/tree/main/etl) | 2.0.0 | Data collection and transformation |
| [creator-squad](https://github.com/Electron AAOSAI/electron-aaos-squads/tree/main/creator) | 1.0.0 | Content generation utilities |
...
- Node.js 18+
- Electron AAOS project initialized (`.electron-aaos-core/` exists)
...
Tasks must follow [TASK-FORMAT-SPECIFICATION-V1](../../.electron-aaos-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Mode | Behavior |
|------|----------|
| `extend` | Add squad rules to core Electron AAOS rules |
...
### Level 2: electron-aaos-squads Repository (Public)
...
This creates a PR to [Electron AAOSAI/electron-aaos-squads](https://github.com/Electron AAOSAI/electron-aaos-squads).

### Level 3: Electron AAOS Marketplace
...
- `electron-aaos.type` field
- `electron-aaos.minVersion` field
...
3. **Add Fields** - `electron-aaos.type`, `electron-aaos.minVersion`
...
# - electron-aaos.type: must be "squad"
# - electron-aaos.minVersion: must be valid semver
```
...
**Squads** are the new standard (Electron AAOS 2.1+) replacing Expansion Packs. Squads have:
- Task-first architecture
...
3. Re-publish: `*publish-squad` or `*sync-squad-electron-aaos`
...
- **Level 3**: Sync with `--private` flag: `*sync-squad-electron-aaos my-squad --private`

### What's the minimum Electron AAOS version for Squads?

Squads require Electron AAOS 2.1.0+. Set in manifest:
...
- [@squad-creator Agent](../../.electron-aaos-core/development/agents/squad-creator.md)
- [electron-aaos-squads Repository](https://github.com/Electron AAOSAI/electron-aaos-squads)
