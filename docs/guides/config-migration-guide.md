...
Electron AAOS v3.12+ introduces a 4-level configuration hierarchy that replaces the single `core-config.yaml` file:
...
| **Pro** Extension | `pro/pro-config.yaml` | Submodule | Premium features (electron-aaos-pro) |
| **L3** App | `apps/<name>/electron-aaos-app.config.yaml` | Committed | Per-app overrides |
...
If `${...}` patterns are found in L1 or L2, `electron-aaos config validate` will warn — these files are committed to git and should not contain environment-specific values.
...
To suppress deprecation warnings: `Electron AAOS_SUPPRESS_DEPRECATION=1`
...
electron-aaos config show --level L2
```
...
electron-aaos config validate --level L4
```
...
| Command | Description |
|---------|-------------|
| `electron-aaos config show` | Show fully resolved configuration |
| `electron-aaos config show --level L2` | Show single level (raw, no merge) |
| `electron-aaos config show --debug` | Show with source annotations per value |
| `electron-aaos config diff --levels L1,L2` | Compare two levels |
| `electron-aaos config migrate` | Migrate monolithic to layered |
| `electron-aaos config migrate --dry-run` | Preview migration without changes |
| `electron-aaos config validate` | Validate YAML syntax and patterns |
| `electron-aaos config init-local` | Create local-config.yaml from template |
...
**Q: How does electron-aaos-pro config work?**
