...
**Scope:** `.electron-aaos-core/core/` -- 130 JavaScript files across 14 subsystems
...
| File | Purpose | Internal Deps | External Consumers | Orphan? |
|------|---------|---------------|-------------------|---------|
| `elicitation-engine.js` | Core interactive elicitation engine (inquirer-based) | session-manager.js; optional: infrastructure/scripts/security-checker | core/index.js, .electron-aaos-core/index.js, tests/core/, tests/security/ | No |
...
| `checks/project/` | 8 checks: package-json, node-version, dependencies, electron-aaos-directory, agent-config, framework-config, task-definitions, workflow-dependencies | project/index.js | base-check.js |
...
| `global-config-manager.js` | Manages ~/.electron-aaos/mcp/ global config | os-detector.js | config-migrator.js, CLI mcp/add, mcp/status, mcp/setup, mcp/link, tests/ | No |
...
| `skill-dispatcher.js` | Maps agent IDs to Electron AAOS Skill invocations | None | workflow-orchestrator.js, orchestration/index.js | No |
...
| `build-registry.js` | Scans Electron AAOS modules and builds registry JSON | None (glob) | **None found** (standalone script) | **Partial** -- used as CLI script |
...
| `context-loader.js` | Multi-agent session context loader | context-detector.js | core/index.js, .electron-aaos-core/scripts/session-context-loader.js | No |
...
### 4.1 CLI Commands (`.electron-aaos-core/cli/commands/`)

| CLI Command | Core Module(s) Used |
|-------------|-------------------|
| `electron-aaos workers list/info/search-keyword/search-semantic` | `registry/registry-loader` |
| `electron-aaos manifest validate` | `manifest/manifest-validator` |
| `electron-aaos manifest regenerate` | `manifest/manifest-generator` |
| `electron-aaos qa run/status` | `quality-gates/quality-gate-manager` |
| `electron-aaos mcp add` | `mcp/global-config-manager`, `mcp/os-detector` |
| `electron-aaos mcp status` | `mcp/global-config-manager`, `mcp/symlink-manager`, `mcp/os-detector`, `mcp/config-migrator` |
| `electron-aaos mcp setup` | `mcp/global-config-manager`, `mcp/os-detector` |
| `electron-aaos mcp link` | `mcp/symlink-manager`, `mcp/global-config-manager`, `mcp/os-detector`, `mcp/config-migrator` |
| `electron-aaos config show/diff/migrate/validate/init-local` | `config/config-resolver`, `config/merge-utils`, `config/env-interpolator` |

### 4.2 Development Scripts (`.electron-aaos-core/development/scripts/`)
...
### 4.3 Package Entry Point (`.electron-aaos-core/index.js`)

| Entry Point | Core Module(s) Used |
|-------------|-------------------|
| `.electron-aaos-core/index.js` (npm package) | `elicitation/elicitation-engine` |
...
| `../../infrastructure/scripts/security-checker` | elicitation/elicitation-engine.js | Exists outside core/ at `.electron-aaos-core/infrastructure/scripts/` |
...
| elicitation | (none internally) | core/index, .electron-aaos-core/index, development/scripts |
