# Electron AAOS Pro Developer Workflow

This guide covers the two developer workflows for working with the Electron AAOS Open Core architecture.

## Architecture Overview

Electron AAOS uses a dual-repository Open Core model:

- **electron-aaos-core** (public) — Open-source framework, available to everyone
- **electron-aaos-pro** (private) — Premium features, available to team members and licensees

The `pro/` directory in electron-aaos-core is a git submodule pointing to the electron-aaos-pro repository.
...
├── src/
├── packages/
├── pro/ ─── git submodule ──► Electron AAOSAI/electron-aaos-pro (private)
├── squads/ (community)
└── package.json
```
...
- You do NOT need access to Electron AAOSAI/electron-aaos-pro

---

## Workflow 2: Pro Developer (Team Member with Access)

Team members with access to the private electron-aaos-pro repository can work on both repos.

### Initial Setup (New Clone)

```bash
# Clone with submodules
git clone --recurse-submodules https://github.com/Electron AAOSAI/electron-aaos-core.git
cd electron-aaos-core
...
2. Push `electron-aaos-core` changes second: `cd .. && git push`

This ensures the submodule pointer in electron-aaos-core references a valid commit in electron-aaos-pro.
...
if (isProAvailable()) {
  console.log('Electron AAOS Pro v' + getProVersion() + ' detected');
...
|------------|----------|-------|---------|
| **electron-aaos-core** | Without submodules | Core-only (pro/ absent) | npm (excludes pro/) |
| **electron-aaos-pro** | With electron-aaos-core cloned | Integration (pro/ symlinked) | GitHub Packages |
...
You need access to `Electron AAOSAI/electron-aaos-pro`. Contact the team lead.
...
- [Story PRO-5: Repository Bootstrap](../../stories/epics/epic-pro-electron-aaos-pro-architecture/story-pro-5-repo-bootstrap.md)
