# Electron AAOS Cross-Reference Phase 6: Supporting Systems Analysis

> **Generated:** 2026-02-05
> **Scope:** All supporting systems in `.electron-aaos-core/`
...
**Location:** `.electron-aaos-core/workflow-intelligence/`
**Total Files:** 19
**Purpose:** Provides intelligent workflow analysis, suggestions, pattern learning, and confidence scoring for the Electron AAOS agent orchestration system.
...
| `registry/workflow-registry.js` | Central registry of all available workflows | `index.js`, `suggestion-engine.js` | Loads from `.electron-aaos-core/development/workflows/` | No |
...
**Location:** `.electron-aaos-core/monitor/hooks/`
**Total Files:** 10
...
**Location:** `.electron-aaos-core/core/quality-gates/` (10 files) and `.electron-aaos-core/quality/` (4 files)
**Total Files:** 14
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `metrics-collector.js` | Collects and aggregates quality metrics across layers | `metrics-hook.js`, CLI metrics commands | Stores to `.electron-aaos/data/quality-metrics.json` | No |
...
| `seed-metrics.js` | Seeds initial demo metrics data | CLI `electron-aaos metrics seed` command | Uses `MetricsCollector` | No |
...
**Location:** `.electron-aaos-core/schemas/`
**Total Files:** 6
...
**Location:** `.electron-aaos-core/cli/commands/`
**Total Files:** 37 (across 8 command groups)
**Purpose:** Commander.js-based CLI commands for the `electron-aaos` executable. Provides the CLI-first interface for all framework operations.

### Architecture

Commands are organized by domain into subdirectories, each with an `index.js` entry point. Registered in the main CLI (`cli/index.js`) and invoked via `bin/electron-aaos.js`.
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `config/index.js` | Config hierarchy management: show, diff, migrate, validate, init-local | `cli/index.js`, `bin/electron-aaos.js` | Uses `core/config/config-resolver.js`, `merge-utils.js`, `env-interpolator.js` | No |
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `validate/index.js` | Validate Electron AAOS installation integrity | `cli/index.js` | Uses `install-manifest.yaml` | No |

### External References (20+ files reference CLI commands)

- **Main CLI:** `cli/index.js` registers all command groups
- **Entry point:** `bin/electron-aaos.js` creates the Commander program
...
**Location:** `.electron-aaos-core/agent-teams/`
**Total Files:** 0 (directory does not exist or is empty)
**Purpose:** Was intended to hold agent team configurations.

### Status: DOES NOT EXIST

The `agent-teams/` directory was not found in the `.electron-aaos-core/` directory. However, references to "agent-teams" exist in:
...
**Location:** `.electron-aaos-core/tools/`
...
The `tools/` directory exists (referenced in `core-config.yaml` as `toolsLocation: .electron-aaos-core/tools`) but contains no files. Tool configurations may have been:
...
- `core-config.yaml`: `toolsLocation: .electron-aaos-core/tools`
- `framework-config.yaml`: `tools_dir: ".electron-aaos-core/tools"`
...
**Location:** `.electron-aaos-core/manifests/`
**Total Files:** 4
**Purpose:** CSV-based registry of all agents, tasks, and workers with JSON schema validation. Acts as the central inventory of all Electron AAOS components.
...
**Location:** `.electron-aaos-core/core/elicitation/` (5 files) and `.electron-aaos-core/elicitation/` (3 files)
**Total Files:** 8
...
| `session-manager.js` | Manages elicitation session persistence (save/load/resume) | `elicitation-engine.js` | Stores sessions in `.electron-aaos-sessions/` | No |
...
- **Core config:** `core-config.yaml` has `elicitationLocation: .electron-aaos-core/elicitation`
- **Framework config:** `framework-config.yaml` has `elicitation_dir: ".electron-aaos-core/elicitation"`
...
**Location:** `.electron-aaos-core/docs/standards/`
**Total Files:** 16
...
| `Electron AAOS-LIVRO-DE-OURO-V2.1-COMPLETE.md` | **PRIMARY** -- Complete v2.1 framework guide (consolidated) | All agents, onboarding | Supersedes all legacy Livro docs | No |
...
| `Electron AAOS-COLOR-PALETTE-V2.1.md` | Complete color system for Electron AAOS branding | Dashboard, UI components | Referenced by UX agent | No |
| `Electron AAOS-COLOR-PALETTE-QUICK-REFERENCE.md` | Quick reference for color palette | Developers, designers | Subset of full palette doc | No |
...
| `Electron AAOS-LIVRO-DE-OURO.md` | v2.0 base document (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `Electron AAOS-LIVRO-DE-OURO-V2.1.md` | v2.1 delta document (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `Electron AAOS-LIVRO-DE-OURO-V2.1-SUMMARY.md` | v2.1 summary (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `Electron AAOS-LIVRO-DE-OURO-V2.2-SUMMARY.md` | Future v2.2 planning (DRAFT) | Planning only | Not yet active | No (Draft) |
| `Electron AAOS-FRAMEWORK-MASTER.md` | v2.0 framework doc (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
...
4 files are explicitly deprecated (superseded by `Electron AAOS-LIVRO-DE-OURO-V2.1-COMPLETE.md`), 1 is an archive candidate. These should be cleaned up or moved to an `archive/` directory.

---

## 11. Processes System

**Location:** `.electron-aaos-core/processes/`
**Total Files:** 0 (directory does not exist)
**Purpose:** Was intended to hold process definitions.

### Status: DOES NOT EXIST

No `processes/` directory was found under `.electron-aaos-core/`. This may be:
...
No references to `.electron-aaos-core/processes/` were found in the codebase.

---

## 12. Root Config System

**Location:** `.electron-aaos-core/` (root-level files)
**Total Files:** 8
...
| `install-manifest.yaml` | Complete inventory of all files in the framework | Installer, validator, upgrader | Lists every file in `.electron-aaos-core/` | No |
| `user-guide.md` | User guide for Electron AAOS Electron AAOS | Users, onboarding | Referenced by workflows, brownfield guide | No |
...
| `package.json` | npm package definition for `@electron-aaos-fullstack/core` (v4.31.0) | npm, build system | Defines dependencies, exports, scripts | No |
...
| 13 | Root Config | `.electron-aaos-core/` root | 8 | Active | In migration (monolithic to layered) |
...
|------|--------|--------|----------------|
| `docs/standards/Electron AAOS-LIVRO-DE-OURO.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/Electron AAOS-LIVRO-DE-OURO-V2.1.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/Electron AAOS-LIVRO-DE-OURO-V2.1-SUMMARY.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/Electron AAOS-FRAMEWORK-MASTER.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
...
|-----------|---------------|--------|
| `.electron-aaos-core/agent-teams/` | install-manifest, source-tree docs, core-architecture, data/electron-aaos-kb.md | Directory does not exist |
| `.electron-aaos-core/tools/` (content) | core-config.yaml, framework-config.yaml | Directory exists but is empty |
| `.electron-aaos-core/processes/` | Not referenced | Does not exist |
...
2. **Manifests are the Registry:** `manifests/workers.csv` contains 204 entries covering all Electron AAOS components. The CLI `workers` commands are the primary consumer.
...
User Request
    |
    v
CLI Commands (bin/electron-aaos.js)
    |
