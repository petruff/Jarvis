...
flowchart TB
    ...
    PF3 -->|Existe| PF4
    PF3 -->|Nao existe| ERR3[Erro: Electron AAOS incompleto]
...
```mermaid
graph TB
    subgraph Electron AAOS["Electron AAOS Core"]
    ...
    subgraph FS["File System"]
        WT[.electron-aaos/worktrees/{storyId}/<br/>Worktree Directory]
        LOG[.electron-aaos/logs/merges/<br/>Merge Audit Logs]
    end
    ...
    style Electron AAOS fill:#e3f2fd
...
**Estrutura Criada:**
```
.electron-aaos/worktrees/{storyId}/     # Diretorio de trabalho
...
```bash
git worktree add .electron-aaos/worktrees/{storyId} -b auto-claude/{storyId}
```
...
```bash
Electron AAOS_WORKTREE=/path/to/.electron-aaos/worktrees/{storyId}
Electron AAOS_STORY={storyId}
```
...
| Propriedade | Valor |
|-------------|-------|
| **Localizacao** | `.electron-aaos-core/development/tasks/create-worktree.md` |
| **Agente** | @devops (Gage) |
| **Versao** | 1.0 |
| **Story** | 1.3 |
...
| **Electron AAOS Core** | Instalado | Verificar `.electron-aaos-core/` |
...
interface WorktreeInfo {
  ...
  path: string;              // '/abs/path/.electron-aaos/worktrees/STORY-42'
...
| `WorktreeManager not found` | Electron AAOS incompleto | Reinstalar Electron AAOS |
...
ls .electron-aaos-core/infrastructure/scripts/worktree-manager.js
```
...
3. Aumentar limite (se necessario) em `.electron-aaos/config.yaml`
...
3. Remocao manual:
   ```bash
   git worktree remove .electron-aaos/worktrees/{storyId} --force
...
| Arquivo | Caminho |
|---------|---------|
| **Workflow Definition** | `.electron-aaos-core/development/workflows/auto-worktree.yaml` |
| **Task Create** | `.electron-aaos-core/development/tasks/create-worktree.md` |
| **WorktreeManager** | `.electron-aaos-core/infrastructure/scripts/worktree-manager.js` |
...
*Documentacao gerada automaticamente pelo Electron AAOS-FULLSTACK*
