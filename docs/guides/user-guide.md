# Electron AAOS User Guide

> **EN**

---

Complete guide for using Electron AAOS Electron AAOS - the AI-Orchestrated System for Full Stack Development.
...
Before using Electron AAOS, ensure you have:
...
Electron AAOS provides orchestrated structure while allowing flexibility in communication. This means:
...
### The Electron AAOS Difference

| Traditional AI Dev            | Electron AAOS                                    |
| ----------------------------- | --------------------------------------- |
...
Electron AAOS includes 11 specialized agents, each with a distinct role and personality:
...
@electron-aaos-master        # Activate Pax (Orchestrator)
...
Tasks are the primary entry point in Electron AAOS. Everything is a task.
...
Squads are modular teams of AI agents that extend Electron AAOS functionality.
...
```
my-project/
├── .electron-aaos-core/                # Framework configuration
...
The primary configuration is in `.electron-aaos-core/core/config/`:
...
Electron AAOS supports multiple IDEs. Configuration is synchronized across:
...
Electron AAOS implements 3-layer quality gates:
...
_Electron AAOS Electron AAOS User Guide v2.1.0_
