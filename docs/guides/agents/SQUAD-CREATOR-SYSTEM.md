# Sistema de Criacao e Gerenciamento de Squads Electron AAOS
...
O **Squad Creator** (Craft) e o agente especializado do Electron AAOS para criacao, validacao, publicacao e gerenciamento de squads. Squads sao pacotes modulares de agentes, tasks, workflows e recursos que podem ser reutilizados entre projetos.

Este sistema implementa a **arquitetura task-first** do Electron AAOS, onde tasks sao o ponto de entrada principal para execucao, e agentes orquestram essas tasks.

### Propositos do Sistema

- **Criar squads** seguindo padroes e estrutura do Electron AAOS
- **Validar squads** contra JSON Schema e especificacoes de task
- **Listar squads** locais do projeto
- **Distribuir squads** em 3 niveis (Local, electron-aaos-squads, Electron AAOS API)
...
4. **3 Niveis de Distribuicao**: Local, Publico (electron-aaos-squads), Marketplace (Electron AAOS API)
5. **Integracao com electron-aaos-core**: Squads trabalham em sinergia com o framework
...
| Arquivo | Proposito |
|---------|-----------|
| `.electron-aaos-core/development/agents/squad-creator.md` | Definicao core do agente Squad Creator |
| `.claude/commands/Electron AAOS/agents/squad-creator.md` | Comando Claude Code para ativar @squad-creator |

### Arquivos de Tasks do @squad-creator

| Arquivo | Comando | Proposito | Status |
|---------|---------|-----------|--------|
| `.electron-aaos-core/development/tasks/squad-creator-create.md` | `*create-squad` | Cria novo squad com estrutura completa | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-design.md` | `*design-squad` | Analisa documentacao e gera blueprint | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-validate.md` | `*validate-squad` | Valida squad contra schema e padroes | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-list.md` | `*list-squads` | Lista squads locais | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-analyze.md` | `*analyze-squad` | Analisa estrutura e sugere melhorias | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-extend.md` | `*extend-squad` | Estende squad com novos componentes | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-migrate.md` | `*migrate-to-v2` | Migra squad para formato v2 | Ativo |
| `.electron-aaos-core/development/tasks/squad-generate-skills.md` | `*generate-skills` | Gera skills de conhecimento do squad | Ativo |
| `.electron-aaos-core/development/tasks/squad-generate-workflow.md` | `*generate-workflow` | Gera workflow de orquestracao YAML | Ativo |
| `.electron-aaos-core/development/tasks/squad-creator-download.md` | `*download-squad` | Baixa squad do repositorio publico | Placeholder (Sprint 8) |
| `.electron-aaos-core/development/tasks/squad-creator-publish.md` | `*publish-squad` | Publica squad no electron-aaos-squads | Placeholder (Sprint 8) |
| `.electron-aaos-core/development/tasks/squad-creator-sync-electron-aaos.md` | `*sync-squad-electron-aaos` | Sincroniza squad com Electron AAOS API | Placeholder (Sprint 8) |

### Arquivos de Tasks Relacionadas

| Arquivo | Comando | Proposito |
|---------|---------|-----------|
| `.electron-aaos-core/development/tasks/create-agent.md` | `*create-agent` | Cria definicao de agente individual |
| `.electron-aaos-core/development/tasks/create-task.md` | `*create-task` | Cria arquivo de task individual |
| `.electron-aaos-core/development/tasks/create-workflow.md` | `*create-workflow` | Cria workflow de orquestracao |

### Scripts de Suporte

| Arquivo | Classe/Funcao | Proposito |
|---------|---------------|-----------|
| `.electron-aaos-core/development/scripts/squad/squad-generator.js` | `SquadGenerator` | Gera estrutura de squad completa |
| `.electron-aaos-core/development/scripts/squad/squad-validator.js` | `SquadValidator` | Valida squad contra schema e padroes |
| `.electron-aaos-core/development/scripts/squad/squad-loader.js` | `SquadLoader` | Carrega e resolve squads |
| `.electron-aaos-core/development/scripts/squad/squad-designer.js` | `SquadDesigner` | Analisa docs e gera blueprints |
| `.electron-aaos-core/development/scripts/squad/squad-analyzer.js` | `SquadAnalyzer` | Analisa estrutura de squads |
| `.electron-aaos-core/development/scripts/squad/squad-extender.js` | `SquadExtender` | Estende squads existentes |
| `.electron-aaos-core/development/scripts/squad/squad-migrator.js` | `SquadMigrator` | Migra squads para v2 |
| `.electron-aaos-core/development/scripts/squad/squad-downloader.js` | `SquadDownloader` | Baixa squads do repositorio |
| `.electron-aaos-core/development/scripts/squad/squad-publisher.js` | `SquadPublisher` | Publica squads |

### Schemas JSON

| Arquivo | Proposito |
|---------|-----------|
| `.electron-aaos-core/schemas/squad-schema.json` | Schema de validacao do squad.yaml |
| `.electron-aaos-core/schemas/squad-design-schema.json` | Schema de validacao de blueprints |
...
flowchart TB
    ...
    subgraph DISTRIBUTE["🚀 DISTRIBUTION"]
        ...
        PUBLIC["🌐 Public<br/>github.com/Electron AAOSAI/electron-aaos-squads"]
        MARKET["💰 Marketplace<br/>api.electron-aaos.dev/squads"]
    end

    VALID --> LOCAL
    VALID -->|"*publish-squad"| PUBLIC
    VALID -->|"*sync-squad-electron-aaos"| MARKET
...
| Comando | Task File | Operacao |
|---------|-----------|----------|
| `*download-squad` | `squad-creator-download.md` | DOWNLOAD squad do electron-aaos-squads |
| `*publish-squad` | `squad-creator-publish.md` | PUBLISH squad para electron-aaos-squads |
| `*sync-squad-electron-aaos` | `squad-creator-sync-electron-aaos.md` | SYNC squad para Electron AAOS API |
...
flowchart LR
    ...
    SQUADS[("📦 ./squads/")]
    Electron AAOS_SQUADS[("🌐 electron-aaos-squads")]
    SYNKRA[("💰 Electron AAOS API")]

    SC_CREATE --> SQUADS
    SC_VALIDATE --> SQUADS
    SC_LIST --> SQUADS
    DEVOPS_PUB --> Electron AAOS_SQUADS
    DEVOPS_PUB --> SYNKRA
...
electron-aaos:
  minVersion: string  # versao minima do Electron AAOS
  type: squad
...
flowchart LR
    ...
    subgraph PUBLIC["🌐 Nivel 2: Publico"]
        P_REPO["github.com/Electron AAOSAI/electron-aaos-squads"]
        P_DESC["Squads da comunidade (gratuitos)"]
        P_CMD["*publish-squad"]
    end

    subgraph MARKET["💰 Nivel 3: Marketplace"]
        M_API["api.electron-aaos.dev/squads"]
        M_DESC["Squads premium via Electron AAOS API"]
        M_CMD["*sync-squad-electron-aaos"]
    end
...
- [Task: squad-creator-create.md](.electron-aaos-core/development/tasks/squad-creator-create.md)
- [Task: squad-creator-validate.md](.electron-aaos-core/development/tasks/squad-creator-validate.md)
- [Task: squad-creator-design.md](.electron-aaos-core/development/tasks/squad-creator-design.md)
- [Script: squad-generator.js](.electron-aaos-core/development/scripts/squad/squad-generator.js)
- [Script: squad-validator.js](.electron-aaos-core/development/scripts/squad/squad-validator.js)
- [Schema: squad-schema.json](.electron-aaos-core/schemas/squad-schema.json)
- [Agent: squad-creator.md](.electron-aaos-core/development/agents/squad-creator.md)
- [Command: squad-creator.md](.claude/commands/Electron AAOS/agents/squad-creator.md)
...
| **Tasks Placeholder** | 3 (download, publish, sync-electron-aaos) |
...
| **Niveis Distribuicao** | 3 (Local, electron-aaos-squads, Electron AAOS API) |
