...
> Source: `.electron-aaos-core/development/scripts/unified-activation-pipeline.js` (Story ACT-6)
> Previous source: `.electron-aaos-core/development/scripts/greeting-builder.js` (949 lines)

## Overview

Every Electron AAOS agent goes through a **single unified activation pipeline** before presenting its greeting. As of Story ACT-6, the previous two-path architecture (Path A: direct GreetingBuilder invocation, Path B: generate-greeting.js CLI wrapper) has been consolidated into one entry point.
...
```
.electron-aaos-core/development/agents/{agent-id}.md
```
...
sequenceDiagram
    ...
    alt Cache hit
        ...
        ACL->>FS: readFile(.electron-aaos-core/development/agents/{id}.md)
...
1. **Phase 1 (Parallel):** Loads 5 subsystems concurrently via `Promise.all()`:
   ...
   - `SessionContextLoader.loadContext(agentId)` -- Session state from `.electron-aaos/session-state.json`
...
**Cache file:** `.electron-aaos/project-status.yaml`
...
Reads from `.electron-aaos-core/core-config.yaml` path `agentIdentity.greeting.preference`.
...
await mode.load();  // Reads .electron-aaos/config.yaml -> permissions.mode
...
1. Reads current mode from `.electron-aaos/config.yaml`
...
```javascript
const { enforcePermission } = require('./.electron-aaos-core/core/permissions');
...
The `environment-bootstrap` task initializes `.electron-aaos/config.yaml` with `permissions.mode: ask` as the default. If the config file is missing or the field is absent, the system defaults to `ask` mode.
...
Each agent has specific config requirements defined in `.electron-aaos-core/data/agent-config-requirements.yaml`:

| Agent | Config Sections | Files Loaded | Performance Target |
|-------|----------------|--------------|-------------------|
| `electron-aaos-master` | dataLocation, registry | electron-aaos-kb.md (lazy) | <30ms |
...
| File | Loader | Purpose |
|------|--------|---------|
| `.electron-aaos-core/development/agents/{agent-id}.md` | AgentConfigLoader | Agent definition |
| `.electron-aaos-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| `.electron-aaos-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Per-agent config needs |
| `.electron-aaos-core/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |

### Loaded conditionally

| File | Condition | Loader |
|------|-----------|--------|
| `.electron-aaos/session-state.json` | Path B (CLI wrapper) or file-based session detection | ContextDetector / SessionContextLoader |
| `.electron-aaos/project-status.yaml` | Cache check (60s TTL) | ProjectStatusLoader |
...
graph TD
    ...
    SCL --> SSF[.electron-aaos/session-state.json]
    PSL --> GIT[git CLI commands]
    PSL --> STORIES[docs/stories/**/*.md]
    PSL --> WTM[WorktreeManager]
    PSL --> PSC[.electron-aaos/project-status.yaml]
    GCD_R --> GIT
    WN --> WP[.electron-aaos-core/data/workflow-patterns.yaml]
    GPM --> CC
```
...
The `user_profile` setting (`bob` or `advanced`) affects behavior across the entire Electron AAOS pipeline. This section documents every file that references `user_profile`/`userProfile` and the behavioral difference between modes.
...
|---|------|----------|----------------|---------------------|
| 1 | `.electron-aaos-core/core-config.yaml` | Config | `user_profile: bob` | `user_profile: advanced` |
| 2 | `.electron-aaos-core/development/scripts/greeting-builder.js` | Greeting | Redirects non-PM agents to @pm; hides role/status sections; returns empty commands for non-PM | Full contextual greeting with all sections and commands |
| 3 | `.electron-aaos-core/development/scripts/generate-greeting.js` | Greeting | Uses GreetingBuilder, same bob restrictions | Uses GreetingBuilder, full features |
| 4 | `.electron-aaos-core/development/scripts/greeting-preference-manager.js` | Greeting | Forces preference to `minimal` or `named`; overrides `auto`/`archetypal` | All 4 preferences available (`auto`, `minimal`, `named`, `archetypal`) |
| 5 | `.electron-aaos-core/infrastructure/scripts/validate-user-profile.js` | Validation | Validates `bob` as legal value; normalizes case | Validates `advanced` as legal value; normalizes case |
| 6 | `.electron-aaos-core/core/config/config-resolver.js` | Config | `toggleUserProfile()` switches bob<->advanced; L5 user layer has priority | Same toggle; resolveConfig merges layers |
| 7 | `.electron-aaos-core/core/config/migrate-config.js` | Config | Categorizes `user_profile` as USER_FIELD during migration | Same categorization |
| 8 | `.electron-aaos-core/core/config/schemas/user-config.schema.json` | Schema | `enum: ["bob", "advanced"]` validation | Same validation |
| 9 | `.electron-aaos-core/core/config/templates/user-config.yaml` | Template | Default template value: `bob` | N/A (template default is bob) |
| 10 | `.electron-aaos-core/development/agents/pm.md` | Agent | PM becomes sole orchestrator; bob mode session detection; orchestrates other agents internally | PM operates as normal PM with standard workflow |
...
| 16 | `packages/electron-aaos-install/src/installer.js` | Install | Sets `config.user_profile = 'bob'` in YAML | Sets `config.user_profile = 'advanced'` |
| 17 | `.electron-aaos-core/development/tasks/environment-bootstrap.md` | Task | Documents bob selection flow | Documents advanced selection flow |
| 18 | `docs/electron-aaos-workflows/bob-orchestrator-workflow.md` | Docs | Full bob orchestrator workflow documentation | N/A (bob-specific doc) |
...
| `@electron-aaos-master` | 0 (uses string visibility) | Empty (redirect to @pm) | Fallback: first 12 commands |

**Note:** Agents with 0 `key` commands (`qa`, `data-engineer`, `devops`, `ux-design-expert`, `electron-aaos-master`) lack `visibility` array metadata on their commands. In `advanced` mode `workflow` sessions, they fall back to showing first 12 commands. This is a known gap tracked for future improvement.
