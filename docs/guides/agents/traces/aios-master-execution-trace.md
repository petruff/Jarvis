# @electron-aaos-master (Orion) - Execution Trace

> Traced from source code, not documentation.
> Agent definition: `.electron-aaos-core/development/agents/electron-aaos-master.md`
...
|-------|------|--------|---------|
| 1 | `.electron-aaos-core/development/agents/electron-aaos-master.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
| 2 | `.electron-aaos-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| 3 | `.electron-aaos-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Config sections: dataLocation, registry |
...
| File | Condition | Size |
|------|-----------|------|
| `.electron-aaos-core/data/electron-aaos-kb.md` | ONLY when user types `*kb` | 35KB |
...
sequenceDiagram
    ...
    participant AMd as electron-aaos-master.md
    ...
    CC->>ACL: loadAgentDefinition('electron-aaos-master')
...
electron-aaos-master:
  config_sections:
    ...
    - registry
  files_loaded:
    - path: .electron-aaos-core/data/electron-aaos-kb.md
      lazy: true
      condition: kb_command
      size: 35KB
  lazy_loading:
    registry: false      # Always load (15KB, frequently used)
    electron-aaos-kb: true        # Load only on *kb command
...
| Role | `persona.role` | Master Orchestrator, Framework Developer & Electron AAOS Method Expert |
...
| `*list-components` | (built-in, scans .electron-aaos-core/) | full | No |
...
**Task file:** `.electron-aaos-core/development/tasks/kb-mode-interaction.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `kb-mode-interaction.md` | Task | EXISTS |
| `.electron-aaos-core/data/electron-aaos-kb.md` | Data | EXISTS (lazy, 35KB) |
...
flowchart TD
    ...
    B --> C[Load electron-aaos-kb.md into context]
    C --> D[Toggle KB mode ON]
    D --> E[Agent now answers with full Electron AAOS Method knowledge]
    E --> F[User types *kb again to toggle OFF]
```

**CRITICAL:** electron-aaos-kb.md is NEVER loaded unless user explicitly types `*kb`.

---

### `*create`

**Task files:** `.electron-aaos-core/development/tasks/create-agent.md`, `create-task.md`, `create-workflow.md`
...
| `agent-template.yaml` | Template | EXISTS (in .electron-aaos-core/product/templates/) |
| `task-template.md` | Template | EXISTS (in .electron-aaos-core/product/templates/) |
| `workflow-template.yaml` | Template | EXISTS (in .electron-aaos-core/product/templates/) |
| `security-checker.js` | Util | EXISTS (in .electron-aaos-core/infrastructure/scripts/) |
| `yaml-validator.js` | Util | EXISTS (in .electron-aaos-core/infrastructure/scripts/) |
...
flowchart TD
    ...
    M --> N{Validation pass?}
    N -->|yes| O[Write component to .electron-aaos-core/development/{type}/]
    N -->|no| P[Report validation errors]
    O --> Q[Update manifest if applicable]
```

**Expected output:** New Electron AAOS component file written to appropriate directory.

---

### `*modify`

**Task files:** `.electron-aaos-core/development/tasks/modify-agent.md`, `modify-task.md`, `modify-workflow.md`
...
**Task file:** `.electron-aaos-core/development/tasks/update-manifest.md`
...
**Task file:** `.electron-aaos-core/development/tasks/deprecate-component.md`
...
**Task file:** `.electron-aaos-core/development/tasks/propose-modification.md`
...
**Task file:** `.electron-aaos-core/development/tasks/undo-last.md`
...
**Task file:** `.electron-aaos-core/development/tasks/validate-workflow.md`
...
| `yaml-validator.js` | Util | EXISTS |
| `.electron-aaos-core/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task files:** `.electron-aaos-core/development/tasks/run-workflow.md`, `run-workflow-engine.md`
**Template:** `.electron-aaos-core/development/templates/subagent-step-prompt.md`
...
| `subagent-step-prompt.md` | Template | EXISTS (in .electron-aaos-core/development/templates/) |
| `workflow-management.md` | Util | EXISTS (in .electron-aaos-core/scripts/) |
| `.electron-aaos-core/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task file:** `.electron-aaos-core/development/tasks/analyze-framework.md`
...
flowchart TD
    ...
    B --> C[Scan .electron-aaos-core/ directory structure]
...
**Task file:** `.electron-aaos-core/development/tasks/execute-checklist.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `execute-checklist.md` | Task | EXISTS |
| `.electron-aaos-core/product/checklists/{checklist}.md` | Checklist | Dynamically loaded |
...
flowchart TD
    ...
    B --> C{Name provided?}
    C -->|yes| D[Resolve checklist from .electron-aaos-core/product/checklists/]
...
**Task file:** `.electron-aaos-core/development/tasks/create-doc.md`
**Templates:** All 15 templates from `.electron-aaos-core/product/templates/`
...
| `.electron-aaos-core/product/data/elicitation-methods.md` | Data | EXISTS |
...
flowchart TD
    ...
    C -->|yes| D[Load template from .electron-aaos-core/product/templates/]
...
**Task file:** `.electron-aaos-core/development/tasks/shard-doc.md`
...
**Task file:** `.electron-aaos-core/development/tasks/document-project.md`
...
**Task file:** `.electron-aaos-core/development/tasks/add-tech-doc.md` (MISSING)
...
**Task file:** `.electron-aaos-core/development/tasks/create-next-story.md`
...
**Task file:** `.electron-aaos-core/development/tasks/advanced-elicitation.md`
...
**Task file:** `.electron-aaos-core/development/tasks/correct-course.md`
...
**Task file:** `.electron-aaos-core/development/tasks/index-docs.md`
...
| `*guide` | Renders the `## Electron AAOS Master Guide` section from agent .md |
...
| `*exit` | Exits electron-aaos-master mode, returns to base Claude Code |
| `*agent {name}` | Reads and displays info about a specialized agent (use `@` to transform into it) |
| `*list-components` | Scans .electron-aaos-core/ and lists all framework components (agents, tasks, workflows, templates, checklists) |
...
|-------------|-----------|---------|
| @electron-aaos-master -> @pm | Delegate | Epic/story creation (brownfield-create-epic, brownfield-create-story) |
| @electron-aaos-master -> @analyst | Delegate | Brainstorming sessions (*brainstorm) |
| @electron-aaos-master -> @qa | Delegate | Test suite creation (*create-suite) |
| @electron-aaos-master -> @architect | Delegate | AI prompt generation (*generate-ai-prompt) |
| @electron-aaos-master -> @dev | Handoff | Story implementation after story creation |
| @electron-aaos-master -> @devops | Delegate | Git push operations, PR creation |
| @electron-aaos-master -> any agent | Execute | Can execute any task from any agent directly |
| Any agent -> @electron-aaos-master | Escalate | Cross-agent coordination, framework operations |
...
|------------|---------------|-------------|
| `security-checker.js` | `.electron-aaos-core/development/utils/` | `.electron-aaos-core/infrastructure/scripts/security-checker.js` |
| `yaml-validator.js` | `.electron-aaos-core/development/utils/` | `.electron-aaos-core/infrastructure/scripts/yaml-validator.js` |
| `workflow-management.md` | `.electron-aaos-core/development/utils/` | `.electron-aaos-core/scripts/workflow-management.md` |
| `subagent-step-prompt.md` | `.electron-aaos-core/product/templates/` | `.electron-aaos-core/development/templates/subagent-step-prompt.md` |
| `elicitation-methods.md` | `.electron-aaos-core/data/` | `.electron-aaos-core/product/data/elicitation-methods.md` |
| `brainstorming-techniques.md` | `.electron-aaos-core/data/` | `.electron-aaos-core/product/data/brainstorming-techniques.md` |
