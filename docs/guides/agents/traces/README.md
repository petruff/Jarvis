# Electron AAOS Agent Execution Traces - Index

> **Story:** Electron AAOS-TRACE-001 | **Traced from source code, not documentation.**

## Overview

This directory contains comprehensive runtime execution trace documentation for all 12 Electron AAOS agents. Each document shows **exactly** what files are loaded, in what order, and why - from agent activation through every command execution.
...
| @electron-aaos-master | Nova (Orchestrator) | 33 | Direct | [electron-aaos-master-execution-trace.md](./electron-aaos-master-execution-trace.md) |
...
graph TD
    subgraph "Orchestration Layer"
        MASTER[@electron-aaos-master<br/>Nova]
    end
...
|-------|----------|-----------------|--------------|-------------|
| @electron-aaos-master | Critical | dataLocation, registry | electron-aaos-kb.md (lazy) | <30ms |
...
Source: `.electron-aaos-core/data/agent-config-requirements.yaml`
...
| File | Purpose | Lines |
|------|---------|-------|
| `.electron-aaos-core/development/scripts/greeting-builder.js` | Main activation pipeline | 949 |
| `.electron-aaos-core/development/scripts/agent-config-loader.js` | Config loading per agent | ~400 |
| `.electron-aaos-core/development/scripts/generate-greeting.js` | CLI wrapper (3 agents) | 173 |
| `.electron-aaos-core/core/session/context-detector.js` | Session type detection | ~100 |
| `.electron-aaos-core/infrastructure/scripts/project-status-loader.js` | Git/project status | ~524 |
| `.electron-aaos-core/infrastructure/scripts/git-config-detector.js` | Git config detection | ~294 |
| `.electron-aaos-core/development/scripts/greeting-preference-manager.js` | Greeting level preference | ~146 |
| `.electron-aaos-core/development/scripts/workflow-navigator.js` | Workflow suggestions | ~200 |
| `.electron-aaos-core/core/permissions/index.js` | Permission mode badge | ~100 |
| `.electron-aaos-core/data/agent-config-requirements.yaml` | Per-agent config requirements | 369 |
...
**Resolution:** The `AgentConfigLoader` resolves dependencies by searching multiple paths: `development/`, `product/`, and root `.electron-aaos-core/` directories. The path mismatch does not cause runtime failures but is documented for accuracy.
...
| @electron-aaos-master | add-tech-doc.md | `*add-tech-doc` command non-functional |
