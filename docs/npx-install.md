...
Electron AAOS Electron AAOS can be installed via NPX for quick setup without global installation. This guide covers proper usage and troubleshooting for NPX-based installations.

## Quick Start

### Correct Usage

Always run `npx @electron-aaos/electron-aaos-core install` **from your project directory**:
...
NPX executes packages in **temporary directories** (e.g., `/private/var/folders/.../npx-xxx/` on macOS). When Electron AAOS Electron AAOS runs from these temporary locations, it cannot:
...
As of version 4.31.1, Electron AAOS Electron AAOS automatically detects when it's running from an NPX temporary directory and displays a helpful error message:
...
Electron AAOS Electron AAOS detects these patterns and prevents incorrect installation.
...
Electron AAOS Electron AAOS v4.31.1+ implements two-layer detection:

1. **PRIMARY Layer** (`tools/electron-aaos-npx-wrapper.js`):
...
2. **SECONDARY Layer** (`tools/installer/bin/electron-aaos.js`):
...
- Documentation: https://@electron-aaos/electron-aaos-core.dev/docs
