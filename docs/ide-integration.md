...
Guide for integrating Electron AAOS with supported IDEs and AI development platforms.
...
Electron AAOS supports 9 AI-powered development platforms. Choose the one that best fits your workflow.
...
**Recommendation Level:** Best Electron AAOS integration
...
1. Electron AAOS automatically creates `.claude/` directory on init
...
1. Electron AAOS creates `.cursor/` directory on init
...
1. Electron AAOS creates `.windsurf/` directory and `.windsurfrules` file
...
2. Electron AAOS creates `.cline/` directory on init
...
2. Electron AAOS creates `.github/copilot-instructions.md`
...
1. Electron AAOS creates `.antigravity/` directory
...
Electron AAOS maintains a single source of truth for agent definitions and synchronizes them to all configured IDEs:
...
Electron AAOS can be configured to automatically sync on agent changes:
...
ls .claude/commands/Electron AAOS/agents/  # For Claude Code
```
...
├── Yes --> Claude Code (Best Electron AAOS integration)
└── No
...
_Electron AAOS Electron AAOS IDE Integration Guide v2.1.0_
