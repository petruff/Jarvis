...
# Especificação de Nomenclatura do Electron AAOS

> 🌐 [EN](../electron-aaos-nomenclature-specification.md) | **PT** | [ES](../es/electron-aaos-nomenclature-specification.md)
...
Este documento estabelece nomenclatura clara para o Electron AAOS diferenciando entre:
...
**Características:**
- **Escopo:** Interno a um único arquivo de task (`.electron-aaos-core/tasks/*.md`)
...
- **Localização:** Definido em `.electron-aaos-core/workflows/*.yaml` ou seções de workflow de stories
...
**Conceito:** Inspirado pelo protocolo Organizer-Worker do AsyncThink, workflows do Electron AAOS podem usar um **Organizer Agent** que coordena **Worker Agents** executando tasks em paralelo.

**Aplicação ao Electron AAOS:**
...
- Exemplo: `.electron-aaos-core/tasks/security-scan.md`

**Arquivos de Workflow:**
- Localização: `.electron-aaos-core/workflows/{workflow-name}.yaml`
- Contém: Definição de orquestração multi-task
- Exemplo: `.electron-aaos-core/workflows/story-development-workflow.yaml`
...
3. **Gerenciamento de Workflow Electron AAOS:** Padrões de workflow existentes no Electron AAOS
