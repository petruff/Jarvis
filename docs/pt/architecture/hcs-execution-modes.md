...
Este documento define os modos de execução para o Sistema de Verificação de Saúde do Electron AAOS (HCS), baseado em pesquisa de melhores práticas da indústria do Kubernetes, VS Code, Terraform, npm/yarn e padrões CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Na instalação/bootstrap**  | `npx electron-aaos install` | 60-120s | Esperado                       | Validação de setup      | ✅ **Obrigatório**           |
...
**Veredito:** Obrigatório. Parte do `npx electron-aaos install` e `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Postar no Slack (em caso de falha)
        ...
        with:
          payload: |
            {
              "text": "⚠️ Electron AAOS Health Check Falhou",
              "blocks": [
                {
                  "type": "section",
                  "text": {
