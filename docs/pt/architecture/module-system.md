...
# Arquitetura do Sistema de Módulos do Electron AAOS

> 🌐 [EN](../../architecture/module-system.md) | **PT** | [ES](../../es/architecture/module-system.md)

---

> Guia completo para a arquitetura modular v2.1 do Electron AAOS Electron AAOS.
...
O Electron AAOS Electron AAOS organiza o diretório `.electron-aaos-core/` em quatro módulos principais:
...
```mermaid
graph TB
    subgraph "Framework Electron AAOS v2.1"
        CLI[CLI / Ferramentas]
...
**Caminho:** `.electron-aaos-core/core/`
...
| `data/`          | `electron-aaos-kb.md`, `workflow-patterns.yaml`           | Base de conhecimento do framework           |
...
```javascript
// Configuração
const { loadAgentConfig, globalConfigCache } = require('./.electron-aaos-core/core');

// Sessão
const { ContextDetector, SessionContextLoader } = require('./.electron-aaos-core/core');

// Elicitation
const { ElicitationEngine, ElicitationSessionManager } = require('./.electron-aaos-core/core');
...
**Caminho:** `.electron-aaos-core/development/`
...
| --------------- | ------------------ | ------------------------- |
| Electron AAOS Master     | `electron-aaos-master`      | Orquestração do framework |
...
**Caminho:** `.electron-aaos-core/product/`
...
**Caminho:** `.electron-aaos-core/infrastructure/`
...
# Validar migração
electron-aaos migrate --validate
```
...
_Arquitetura do Sistema de Módulos Electron AAOS Electron AAOS v2.1_
