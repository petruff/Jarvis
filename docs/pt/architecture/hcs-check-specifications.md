...
**Propósito:** Verificar se os arquivos do framework Electron AAOS estão corretamente configurados e consistentes.

### Verificações

| ID     | Nome                           | Severidade | Nível | Modo  | Timeout | Descrição                                            |
| ------ | ------------------------------ | ---------- | ----- | ----- | ------- | ---------------------------------------------------- |
| PC-001 | Config existe                  | CRITICAL   | 1     | quick | 100ms   | `.electron-aaos/config.yaml` existe e é YAML válido           |
...
| PC-006 | Sem arquivos órfãos            | LOW        | 3     | full  | 5s      | Todos os arquivos em `.electron-aaos-core/` são referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    const configPath = path.join(context.projectRoot, '.electron-aaos', 'config.yaml');

    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.electron-aaos/config.yaml não encontrado',
  ...
  async heal(context) {
    const templatePath = '.electron-aaos-core/infrastructure/templates/core-config/config-template.yaml';
    const configPath = path.join(context.projectRoot, '.electron-aaos', 'config.yaml');
...
| **Windsurf**    | `.windsurf/rules/`           | Diretório existe    | TBD                        |
...
const requiredSections = [
  'Project Overview', // ou 'Electron AAOS-FULLSTACK Development Rules'
...
```yaml
# .electron-aaos-core/health-check/checks/ide-checks.yaml
checks:
  - id: IDE-VSCODE
    ...
    validation:
      type: json-schema
      schema: '.electron-aaos-core/schemas/vscode-settings.json'
    autoFix:
      tier: 2
      action: 'create-from-template'
      template: '.electron-aaos-core/infrastructure/templates/ide/vscode-settings.json'
...
Usuários podem definir verificações específicas do projeto em `.electron-aaos/custom-checks.yaml`:

```yaml
# .electron-aaos/custom-checks.yaml
version: 1.0
...
checks:
  ...
  - id: CUSTOM-001
    ...
    autoFix:
      action: create-from-template
      template: '.electron-aaos-core/infrastructure/templates/project-docs/readme-template.md'
  ...
  - id: CUSTOM-005
    ...
    type: json-schema
    path: 'package.json'
    schema: '.electron-aaos-core/schemas/package-json.json'
    severity: CRITICAL
    tier: 3
    mode: quick
```
