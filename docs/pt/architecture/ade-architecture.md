...
O **Motor de Desenvolvimento Autônomo (ADE)** é a infraestrutura do Electron AAOS para workflows de desenvolvimento autônomo. Ele permite que agentes de IA trabalhem de forma independente através de pipelines inteligentes, loops auto-recuperáveis e aprendizado persistente.
...
Todo o estado do ADE é persistido em `.electron-aaos/` para:
...
Arquivo de Estado: .electron-aaos/project-status.yaml
```
...
| ------------------- | ---------------------------------- | --------------------------------- |
| Padrões de Código   | Padrões reutilizáveis da base     | .electron-aaos/patterns/code-patterns.json |
| Gotchas             | Problemas conhecidos e soluções   | .electron-aaos/patterns/gotchas.json       |
| Insights de Sessão  | Descobertas durante sessões       | .electron-aaos/sessions/                   |
| Mapa de Codebase    | Análise de estrutura do projeto   | .electron-aaos/codebase-map.json           |
...
ADE integra-se com agentes Electron AAOS através de:
...
Todos os componentes do ADE atualizam `.electron-aaos/status.json`:
...
Localizada em `.electron-aaos-core/core-config.yaml`:
...
_Este é um padrão oficial do framework Electron AAOS documentando o Motor de Desenvolvimento Autônomo._
