...
Este documento especifica as capacidades de auto-recuperação do Sistema de Verificação de Saúde do Electron AAOS (HCS). A auto-recuperação permite que o sistema corrija automaticamente certos problemas enquanto mantém a segurança e o controle do usuário.
...
| ---------------------- | -------------------------------------------------------- | ------------- |
| `recreate_config`      | Recriar `.electron-aaos/config.yaml` ausente a partir do template | Sim           |
| `clear_cache`          | Limpar arquivos de cache obsoletos em `.electron-aaos/cache/`     | Sim           |
...
- id: PC-001
  description: '.electron-aaos/config.yaml ausente'
  severity: CRITICAL
  tier: 1
...
✅ 3 problemas auto-corrigidos:
   • Recriado .electron-aaos/config.yaml (backup: .electron-aaos/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Adicionado .electron-aaos/cache/ ao .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      ...
      - 'Verificar com: npx electron-aaos task validate deploy'
...
Passos para resolver:
  ...
  4. Verificar com: npx electron-aaos task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config do usuário (fora de .electron-aaos/)
...
safeToAutoFix:
  paths:
    - '.electron-aaos/**' # Arquivos de workspace do Electron AAOS
    - '.electron-aaos-core/**/*.yaml' # YAML do framework (cuidado)
