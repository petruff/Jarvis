...
- [Repositório Principal (electron-aaos-core)](#repositório-principal-electron-aaos-core)
...
Electron AAOS v2.1 adota uma **estratégia multi-repositório** para viabilizar desenvolvimento modular, contribuições comunitárias e separação clara entre framework principal, extensões (squads) e componentes proprietários.
...
```
Organização Electron AAOSAI
├── REPOSITÓRIOS PÚBLICOS
│   ├── electron-aaos-core          # Framework principal (Commons Clause)
│   ├── electron-aaos-squads        # Squads comunitárias (MIT)
...
## Repositório Principal (electron-aaos-core)

### Propósito

O repositório principal contém o framework Electron AAOS fundamental que todos os projetos dependem.

### Conteúdo

| Diretório                    | Descrição                                                |
| ---------------------------- | -------------------------------------------------------- |
| `.electron-aaos-core/core/`           | Fundações do framework (config, registry, gates de qualidade) |
| `.electron-aaos-core/development/`    | Definições de agentes, tarefas, workflows                |
| `.electron-aaos-core/product/`        | Templates, checklists, dados de PM                       |
| `.electron-aaos-core/infrastructure/` | Scripts, ferramentas, integrações                        |
...
Squads são extensões modulares que adicionam capacidades especializadas ao Electron AAOS.

### Repositório electron-aaos-squads
...
### Electron AAOSAI/mmos (Proprietário + NDA)
...
### Electron AAOSAI/certified-partners (Proprietário)

Recursos para parceiros Electron AAOS certificados:
...
```bash
# A partir de electron-aaos-core
npm publish --access public

# A partir de electron-aaos-squads/etl
...
2. **Dependências de Pares** - Especifique requisitos exatos de versão electron-aaos-core
...
_Última Atualização: 2026-01-28 | Time do Framework Electron AAOS_
