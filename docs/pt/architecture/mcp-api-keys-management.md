...
Este documento descreve as melhores práticas para gerenciar chaves de API usadas por servidores MCP (Model Context Protocol) no Electron AAOS. O gerenciamento adequado de chaves de API é crítico para segurança e integridade operacional.

---

## Arquitetura MCP no Electron AAOS

Electron AAOS utiliza Docker MCP Toolkit como a infraestrutura principal de MCP:
...
## Governança MCP no Electron AAOS
