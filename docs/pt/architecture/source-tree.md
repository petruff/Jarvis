...
# Estrutura da Árvore de Código Electron AAOS
...
**Aviso de Migração:** Este documento será migrado para o repositório `Electron AAOSAI/electron-aaos-core` no Q2 2026 (veja Decisão 005)
...
- [Estrutura Atual (electron-aaos-core Brownfield)](#estrutura-atual-electron-aaos-core-brownfield)
...
Electron AAOS utiliza uma **arquitetura de camada dupla**:

1. **Framework Core** (`.electron-aaos-core/`) - Componentes portáteis do framework
...
## Estrutura Atual (electron-aaos-core Brownfield)

```
electron-aaos-core/                             # Raiz (projeto brownfield)
├── .electron-aaos-core/                        # Core do framework (portátil)
...
│
├── bin/                               # Executáveis CLI
│   ├── @electron-aaos/electron-aaos-core.js              # Ponto de entrada principal da CLI
│   └── electron-aaos-minimal.js                # CLI mínima
│
...
**Propósito:** Componentes portáteis do framework que funcionam em qualquer projeto Electron AAOS.
...
Workflows:
  Location: .electron-aaos-core/workflows/
  Format: YAML
...
Utilities:
  Location: .electron-aaos-core/utils/
...
Squads são extensões modulares que adicionam capacidades especializadas ao Electron AAOS. Diferente dos Squads descontinuados, Squads seguem uma estrutura de template padronizada.
...
│   └── cli/                           # Interface CLI
│
├── .electron-aaos-core/                        # Ativos do framework (estrutura atual)
│   ├── agents/
│   ├── tasks/
│   ├── templates/
│   └── ...
...
Localização: .electron-aaos-core/agents/{agent-name}.md
Exemplo: .electron-aaos-core/agents/security-expert.md

# Estou criando uma nova tarefa:
Localização: .electron-aaos-core/tasks/{task-name}.md
Exemplo: .electron-aaos-core/tasks/deploy-to-production.md

# Estou criando um novo workflow:
Localização: .electron-aaos-core/workflows/{workflow-name}.yaml
Exemplo: .electron-aaos-core/workflows/continuous-deployment.yaml

# Estou criando um novo template:
Localização: .electron-aaos-core/product/templates/{template-name}-tmpl.{yaml|md}
Exemplo: .electron-aaos-core/product/templates/deployment-plan-tmpl.yaml

# Estou escrevendo uma story:
Localização: docs/stories/{epic-context}/{story-file}.md
Exemplo: docs/stories/electron-aaos migration/story-6.1.2.6.md
...
Localização: .electron-aaos-core/utils/{utility-name}.js
Exemplo: .electron-aaos-core/utils/performance-monitor.js
