# 🏛️ Electron AAOS Dashboard - Arquitetura Completa
...
O Electron AAOS Dashboard é uma aplicação web Next.js que fornece uma interface visual para monitorar e gerenciar o sistema Electron AAOS. Ele se comunica com o CLI/Electron AAOS através de arquivos de status no filesystem e Server-Sent Events (SSE).

### Princípios Arquiteturais

1. **CLI-First**: Dashboard é complementar ao CLI, não substituto
2. **File-Based Communication**: Status via `.electron-aaos/dashboard/status.json`
...
│   │   │   ├── status/route.ts       # Electron AAOS status polling
...
│   │   ├── use-electron-aaos-status.ts        # Status with SWR
...
| ---------------- | ------------------------- | -------------------------------- |
| `story-store`    | `electron-aaos-stories`            | `storyOrder` (ordem das colunas) |
| `ui-store`       | `electron-aaos-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `electron-aaos-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `electron-aaos-dashboard-settings` | Todo o objeto `settings`         |
...
```typescript
// Retorna status atual do Electron AAOS
// Lê de: .electron-aaos/dashboard/status.json
...
└──────┬──────┘                              └────────┬─────────┘
       ...
       │  1. Agent ativado                            │
       │  ─────────────────────────────────────────▶  │
       │     Escreve .electron-aaos/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
_Documentação gerada por @architect (Aria) - Electron AAOS Core v2.0_
