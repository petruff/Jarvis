...
# Stack de Tecnologia Electron AAOS
...
Electron AAOS é construído em JavaScript/TypeScript moderno com runtime Node.js, otimizado para desenvolvimento de CLI cross-platform com UX interativa e capacidades de orquestração de agentes.
...
Electron AAOS integra com servidores Model Context Protocol (MCP):
...
```yaml
Baseline: 30MB (Node.js + Electron AAOS core)
...
```bash
# Configuração Electron AAOS
Electron AAOS_DEBUG=true                    # Habilitar logging de debug
Electron AAOS_CONFIG_PATH=/custom/path      # Localização customizada de config
Electron AAOS_YOLO_MODE=true               # Forçar modo yolo
...
| 1.1    | 2025-12-14 | Atualizado aviso de migração para Electron AAOSAI/electron-aaos-core, semantic-release para v25.0.2 [Story 6.10] | Dex (dev)        |

---

_Este é um padrão oficial do framework Electron AAOS. Todas as escolhas de tecnologia devem estar alinhadas com este stack._
