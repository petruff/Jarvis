...
Guia para integrar o Electron AAOS com IDEs e plataformas de desenvolvimento com IA suportadas.
...
O Electron AAOS suporta 9 plataformas de desenvolvimento com IA. Escolha a que melhor se adapta ao seu fluxo de trabalho.
...
**Nível de Recomendação:** Melhor integração com Electron AAOS
...
1. Electron AAOS cria automaticamente o diretório `.claude/` durante a inicialização
...
ls -la .claude/commands/Electron AAOS/agents/
```
...
1. Electron AAOS cria o diretório `.cursor/` durante a inicialização
...
1. Electron AAOS cria o diretório `.windsurf/` e o arquivo `.windsurfrules`
...
2. Electron AAOS cria o diretório `.cline/` durante a inicialização
...
2. Electron AAOS cria `.github/copilot-instructions.md`
...
1. Electron AAOS cria o diretório `.antigravity/`
...
O Electron AAOS mantém uma única fonte de verdade para definições de agentes e as sincroniza com todas as IDEs configuradas:

```
┌─────────────────────────────────────────────────────┐
│                    Electron AAOS Core                         │
│  .electron-aaos-core/development/agents/  (Source of Truth)  │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
...
O Electron AAOS pode ser configurado para sincronizar automaticamente quando houver mudanças nos agentes:

```yaml
# .electron-aaos-core/core/config/sync.yaml
auto_sync:
  enabled: true
  watch_paths:
...
ls .claude/commands/Electron AAOS/agents/  # Para Claude Code
```
...
├── Sim --> Claude Code (Melhor integração com Electron AAOS)
└── Não
...
# Verificar migração
diff -r ./rules-backup/ .claude/commands/Electron AAOS/agents/
```
...
_Guia de Integração com IDEs do Electron AAOS Electron AAOS v2.1.0_
