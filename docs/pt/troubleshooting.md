...
# Guia de Solução de Problemas do Electron AAOS Electron AAOS

> 🌐 [EN](../troubleshooting.md) | **PT** | [ES](../es/troubleshooting.md)

---

Este guia abrangente ajuda você a diagnosticar e resolver problemas comuns com o Electron AAOS Electron AAOS.
...
```bash
# Diagnóstico básico
npx @electron-aaos/electron-aaos-core doctor

# Corrigir automaticamente problemas comuns
npx @electron-aaos/electron-aaos-core doctor --fix

# Saída detalhada
npx @electron-aaos/electron-aaos-core doctor --verbose

# Verificar componente específico
npx @electron-aaos/electron-aaos-core doctor --component memory-layer
```
...
npm exec @electron-aaos/electron-aaos-core init my-project
```
...
npx @electron-aaos/electron-aaos-core init my-project --skip-install
...
ls -la .electron-aaos/config.json

# Validar configuração
npx @electron-aaos/electron-aaos-core doctor --component config

# Resetar se corrompido
rm .electron-aaos/config.json
npx @electron-aaos/electron-aaos-core doctor --fix
```
...
exit
npx @electron-aaos/electron-aaos-core
```
...
```bash
# Verificar config
cat .electron-aaos/memory-config.json
...
```javascript
// Editar .electron-aaos/memory-config.json
{
...
```javascript
// .electron-aaos/memory-config.json
{
  "embedModel": "local",
...
echo ".env" >> .gitignore
echo ".electron-aaos/logs/" >> .gitignore
```
...
cd C:\
npx @electron-aaos/electron-aaos-core init myapp
```
...
```
Error: "@electron-aaos/electron-aaos-core" cannot be opened
```
...
xattr -d com.apple.quarantine /usr/local/bin/@electron-aaos/electron-aaos-core
```
...
export DEBUG=electron-aaos:*
npx @electron-aaos/electron-aaos-core

# Componentes específicos
export DEBUG=electron-aaos:memory,electron-aaos:agent
```
...
```bash
# Ver logs recentes
tail -f .electron-aaos/logs/electron-aaos.log

# Buscar por erros
grep -i error .electron-aaos/logs/*.log
...
npx @electron-aaos/electron-aaos-core doctor --report diagnostic.json

# Incluir informações do sistema
npx @electron-aaos/electron-aaos-core info --detailed >> diagnostic.json

# Criar pacote de suporte
tar -czf electron-aaos-support.tar.gz .electron-aaos/logs diagnostic.json
```
...
// Habilitar profiling na config
{
  "debug": {
    "profiling": true,
    "profileOutput": ".electron-aaos/profiles/"
  }
}
```
...
```bash
# Analisar perfil
*debug analyze-profile .electron-aaos/profiles/latest.cpuprofile
```
...
1. **Execute diagnósticos:**
   ```bash
   npx @electron-aaos/electron-aaos-core doctor --verbose > diagnostic.log
   ```
...
2. **Colete informações:**
   ...
   - Versão do Electron AAOS: `npx @electron-aaos/electron-aaos-core version`

3. **Verifique issues existentes:**
   - [GitHub Issues](https://github.com/@electron-aaos/electron-aaos-core/@electron-aaos/electron-aaos-core/issues)
   - [Discussions](https://github.com/@electron-aaos/electron-aaos-core/@electron-aaos/electron-aaos-core/discussions)
...
- **Stack Overflow**: Marque perguntas com `@electron-aaos/electron-aaos-core`
...
1. Executar `npx @electron-aaos/electron-aaos-core init test`
...
cp -r .electron-aaos .electron-aaos.backup

# Reset completo
rm -rf .electron-aaos node_modules package-lock.json
npm cache clean --force
...
cp .electron-aaos.backup/memory.db .electron-aaos/
```

---

**Lembre-se**: A maioria dos problemas pode ser resolvida com:
1. `npx @electron-aaos/electron-aaos-core doctor --fix`
2. Limpando caches
