...
O Electron AAOS Electron AAOS pode ser instalado via NPX para uma configuração rápida sem instalação global. Este guia cobre o uso correto e a resolução de problemas para instalações baseadas em NPX.

## Início Rápido

### Uso Correto

Sempre execute `npx @electron-aaos/electron-aaos-core install` **a partir do diretório do seu projeto**:
...
O NPX executa pacotes em **diretórios temporários** (ex: `/private/var/folders/.../npx-xxx/` no macOS). Quando o Electron AAOS Electron AAOS é executado a partir desses locais temporários, ele não consegue:
...
A partir da versão 4.31.1, o Electron AAOS Electron AAOS detecta automaticamente quando está sendo executado a partir de um diretório temporário do NPX e exibe uma mensagem de erro útil:

```
⚠️  Diretório Temporário do NPX Detectado

O NPX executa em um diretório temporário, o que impede
o Electron AAOS de detectar sua IDE corretamente.

Solução:
  cd /path/to/your/project
  npx @electron-aaos/electron-aaos-core install

Veja: https://@electron-aaos/electron-aaos-core.dev/docs/npx-install
```
...
- Pastas de configuração da IDE (`.cursor`, `.windsurf`, `.claude`, etc.)
...
O Electron AAOS Electron AAOS detecta esses padrões e previne instalação incorreta.
...
2. Verifique se as pastas de configuração da IDE existem (`.cursor`, `.windsurf`, etc.)
...
O Electron AAOS Electron AAOS v4.31.1+ implementa detecção em duas camadas:

1. **Camada PRIMÁRIA** (`tools/electron-aaos-npx-wrapper.js`):
...
2. **Camada SECUNDÁRIA** (`tools/installer/bin/electron-aaos.js`):
...
- GitHub Issues: https://github.com/Electron AAOSAIinc/@electron-aaos/electron-aaos-core/issues
- Documentação: https://@electron-aaos/electron-aaos-core.dev/docs
