# Guia Electron AAOS para Gemini CLI
...
### Por que usar Electron AAOS com Gemini CLI?
...
```markdown
# Regras Electron AAOS Electron AAOS para Gemini CLI
...
## Ativação
Inclua "Como agente Electron AAOS dev" no seu prompt.
...
```bash
# Agente desenvolvedor
gemini "Como agente Electron AAOS dev, implemente a feature de login"

# Agente QA
gemini "Como agente Electron AAOS qa, revise este código para segurança"

# Agente arquiteto
gemini "Como agente Electron AAOS architect, projete o schema do banco de dados"
```
...
cat src/auth.ts | gemini "Como Electron AAOS qa, revise este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como Electron AAOS qa, revise este arquivo" >> reviews.md
done
```
...
**Frase de Ativação:** "Como agente Electron AAOS dev"
...
1. Instale Electron AAOS para Gemini CLI:
   ```bash
...
1. Agentes Electron AAOS em `.electron-aaos-core/` são portáveis
...
*Electron AAOS Electron AAOS - Guia da Plataforma Gemini CLI v1.0*
