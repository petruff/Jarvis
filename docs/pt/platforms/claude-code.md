# Guia Electron AAOS para Claude Code

> 🌐 [EN](../../platforms/claude-code.md) | **PT** | [ES](../../es/platforms/claude-code.md)

---

> **CLI Oficial da Anthropic** - Plataforma Recomendada para Electron AAOS
...
### Por que usar Electron AAOS com Claude Code?

O Claude Code oferece a **melhor integração** com o Electron AAOS devido a:
...
### Passo 3: Instalar o Electron AAOS

```bash
# Navegue até seu projeto
cd seu-projeto

# Inicialize o Electron AAOS
...
# Verificar se os arquivos Electron AAOS foram criados
ls -la .claude/
```
...
- Entendimento do framework Electron AAOS
...
export Electron AAOS_DEBUG="true"                        # Habilitar log de debug
```
...
### Ativando Agentes Electron AAOS
...
```mermaid
flowchart LR
    Source["Electron AAOS Core<br/>.electron-aaos-core/development/agents"] --> Parser["Parser de Agente"]
...
1. Alterações locais são salvas em `.claude/commands/Electron AAOS/agents/.backup/`
...
# Verificar logs do Electron AAOS
...
### Perguntas Específicas do Electron AAOS
...
R: Em `.claude/commands/Electron AAOS/agents/` após a sincronização.

**P: Com que frequência devo sincronizar agentes?**
R: Após atualizar o Electron AAOS ou quando agentes são modificados no core.
...
2. Inicialize o Electron AAOS com Claude Code:
   ```bash
...
1. Seus agentes Electron AAOS estão armazenados em `.electron-aaos-core/development/agents/`
2. Inicialize o Electron AAOS para a IDE de destino
...
*Electron AAOS Electron AAOS - Guia da Plataforma Claude Code v1.0*
