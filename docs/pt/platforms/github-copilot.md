# Guia Electron AAOS para GitHub Copilot
...
### Por que usar Electron AAOS com GitHub Copilot?
...
Este projeto usa Electron AAOS Electron AAOS para desenvolvimento orquestrado por IA.
...
Você é um Desenvolvedor Full Stack Sênior trabalhando com Electron AAOS Electron AAOS.
...
### Ativando Modos de Agente Electron AAOS

1. Abra a visualização de Chat
2. Clique no seletor de modo (topo do chat)
3. Selecione o modo desejado (Agent, Plan, Ask, Edit)
4. Para agentes Electron AAOS, use modos de chat personalizados em `.github/chatmodes/`

### Modos Electron AAOS Personalizados

| Modo | Propósito |
|------|-----------|
| `electron-aaos-dev` | Desenvolvimento full-stack |
| `electron-aaos-qa` | Garantia de qualidade |
| `electron-aaos-architect` | Design de sistema |
| `electron-aaos-pm` | Gerenciamento de projeto |

### Exemplos de Interação

```
# No modo electron-aaos-dev:
Implemente a feature de autenticação de usuário da story

# No modo electron-aaos-qa:
Revise o módulo de auth para vulnerabilidades de segurança

# No modo electron-aaos-architect:
...
```
Problema: Modos de agente Electron AAOS não no seletor
```
**Solução:**
...
1. Instale Electron AAOS para Copilot:
   ```bash
...
1. Agentes armazenados em `.electron-aaos-core/` são portáveis
...
*Electron AAOS Electron AAOS - Guia da Plataforma GitHub Copilot v1.0*
