...
Guia de referência completo para todos os comandos do meta-agente Electron AAOS Electron AAOS.
...
Atualiza o Electron AAOS Electron AAOS.
...
```bash
Electron AAOS_AI_PROVIDER=openai          # Provedor de IA
Electron AAOS_AI_MODEL=gpt-4             # Modelo de IA
Electron AAOS_LOG_LEVEL=debug            # Nível de log
Electron AAOS_TELEMETRY=disabled         # Configuração de telemetria
Electron AAOS_TIMEOUT=30000             # Timeout de comando (ms)
Electron AAOS_MEMORY_CACHE=true         # Cache de memória
```
