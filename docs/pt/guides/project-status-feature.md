...
A funcionalidade de Status do Projeto exibe automaticamente seu contexto de trabalho atual quando qualquer agente Electron AAOS é ativado. Isso inclui:
...
- **Framework Electron AAOS-FullStack** instalado
...
3. Criar arquivo de cache `.electron-aaos/project-status.yaml`
...
1. Edite `.electron-aaos-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
2. Crie o diretório `.electron-aaos/`:
   ```bash
   mkdir .electron-aaos
   ```
...
Localização: `.electron-aaos-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .electron-aaos/project-status.yaml  # Localização do arquivo de cache
...
1. **Verifica cache** - Procura por `.electron-aaos/project-status.yaml`
...
- **Localização do Cache:** `.electron-aaos/project-status.yaml`
...
Todos os 11 agentes Electron AAOS exibem status do projeto:
...
11. **@electron-aaos-master** (Orion - Orchestrator)
...
3. O arquivo `.electron-aaos-core/infrastructure/scripts/project-status-loader.js` existe?
...
rm .electron-aaos/project-status.yaml
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.electron-aaos-core/infrastructure/scripts/project-status-loader.js');
...
const { clearCache } = require('./.electron-aaos-core/infrastructure/scripts/project-status-loader.js');
await clearCache();
```
...
```bash
# Remove script
rm .electron-aaos-core/infrastructure/scripts/project-status-loader.js

# Remove task
rm .electron-aaos-core/tasks/init-project-status.md

# Remove cache
rm .electron-aaos/project-status.yaml

# Remove tests
rm .electron-aaos-core/infrastructure/scripts/__tests__/project-status-loader.test.js
...
R: Sim, todos os agentes usam o mesmo arquivo de cache (`.electron-aaos/project-status.yaml`).

---

## Documentação Relacionada

- **Story:** `docs/stories/electron-aaos migration/story-6.1.2.4-project-status-context.md`
- **Config:** `.electron-aaos-core/core-config.yaml` (seção projectStatus)
- **Script:** `.electron-aaos-core/infrastructure/scripts/project-status-loader.js`
- **Task de Init:** `.electron-aaos-core/tasks/init-project-status.md`
- **Testes:** `.electron-aaos-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
