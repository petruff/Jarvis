...
# Guia de Configuração Global MCP do Electron AAOS

> 🌐 [EN](../../guides/mcp-global-setup.md) | **PT** | [ES](../../es/guides/mcp-global-setup.md)

---

> Configure servidores MCP (Model Context Protocol) globais para Electron AAOS Electron AAOS.
...
O Sistema Global MCP permite que você configure servidores MCP uma única vez e os compartilhe entre todos os projetos Electron AAOS. Isso elimina a necessidade de configurar os mesmos servidores em cada projeto.
...
- `~/.electron-aaos/` - Diretório global do Electron AAOS
- `~/.electron-aaos/mcp/` - Diretório de configuração MCP
- `~/.electron-aaos/mcp/global-config.json` - Arquivo de configuração principal
- `~/.electron-aaos/mcp/servers/` - Configurações individuais de servidores
- `~/.electron-aaos/mcp/cache/` - Cache de respostas
- `~/.electron-aaos/credentials/` - Armazenamento seguro de credenciais
...
O Electron AAOS inclui templates para servidores MCP populares:
...
Credenciais são armazenadas em `~/.electron-aaos/credentials/` com um `.gitignore` para prevenir commits acidentais.
...
| Diretório já existe    | Use `electron-aaos mcp setup --force` para recriar                                 |
...
_Electron AAOS Electron AAOS v2.1 Guia de Configuração Global MCP_
