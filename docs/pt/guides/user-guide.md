...
# Guia do Usuário Electron AAOS

> **PT-BR**

---

Guia completo para usar o Electron AAOS Electron AAOS - o Sistema Orquestrado por IA para Desenvolvimento Full Stack.
...
Antes de usar o Electron AAOS, certifique-se de ter:
...
O Electron AAOS fornece estrutura orquestrada enquanto permite flexibilidade na comunicação. Isso significa:
...
### A Diferença do Electron AAOS

| Desenvolvimento Tradicional com IA | Electron AAOS                                        |
| ---------------------------------- | ------------------------------------------- |
...
O Electron AAOS inclui 11 agentes especializados, cada um com papel e personalidade distintos:
...
| **Pax**   | `@electron-aaos-master`   | Orquestrador | Orquestração do framework |
...
@electron-aaos-master        # Ativar Pax (Orquestrador)
...
Tasks são o ponto de entrada principal no Electron AAOS. Tudo é uma task.
...
| `brownfield-integration` | Adicionar Electron AAOS a existente | dev, architect     |
...
Squads são equipes modulares de agentes de IA que estendem a funcionalidade do Electron AAOS.
...
Nível 2: Electron AAOS-SQUADS  --> github.com/Electron AAOSAI (Público/Gratuito)
...
```
my-project/
├── .electron-aaos-core/                # Configuração do framework
...
```bash
# Comandos do Electron AAOS Master
*help                # Mostrar comandos disponíveis
...
A configuração principal está em `.electron-aaos-core/core/config/`:
...
O Electron AAOS suporta múltiplas IDEs. A configuração é sincronizada entre:
...
O Electron AAOS implementa quality gates em 3 camadas:
...
_Guia do Usuário Electron AAOS Electron AAOS v2.1.0_
