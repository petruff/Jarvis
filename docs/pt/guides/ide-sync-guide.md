...
Sincronize agentes, tasks, workflows e checklists do Electron AAOS entre múltiplas configurações de IDE.

## Visão Geral

A task `*command` automatiza a sincronização de componentes do Electron AAOS para todos os diretórios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operações manuais de cópia.
...
```bash
cp .electron-aaos-core/infrastructure/templates/electron-aaos-sync.yaml.template .electron-aaos-sync.yaml
```

### 2. Configurar IDEs

Edite `.electron-aaos-sync.yaml` para habilitar suas IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Ler configuração .electron-aaos-sync.yaml                 │
...
```
seu-projeto/
├── .electron-aaos-sync.yaml           # Configuração de sincronização
...
### Alias de Pack Ausente

```
Aviso: Nenhum alias de pack para 'new-squad'
```

**Solução**: Adicione o alias em `.electron-aaos-sync.yaml`:
...
- [Arquitetura Electron AAOS](../core-architecture.md)
