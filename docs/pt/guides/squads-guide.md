...
Guia completo para criar, validar, publicar e gerenciar Squads no Electron AAOS.

> **Electron AAOS Squads:** Equipes de AI agents trabalhando com você
...
Squads são equipes modulares de agentes de IA que estendem a funcionalidade do Electron AAOS. Cada squad é um pacote autocontido contendo:
...
│  Nível 2: Electron AAOS-SQUADS  → github.com/Electron AAOSAI (Público/Free) │
│  Nível 3: SYNKRA API   → api.electron-aaos.dev      (Marketplace)  │
└─────────────────────────────────────────────────────────────┘
```
...
| -------------------------------------------------------------------------- | ------ | ---------------------------------- |
| [etl-squad](https://github.com/Electron AAOSAI/electron-aaos-squads/tree/main/etl)         | 2.0.0  | Coleta e transformação de dados    |
| [creator-squad](https://github.com/Electron AAOSAI/electron-aaos-squads/tree/main/creator) | 1.0.0  | Utilitários de geração de conteúdo |
...
- Node.js 18+
- Projeto Electron AAOS inicializado (`.electron-aaos-core/` existe)
...
Tasks devem seguir [TASK-FORMAT-SPECIFICATION-V1](../../../.electron-aaos-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Modo       | Comportamento                                   |
| ---------- | ----------------------------------------------- |
| `extend`   | Adiciona regras do squad às regras core do Electron AAOS |
...
```javascript
const { SquadAnalyzer } = require('./.electron-aaos-core/development/scripts/squad/squad-analyzer');
const { SquadExtender } = require('./.electron-aaos-core/development/scripts/squad/squad-extender');
...
```javascript
const { SquadValidator } = require('./.electron-aaos-core/development/scripts/squad');
...
### Nível 2: Repositório electron-aaos-squads (Público)
...
Isso cria um PR para [Electron AAOSAI/electron-aaos-squads](https://github.com/Electron AAOSAI/electron-aaos-squads).

### Nível 3: Electron AAOS Marketplace
...
*sync-squad-electron-aaos ./squads/my-squad --public
```
...
- Campo `electron-aaos.type`
- Campo `electron-aaos.minVersion`
...
3. **Adicionar Campos** - `electron-aaos.type`, `electron-aaos.minVersion`
...
```
1. Local     → ./squads/{name}/
2. npm       → node_modules/@electron-aaos-squads/{name}/
3. Workspace → ../{name}/ (monorepo)
4. Registry  → api.electron-aaos.dev/squads/{name}
```

### Uso Programático

```javascript
const { SquadLoader } = require('./.electron-aaos-core/development/scripts/squad');
...
```javascript
const { SquadLoader, SquadLoaderError } = require('./.electron-aaos-core/development/scripts/squad');
...
# - electron-aaos.type: deve ser "squad"
# - electron-aaos.minVersion: deve ser semver válido
```
...
**Squads** são o novo padrão (Electron AAOS 2.1+) substituindo Expansion Packs. Squads têm:
...
3. Re-publicar: `*publish-squad` ou `*sync-squad-electron-aaos`
...
- **Nível 3**: Sincronize com flag `--private`: `*sync-squad-electron-aaos my-squad --private`

### Qual a versão mínima do Electron AAOS para Squads?

Squads requerem Electron AAOS 2.1.0+. Defina no manifesto:
...
- [TASK-FORMAT-SPECIFICATION-V1](../../../.electron-aaos-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md)
...
- [Agente @squad-creator](../../../.electron-aaos-core/development/agents/squad-creator.md)
- [Repositório electron-aaos-squads](https://github.com/Electron AAOSAI/electron-aaos-squads)

---

## Obtendo Ajuda

- [GitHub Discussions](https://github.com/Electron AAOSAI/electron-aaos-core/discussions)
- [Issue Tracker](https://github.com/Electron AAOSAI/electron-aaos-core/issues)

---

_Electron AAOS Squads: Equipes de AI agents trabalhando com você_
