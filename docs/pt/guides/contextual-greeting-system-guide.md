...
O Sistema de Greeting Contextual é uma melhoria de UX que torna os greetings dos agentes Electron AAOS inteligentes e adaptativos, mostrando informações e comandos relevantes baseados no contexto da sessão.
...
### Scripts Core
- `.electron-aaos-core/core/session/context-detector.js` - Detecção de tipo de sessão
- `.electron-aaos-core/infrastructure/scripts/git-config-detector.js` - Detecção de git config
- `.electron-aaos-core/development/scripts/greeting-builder.js` - Montagem do greeting
- `.electron-aaos-core/development/scripts/workflow-navigator.js` - Navegação de workflow
- `.electron-aaos-core/development/scripts/agent-exit-hooks.js` - Hooks de saída (para persistência)

### Arquivos de Dados
- `.electron-aaos-core/data/workflow-patterns.yaml` - Definições de workflows
...
### Configuração
- `.electron-aaos-core/core-config.yaml` - Configuração global (seções git + agentIdentity)

### Agentes (Atualizados)
- `.electron-aaos-core/agents/dev.md` - Metadata de visibilidade de comandos
- `.electron-aaos-core/agents/po.md` - Metadata de visibilidade de comandos
- `.electron-aaos-core/agents/*.md` - 9 agentes restantes (atualização pendente)
...
R: Use o script de teste: `node .electron-aaos-core/development/scripts/test-greeting-system.js`
