...
> Controle o nível de autonomia que os agentes Electron AAOS têm sobre seu sistema.

---

## Visão Geral

Os Modos de Permissão permitem controlar o nível de autonomia dos agentes Electron AAOS. Seja explorando um novo codebase ou executando builds totalmente autônomos, há um modo para seu workflow.
...
O modo é persistido em `.electron-aaos/config.yaml`:
...
Verifique se `.electron-aaos/config.yaml` existe e é gravável:
