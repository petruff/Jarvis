...
# Começando com o Electron AAOS Electron AAOS

> 🌐 [EN](../getting-started.md) | **PT** | [ES](../es/getting-started.md)

---

Bem-vindo ao Electron AAOS Electron AAOS! Este guia irá orientá-lo nos seus primeiros passos com o framework de desenvolvimento auto-modificável alimentado por IA.
...
Antes de instalar o Electron AAOS Electron AAOS, certifique-se de ter:
...
npx @electron-aaos/electron-aaos-core init my-first-project
...
O Electron AAOS Electron AAOS oferece opções flexíveis de instalação:

```bash
# 1. Criar novo projeto com template personalizado
npx @electron-aaos/electron-aaos-core init my-project --template enterprise
...
npx @electron-aaos/electron-aaos-core init my-project --force
...
```
my-first-project/
├── .electron-aaos/                    # Configuração do framework
...
O arquivo principal de configuração (`.electron-aaos/config.json`) contém:
...
Electron AAOS_TELEMETRY=enabled
```

## Entendendo o Meta-Agent

O meta-agent é o núcleo do Electron AAOS Electron AAOS - uma IA que entende e pode modificar sua própria base de código.
...
- **[Tutoriais em Vídeo](https://@electron-aaos/electron-aaos-core.dev/tutorials)**: Aprendizado visual
...
- **Twitter**: Siga [@electron-aaosfullstack](https://twitter.com/electron-aaosfullstack)
...
Lembre-se: O Electron AAOS Electron AAOS aprende e melhora com o uso. Quanto mais você interagir com ele, melhor ele entenderá seu estilo e necessidades de desenvolvimento.

Bom código com o Electron AAOS Electron AAOS!
