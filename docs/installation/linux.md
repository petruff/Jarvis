# Linux Installation Guide for Electron AAOS Electron AAOS
...
git clone https://github.com/Electron AAOSAI/electron-aaos-core.git ~/.electron-aaos-core-source

# Navigate to the source
cd ~/.electron-aaos-core-source

# Install dependencies
npm install

# Run installer for your project
node bin/electron-aaos-init.js ~/projects/my-project
```
...
2. Commands are installed to `.claude/commands/Electron AAOS/`
3. Use `/agent-name` to activate agents

### Windsurf

1. Install from [codeium.com/windsurf](https://codeium.com/windsurf)
2. Rules are installed to `.windsurf/rules/`
3. Use `@agent-name` to activate agents

### VS Code (with Continue extension)

1. Install Continue extension
2. Configure Electron AAOS rules in `.continue/`
...
chmod -R u+rwX .electron-aaos-core
...
export Electron AAOS_HOME="$HOME/.electron-aaos-core"
export PATH="$Electron AAOS_HOME/bin:$PATH"
...
Electron AAOS Electron AAOS respects XDG directories:

```bash
# Data files: ~/.local/share/electron-aaos/
# Config files: ~/.config/electron-aaos/
# Cache: ~/.cache/electron-aaos/
# State: ~/.local/state/electron-aaos/
```
...
git pull
npm install
node bin/electron-aaos-init.js ~/projects/my-project --update
```

The updater will:

- Detect your existing installation
- Back up any customizations to `.electron-aaos-backup/`
...
```bash
# Remove Electron AAOS from a project
rm -rf .electron-aaos-core .claude/commands/Electron AAOS

# Remove global installation
rm -rf ~/.electron-aaos-core-source ~/.npm-global/lib/node_modules/@electron-aaos
```
