...
| `bin/electron-aaos-init.js`                               | **Modified** | Added `requireSignature` option          |
...
1. Generate a key pair: `minisign -G -p electron-aaos-core.pub -s electron-aaos-core.key`
2. Update `PINNED_PUBLIC_KEY` in `manifest-signature.js` with the real public key
3. Sign the manifest: `minisign -Sm install-manifest.yaml -s electron-aaos-core.key`
...
3. **Sign manifest** before each release:
   ...
   ```bash
   minisign -Sm .electron-aaos-core/install-manifest.yaml -s electron-aaos-core.key
   ```
