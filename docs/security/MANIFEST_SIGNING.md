...
This document explains how to set up and use the cryptographic signing system for Electron AAOS-Core install manifests.

## Overview

Electron AAOS-Core uses **Ed25519 digital signatures** (via minisign format) to verify the integrity and authenticity of the `install-manifest.yaml` file. This ensures that:
...
│  │ (electron-aaos-core.key)  │    │         -s electron-aaos-core.key          │  │
│  │ NEVER SHARE!     │    └───────────────────────────────────┘  │
│  └──────────────────┘                    │                       │
│                                          ▼                       │
│                          ┌───────────────────────────────────┐  │
...
minisign -G -p electron-aaos-core.pub -s electron-aaos-core.key

# You will be prompted for a password to protect the secret key
# CHOOSE A STRONG PASSWORD!

# Output:
#   electron-aaos-core.pub  - PUBLIC key (safe to share, will be hardcoded)
#   electron-aaos-core.key  - SECRET key (NEVER share, store securely)
```

### 3. View Public Key

```bash
cat electron-aaos-core.pub
# Output example:
# untrusted comment: minisign public key Electron AAOS0001
...
const PINNED_PUBLIC_KEY = {
  ...
  keyId: 'Electron AAOS0001',
  // Base64 public key (the second line of electron-aaos-core.pub)
...
- Store `electron-aaos-core.key` in a secure location (password manager, HSM, etc.)
- NEVER commit it to git
- Add to `.gitignore`:
  ```gitignore
  *.key
  electron-aaos-core.key
  ```
...
1. **Generate/Update Manifest**
   ...
   ```bash
   node bin/electron-aaos.js manifest:generate
   # Creates .electron-aaos-core/install-manifest.yaml with all file hashes
   ```
...
2. **Sign the Manifest**
   ...
   minisign -Sm install-manifest.yaml -s /path/to/electron-aaos-core.key
...
3. **Verify Signature (Optional but Recommended)**
   ...
   ```bash
   minisign -Vm install-manifest.yaml -p /path/to/electron-aaos-core.pub
...
4. **Commit Both Files**
   ...
   ```bash
   git add .electron-aaos-core/install-manifest.yaml
   git add .electron-aaos-core/install-manifest.yaml.minisig
...
```bash
minisign -Sm .electron-aaos-core/install-manifest.yaml -s /path/to/electron-aaos-core.key
```
