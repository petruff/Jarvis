...
async function runWizard(options = {}) {
  ...
  if (!fs.existsSync(wizardPath)) {
    ...
    const legacyScript = path.join(__dirname, 'electron-aaos-init.js');
    if (fs.existsSync(legacyScript)) {
      if (!options.quiet) {
        console.log('⚠️  Using legacy wizard (src/wizard not found)');
      }
      // Legacy wizard doesn't support options, pass via env vars
      process.env.Electron AAOS_INSTALL_FORCE = options.force ? '1' : '';
      process.env.Electron AAOS_INSTALL_QUIET = options.quiet ? '1' : '';
      process.env.Electron AAOS_INSTALL_DRY_RUN = options.dryRun ? '1' : '';
      require(legacyScript);
      return;
    }
    console.error('❌ Initialization wizard not found');
    console.error('Please ensure Electron AAOS-FullStack is installed correctly.');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
Electron AAOS-FullStack v${packageJson.version}
AI-Orchestrated System for Full Stack Development

USAGE:
  npx @electron-aaos/electron-aaos-core@latest              # Run installation wizard
  npx @electron-aaos/electron-aaos-core@latest install      # Install in current project
  npx @electron-aaos/electron-aaos-core@latest init <name>  # Create new project
  npx @electron-aaos/electron-aaos-core@latest update       # Update to latest version
  npx @electron-aaos/electron-aaos-core@latest validate     # Validate installation integrity
  ...
  npx @electron-aaos/electron-aaos-core@latest --version    # Show version
  npx @electron-aaos/electron-aaos-core@latest --version -d # Show detailed version info
  npx @electron-aaos/electron-aaos-core@latest --help       # Show this help

UPDATE:
  electron-aaos update                    # Update to latest version
  electron-aaos update --check            # Check for updates without applying
  electron-aaos update --dry-run          # Preview what would be updated
  electron-aaos update --force            # Force update even if up-to-date
  electron-aaos update --verbose          # Show detailed output

VALIDATION:
  electron-aaos validate                    # Validate installation integrity
  electron-aaos validate --repair           # Repair missing/corrupted files
  electron-aaos validate --repair --dry-run # Preview repairs
  electron-aaos validate --detailed         # Show detailed file list

CONFIGURATION:
  electron-aaos config show                       # Show resolved configuration
  electron-aaos config show --debug               # Show with source annotations
  electron-aaos config diff --levels L1,L2        # Compare config levels
  electron-aaos config migrate                    # Migrate monolithic to layered
  electron-aaos config validate                   # Validate config files
  electron-aaos config init-local                 # Create local-config.yaml

SERVICE DISCOVERY:
  electron-aaos workers search <query>            # Search for workers
  electron-aaos workers search "json" --category=data
  electron-aaos workers search "transform" --tags=etl,data
  electron-aaos workers search "api" --format=json
...
EXAMPLES:
  ...
  npx @electron-aaos/electron-aaos-core-minimal@latest

  # Create new project
  npx @electron-aaos/electron-aaos-core@latest init my-project

  # Search for workers
  electron-aaos workers search "json csv"

For more information, visit: https://github.com/Electron AAOSAI/electron-aaos-core
`);
}
...
async function showVersion() {
  ...
  console.log(`Electron AAOS-FullStack v${packageJson.version}`);
  console.log('Package: @electron-aaos/electron-aaos-core');

  // Check for local installation
  const localVersionPath = path.join(process.cwd(), '.electron-aaos-core', 'version.json');
  ...
  if (fs.existsSync(localVersionPath)) {
    try {
      ...
      if (versionInfo.version !== packageJson.version) {
        ...
        console.log('  Run \'npx electron-aaos-core update\' to update.');
      } else {
        console.log('\n✅ Up to date');
      }
    } catch (error) {
    ...
    console.log('  Run \'npx electron-aaos-core install\' to install Electron AAOS in this project.');
  }
}

// Helper: Show system info
function showInfo() {
  console.log('📊 Electron AAOS-FullStack System Information\n');
  ...
  // Check if .electron-aaos-core exists
  const electron-aaosCoreDir = path.join(__dirname, '..', '.electron-aaos-core');
  if (fs.existsSync(electron-aaosCoreDir)) {
    console.log('\n✓ Electron AAOS Core installed');
    ...
    console.log(`  - Agents: ${countFiles(path.join(electron-aaosCoreDir, 'agents'))}`);
    console.log(`  - Tasks: ${countFiles(path.join(electron-aaosCoreDir, 'tasks'))}`);
    console.log(`  - Templates: ${countFiles(path.join(electron-aaosCoreDir, 'templates'))}`);
    console.log(`  - Workflows: ${countFiles(path.join(electron-aaosCoreDir, 'workflows'))}`);
  } else {
    console.log('\n⚠️  Electron AAOS Core not found');
  }

  // Check Electron AAOS Pro status (Task 5.1)
  const proDir = path.join(__dirname, '..', 'pro');
  if (fs.existsSync(proDir)) {
    console.log('\n✓ Electron AAOS Pro installed');
...
async function runValidate() {
  ...
  try {
    ...
    const { createValidateCommand } = require('../.electron-aaos-core/cli/commands/validate/index.js');
    ...
    await validateCmd.parseAsync(['node', 'electron-aaos', ...validateArgs]);
  } catch (_error) {
...
async function runUpdate() {
  ...
  try {
    ...
    const { Electron AAOSUpdater, formatCheckResult, formatUpdateResult } = require(updaterPath);

    const updater = new Electron AAOSUpdater(process.cwd(), {
      verbose: isVerbose,
      force: isForce,
    });
    ...
    if (isCheck) {
      ...
      // Update mode
      console.log('🔄 Electron AAOS Update\n');
...
async function runDoctor() {
  ...
  if (showHelp) {
    console.log(`
Usage: electron-aaos-core doctor [options]
...
Examples:
  $ npx electron-aaos-core doctor              # Run diagnostics
  $ npx electron-aaos-core doctor --fix        # Fix detected issues
  $ npx electron-aaos-core doctor --dry-run    # Preview fixes
`);
    return;
  }

  console.log('🏥 Electron AAOS System Diagnostics\n');
  ...
  // Check 4: Electron AAOS installation
  const electron-aaosCoreDir = path.join(__dirname, '..', '.electron-aaos-core');
  if (fs.existsSync(electron-aaosCoreDir)) {
    console.log(`✔ Electron AAOS Electron AAOS: v${packageJson.version}`);
    ...
    try {
      ...
      if (report.stats && (report.stats.missingFiles > 0 || report.stats.corruptedFiles > 0)) {
        issues.push({
          type: 'electron-aaos_corrupted',
          autoFix: true,
          message: `Electron AAOS Core: ${report.stats.missingFiles} missing, ${report.stats.corruptedFiles} corrupted files`,
          fixAction: async () => {
            console.log('  🔧 Repairing Electron AAOS installation...');
            await validator.repair();
            console.log('  ✓ Repair complete');
          },
        });
        hasErrors = true;
        console.log(`⚠️  Electron AAOS Core: ${report.stats.missingFiles} missing, ${report.stats.corruptedFiles} corrupted files`);
      }
    } catch {
    ...
    issues.push({
      type: 'electron-aaos_missing',
      autoFix: true,
      message: 'Electron AAOS Core not installed',
      fixAction: async () => {
        console.log('  🔧 Installing Electron AAOS...');
        try {
          execSync('npx electron-aaos-core install --force --quiet', { stdio: 'inherit', timeout: 60000 });
    ...
    console.log('  Run: npx @electron-aaos/electron-aaos-core@latest');
  }

  // Check 5: Electron AAOS Pro license status (Task 5.1)
  const proDir = path.join(__dirname, '..', 'pro');
  if (fs.existsSync(proDir)) {
    try {
      const { featureGate } = require(path.join(proDir, 'license', 'feature-gate'));
      ...
      if (state === 'Active') {
        console.log(`${stateEmoji[state]} Electron AAOS Pro: License active`);
      } else if (state === 'Grace') {
        console.log(`${stateEmoji[state]} Electron AAOS Pro: License in grace period`);
        console.log('  Run: electron-aaos pro validate');
      } else if (state === 'Expired') {
        console.log(`${stateEmoji[state]} Electron AAOS Pro: License expired`);
        console.log('  Run: electron-aaos pro activate --key <KEY>');
      } else {
        console.log(`${stateEmoji[state]} Electron AAOS Pro: Not activated`);
        console.log('  Run: electron-aaos pro activate --key <KEY>');
      }
    } catch {
      console.log('⚠️  Electron AAOS Pro: Unable to check license status');
    }
  }
...
// Helper: Remove Electron AAOS sections from .gitignore
function cleanGitignore(gitignorePath) {
  ...
  for (const line of lines) {
    if (line.includes('# Electron AAOS') || line.includes('# Added by Electron AAOS')) {
      inAiosSection = true;
      removedLines++;
      continue;
    }
...
function showUninstallHelp() {
  console.log(`
Usage: npx electron-aaos-core uninstall [options]

Remove Electron AAOS from the current project.

Options:
  --force      Skip confirmation prompt
  --keep-data  Keep .electron-aaos/ directory (settings and history)
  --dry-run    Show what would be removed without removing
  -h, --help   Show this help message

What gets removed:
  - .electron-aaos-core/     Framework core files
  - docs/stories/   Story files (if created by Electron AAOS)
  - squads/         Squad expansion packs
  - .gitignore      Electron AAOS-added entries only

What is preserved (with --keep-data):
  - .electron-aaos/          Project settings and agent history
...
Examples:
  ...
  npx electron-aaos-core uninstall

  # Force uninstall without prompts
  npx electron-aaos-core uninstall --force

  # See what would be removed
  npx electron-aaos-core uninstall --dry-run

  # Uninstall but keep project data
  npx electron-aaos-core uninstall --keep-data
`);
}
...
function showDoctorHelp() {
  console.log(`
Usage: npx electron-aaos-core doctor [options]

Run health checks on your Electron AAOS installation.
...
Checks performed:
  • Required directories exist (.electron-aaos-core/, .electron-aaos/)
...
Examples:
  # Run health check
  npx electron-aaos-core doctor

  # Auto-fix detected issues
  npx electron-aaos-core doctor --fix

  # Preview what would be fixed
  npx electron-aaos-core doctor --fix --dry-run
`);
}

// Uninstall Electron AAOS from project
async function runUninstall(options = {}) {
  ...
  // Items to remove
  const itemsToRemove = [
    { path: '.electron-aaos-core', description: 'Framework core' },
    { path: 'squads', description: 'Squad expansion packs' },
  ];

  // Optionally remove .electron-aaos
  if (!keepData) {
    itemsToRemove.push({ path: '.electron-aaos', description: 'Project data and settings' });
  }
  ...
  if (existingItems.length === 0) {
    console.log('ℹ️  No Electron AAOS installation found in this directory.');
    return;
  }
  ...
  if (!quiet) {
    ...
    if (fs.existsSync(gitignorePath)) {
      const content = fs.readFileSync(gitignorePath, 'utf8');
      if (content.includes('# Electron AAOS') || content.includes('# Added by Electron AAOS')) {
        console.log('  • .gitignore Electron AAOS entries will be cleaned');
      }
    }
    console.log('');
  }
  ...
  if (!force) {
    ...
    const answer = await new Promise(resolve => {
      rl.question('⚠️  Are you sure you want to uninstall Electron AAOS? (y/N): ', resolve);
    });
    rl.close();
  ...
  // Perform removal
  if (!quiet) console.log('\n🗑️  Removing Electron AAOS components...\n');
  ...
  if (gitignoreResult.removed && !quiet) {
    console.log(`  ✓ Cleaned ${gitignoreResult.lines} Electron AAOS entries from .gitignore`);
  }

  // Summary
  if (!quiet) {
    console.log('\n✅ Electron AAOS has been uninstalled.');
    if (keepData) {
      console.log('   Your project data in .electron-aaos/ has been preserved.');
    }
    console.log('\n   To reinstall: npx electron-aaos-core install');
  }
}
...
function showInstallHelp() {
  console.log(`
Usage: npx electron-aaos-core install [options]

Install Electron AAOS in the current directory.

Options:
  --force      Overwrite existing Electron AAOS installation
...
Smart Merge (Brownfield):
  ...
  Electron AAOS can merge new settings while preserving your customizations.

  - .env files: Adds new variables, preserves existing values
  - CLAUDE.md: Updates Electron AAOS sections, keeps your custom rules
...
Examples:
  ...
  npx electron-aaos-core install

  # Force reinstall without prompts
  npx electron-aaos-core install --force

  # Brownfield: merge configs automatically
  npx electron-aaos-core install --merge

  # Silent install for CI/CD
  npx electron-aaos-core install --quiet --force

  # Preview what would be installed
  npx electron-aaos-core install --dry-run
`);
}
...
function showInitHelp() {
  console.log(`
Usage: npx electron-aaos-core init <project-name> [options]

Create a new Electron AAOS project with the specified name.
...
Examples:
  npx electron-aaos-core init my-project
  npx electron-aaos-core init my-project --template minimal
  npx electron-aaos-core init my-project --force --skip-install
  npx electron-aaos-core init . --template enterprise
`);
}
...
async function initProject() {
  ...
  console.log(`Creating new Electron AAOS project: ${displayName}`);
  if (template !== 'default') {
...
async function main() {
  switch (command) {
    case 'workers':
      ...
      try {
        const { run } = require('../.electron-aaos-core/cli/index.js');
    ...
    case 'pro':
      // Electron AAOS Pro License Management - Story PRO-6
      try {
        const { run } = require('../.electron-aaos-core/cli/index.js');
        await run(process.argv);
      } catch (error) {
    ...
    case 'install': {
      ...
      if (!installOptions.quiet) {
        console.log('Electron AAOS-FullStack Installation\n');
      }
