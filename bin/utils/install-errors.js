 ...
 * for Electron AAOS installer failures.
 *
...
const ERROR_MESSAGES = {
  ...
  UNKNOWN_ERROR: {
    ...
    recovery: [
      'Check the installation log: .electron-aaos-install.log',
...
function formatErrorMessage(error, errorCode = null) {
  ...
  lines.push('');
  lines.push(chalk.gray('Installation log: .electron-aaos-install.log'));
  lines.push('');
...
function formatRollbackMessage(success, failedFiles = []) {
  ...
  if (success) {
    ...
    lines.push(chalk.cyan('  1. Check backup directory: .electron-aaos-backup/'));
    lines.push(chalk.cyan('  2. Manually restore failed files from backups'));
    lines.push(chalk.cyan('  3. Review installation log: .electron-aaos-install.log'));
  }
...
function formatSuccessMessage() {
  ...
  lines.push(chalk.green('═══════════════════════════════════════════════════════'));
  lines.push('');
  lines.push(chalk.white('Electron AAOS has been installed and configured.'));
  lines.push('');
  lines.push(chalk.cyan.bold('Next Steps:'));
  lines.push(chalk.cyan('  1. Review installation log: .electron-aaos-install.log'));
  lines.push(chalk.cyan('  2. Verify configuration: electron-aaos config --check'));
  lines.push(chalk.cyan('  3. Run validation: electron-aaos validate'));
  lines.push('');
  return lines.join('\n');
}
