/**
 * Electron AAOS-FullStack Environment Configuration Module
...
async function generateCoreConfigYAML(projectPath, wizardState) {
  try {
    const electron-aaosCoreDir = path.join(projectPath, '.electron-aaos-core');
    const yamlPath = path.join(electron-aaosCoreDir, 'core-config.yaml');

    // Ensure .electron-aaos-core directory exists
    await fs.ensureDir(electron-aaosCoreDir);
