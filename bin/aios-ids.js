...
/**
 * Electron AAOS IDS CLI — Incremental Development System
 * Standalone (no external dependencies for npx compatibility)
 *
 * Commands:
 *   electron-aaos ids:query {intent}     — Query registry for matching artifacts
 *   electron-aaos ids:create-review      — Review CREATE decisions for promotion/deprecation
 *   electron-aaos ids:health             — Run registry health check (self-healing)
 *   electron-aaos ids:check {intent}     — Pre-check registry (FrameworkGovernor advisory)
 *   electron-aaos ids:impact {entity-id} — Impact analysis for modifications
 *   electron-aaos ids:stats              — Registry statistics
 *   electron-aaos ids:register {path}    — Register entity after creation
 *
 * Flags:
...
const { RegistryLoader } = require(path.resolve(__dirname, '..', '.electron-aaos-core', 'core', 'ids', 'registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve(__dirname, '..', '.electron-aaos-core', 'core', 'ids', 'incremental-decision-engine'));
const { RegistryUpdater } = require(path.resolve(__dirname, '..', '.electron-aaos-core', 'core', 'ids', 'registry-updater'));
const { FrameworkGovernor } = require(path.resolve(__dirname, '..', '.electron-aaos-core', 'core', 'ids', 'framework-governor'));

// Optional: RegistryHealer (IDS-4a — may not exist yet)
let RegistryHealer = null;
try {
  RegistryHealer = require(path.resolve(__dirname, '..', '.electron-aaos-core', 'core', 'ids', 'registry-healer')).RegistryHealer;
} catch (_err) {
...
function showHelp() {
  console.log(`
Electron AAOS IDS — Incremental Development System
...
Examples:
  electron-aaos ids:query "validate story drafts"
  electron-aaos ids:query "template rendering engine" --json
  electron-aaos ids:query "database migration" --type script
  electron-aaos ids:create-review
  electron-aaos ids:create-review --json
  electron-aaos ids:health
  electron-aaos ids:health --fix
  electron-aaos ids:check "validate yaml schema" --type task
  electron-aaos ids:impact create-doc
  electron-aaos ids:stats
  electron-aaos ids:stats --json
  electron-aaos ids:register .electron-aaos-core/development/tasks/my-new-task.md
`);
}
...
function runQuery() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: electron-aaos ids:query "your intent here"');
    process.exit(1);
  }
...
async function runCheck() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: electron-aaos ids:check "your intent here"');
    process.exit(1);
  }
...
async function runImpact() {
  ...
  if (!entityId) {
    console.error('Error: Entity ID is required. Usage: electron-aaos ids:impact {entity-id}');
    process.exit(1);
  }
...
async function runRegister() {
  ...
  if (!filePath) {
    console.error('Error: File path is required. Usage: electron-aaos ids:register {file-path}');
    process.exit(1);
  }
