/**
 * Electron AAOS Installer - Main installation logic
 *
 * Orchestrates the complete Electron AAOS installation flow:
 * 1. OS detection
...
/**
 * Check if a config-resolver is available (electron-aaos-core installed)
 * @param {string} projectRoot - Project root
 * @returns {Object|null} Config resolver module or null
 */
function tryLoadConfigResolver(projectRoot) {
  const possiblePaths = [
    path.join(projectRoot, '.electron-aaos-core/core/config/config-resolver.js'),
    path.join(projectRoot, 'node_modules/electron-aaos-core/.electron-aaos-core/core/config/config-resolver.js'),
  ];
...
async function createUserConfigDirect(profile, logger, dryRun) {
  const userConfigDir = path.join(os.homedir(), '.electron-aaos');
...
/**
 * Check if this is a brownfield installation (existing Electron AAOS)
 * @param {string} projectRoot - Project root
...
function detectBrownfield(projectRoot) {
  ...
  const legacyConfigPath = path.join(projectRoot, '.electron-aaos-core/core-config.yaml');
  ...
  const frameworkConfigPath = path.join(projectRoot, '.electron-aaos-core/framework-config.yaml');
  if (fs.existsSync(frameworkConfigPath)) {
    result.isBrownfield = true;
    result.hasLayeredConfig = true;
  }

  // Check for .electron-aaos-core directory
  const electron-aaosCoreDir = path.join(projectRoot, '.electron-aaos-core');
...
/**
 * Run the Electron AAOS doctor command
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function runDoctor(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx electron-aaos-core doctor');
    return;
  }

  const spinner = ora('Running Electron AAOS doctor...').start();

  try {
    const { stdout } = await execa('npx', ['electron-aaos-core', 'doctor'], {
      cwd: projectRoot,
      timeout: 60000,
    });

    spinner.succeed('Electron AAOS doctor completed');

    if (logger.verbose) {
      console.log(chalk.dim(stdout));
    }
  } catch (error) {
    spinner.warn('Electron AAOS doctor had warnings');
    if (logger.verbose) {
...
/**
 * Install electron-aaos-core package
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function installAiosCore(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npm install electron-aaos-core --save-dev');
    return;
  }

  const spinner = ora('Installing electron-aaos-core...').start();

  try {
    await execa('npm', ['install', 'electron-aaos-core', '--save-dev'], {
      cwd: projectRoot,
      timeout: 300000, // 5 minutes
    });

    spinner.succeed('electron-aaos-core installed');
  } catch (error) {
    spinner.fail('Failed to install electron-aaos-core');
    throw error;
  }
}

/**
 * Initialize Electron AAOS in the project
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function initializeAios(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx electron-aaos-core install');
    return;
  }

  const spinner = ora('Initializing Electron AAOS...').start();

  try {
    await execa('npx', ['electron-aaos-core', 'install'], {
      cwd: projectRoot,
      timeout: 120000, // 2 minutes
    });

    spinner.succeed('Electron AAOS initialized');
  } catch (error) {
    spinner.fail('Failed to initialize Electron AAOS');
    throw error;
  }
}
...
async function runInstaller(options = {}) {
  ...
  // Introduction
  intro(chalk.bgCyan(' Electron AAOS Installer '));
  ...
  if (!profile) {
    ...
    const profileSelection = await select({
      message: 'Select your Electron AAOS profile:',
      options: [
        {
          value: 'bob',
  ...
  if (brownfield.isBrownfield) {
    logger.info('Existing Electron AAOS installation detected');
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        try {
          await execa('npx', ['electron-aaos-core', 'config', 'migrate'], {
            cwd: projectRoot,
            timeout: 60000,
          });
        ...
        logger.action('Run: npx electron-aaos-core config migrate');
      }
    } else {
  ...
  if (options.dryRun) {
    ...
    outro(chalk.green(`Electron AAOS installed successfully in ${elapsed}!`));
    console.log('');
    console.log(chalk.dim('Next steps:'));
    console.log(chalk.dim('  1. Run `npx electron-aaos-core info` to see your configuration'));
