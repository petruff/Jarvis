...
describe('Environment Configuration Integration', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `electron-aaos-env-test-${testId}`);
  ...
  describe('configureEnvironment', () => {
    it('should create .env file with skip prompts', async () => {
      ...
      // Version-agnostic: check Electron AAOS_VERSION exists with valid semver format
      expect(content).toMatch(/Electron AAOS_VERSION=\d+\.\d+\.\d+/);
    });
    ...
    it('should create core-config.yaml', async () => {
      ...
      const configPath = path.join(testDir, '.electron-aaos-core', 'core-config.yaml');
  ...
  describe('File Structure', () => {
    ...
    it('should create .electron-aaos-core directory if missing', async () => {
      const electron-aaoscoreDir = path.join(testDir, '.electron-aaos-core');
      expect(await fs.pathExists(electron-aaoscoreDir)).toBe(false);
      ...
      expect(await fs.pathExists(electron-aaoscoreDir)).toBe(true);
    });
  });
});
