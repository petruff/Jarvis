...
describe('Wizard Integration with Project Type Detection', () => {
  ...
  describe('GREENFIELD Detection Flow', () => {
    test('wizard correctly detects and processes GREENFIELD project', async () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        // No markers exist
        if (checkPath.includes('.electron-aaos-core')) return false;
  ...
  describe('BROWNFIELD Detection Flow', () => {
    test('wizard correctly detects and processes BROWNFIELD project with package.json', async () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === targetPath) return true;
        if (checkPath.includes('.electron-aaos-core')) return false; // No Electron AAOS
  ...
  // ============================================================================
  // Task 1.3.5.4: Test full wizard flow with EXISTING_Electron AAOS detection
  // ============================================================================
  describe('EXISTING_Electron AAOS Detection Flow', () => {
    test('wizard correctly detects and processes EXISTING_Electron AAOS installation', async () => {
      // Setup: Directory with .electron-aaos-core
      fs.existsSync.mockImplementation((checkPath) => {
      ...
      fs.readdirSync.mockReturnValue(['.electron-aaos-core', 'package.json', '.git']);

      const result = await runWizard({ targetDir: '/test-existing' });

      expect(result.projectType).toBe('EXISTING_Electron AAOS');
      expect(result.targetDir).toBe('/test-existing');
      // Output shows "brownfield" since EXISTING_Electron AAOS is treated as brownfield update
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('brownfield'));
    });

    test('confirmProjectType shows update/reinstall message for EXISTING_Electron AAOS', async () => {
      const confirmed = await confirmProjectType('EXISTING_Electron AAOS');

      expect(confirmed).toBe('EXISTING_Electron AAOS');
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('update or reinstall'),
      );
    });

    test('getProjectType helper returns EXISTING_Electron AAOS when .electron-aaos-core exists', () => {
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath.includes('existing')) return true;
        if (checkPath.endsWith('.electron-aaos-core')) return true;
        return false;
      });
      fs.readdirSync.mockReturnValue(['.electron-aaos-core']);

      const type = getProjectType('/test/existing');

      expect(type).toBe('EXISTING_Electron AAOS');
    });
  });
  ...
  describe('User Override Functionality', () => {
    ...
    test('confirmProjectType shows appropriate message for each type', async () => {
      const types = ['GREENFIELD', 'BROWNFIELD', 'EXISTING_Electron AAOS', 'UNKNOWN'];
  ...
  describe('Default Wizard Behavior', () => {
    ...
    test('wizard displays welcome message', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Welcome to Electron AAOS Installer'),
      );
    });
