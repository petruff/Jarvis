...
describe('detectProjectType', () => {
  ...
  describe('AC #1: GREENFIELD detection', () => {
    test('detects GREENFIELD when directory is empty', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .electron-aaos-core, package.json, or .git
      });
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('.electron-aaos-core'));
  ...
  describe('AC #2: BROWNFIELD detection - package.json', () => {
    test('detects BROWNFIELD when package.json exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .electron-aaos-core or .git
      });
    ...
    test('detects BROWNFIELD when only package.json exists (no git)', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.electron-aaos-core')) return false;
        return false;
      });
  ...
  describe('AC #3: BROWNFIELD detection - .git', () => {
    test('detects BROWNFIELD when .git exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .electron-aaos-core or package.json
      });
  ...
  // ============================================================================
  // AC #4: .electron-aaos-core Exists → EXISTING_Electron AAOS
  // ============================================================================
  describe('AC #4: EXISTING_Electron AAOS detection', () => {
    test('detects EXISTING_Electron AAOS when .electron-aaos-core exists', () => {
      // Setup
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === path.resolve('/test/existing')) return true;
        if (checkPath.endsWith('.electron-aaos-core')) return true; // .electron-aaos-core exists
        return true; // Other files may exist too
      });
      fs.readdirSync.mockReturnValue(['.electron-aaos-core', 'package.json', '.git', 'src']);
      
      const result = detectProjectType('/test/existing');
      
      expect(result).toBe('EXISTING_Electron AAOS');
    });

    test('EXISTING_Electron AAOS takes priority over BROWNFIELD markers', () => {
      // Setup: All markers present, but EXISTING_Electron AAOS should win
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === path.resolve('/test/priority-test')) return true;
        // All markers present
        if (checkPath.endsWith('.electron-aaos-core')) return true;
        if (checkPath.endsWith('package.json')) return true;
        if (checkPath.endsWith('.git')) return true;
        return false;
      });
      ...
      // EXISTING_Electron AAOS has highest priority
      expect(result).toBe('EXISTING_Electron AAOS');
    });
  });
  ...
  describe('Edge Case: UNKNOWN detection', () => {
    test('returns UNKNOWN when directory has files but no recognized markers', () => {
      // Setup: Files exist but no package.json, .git, or .electron-aaos-core
      fs.existsSync.mockImplementation((checkPath) => {
  ...
  describe('Error Handling', () => {
    ...
    test('throws error when directory cannot be accessed (EACCES)', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.electron-aaos-core')) {
  ...
  describe('Detection Priority Order', () => {
    test('priority: EXISTING_Electron AAOS > GREENFIELD', () => {
      // Even if directory appears empty, .electron-aaos-core presence should win
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === path.resolve('/test/priority1')) return true;
        if (checkPath.endsWith('.electron-aaos-core')) return true;
        return false;
      });
      fs.readdirSync.mockReturnValue(['.electron-aaos-core']); // Only .electron-aaos-core
