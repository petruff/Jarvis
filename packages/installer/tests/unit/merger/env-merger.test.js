...
describe('EnvMerger', () => {
  ...
  describe('merge', () => {
    ...
    it('should preserve existing values over new values', async () => {
      ...
      // The suggested value appears in Electron AAOS_SUGGESTED comment
      expect(result.content).toContain('Electron AAOS_SUGGESTED: API_KEY=placeholder_key');
      expect(result.stats.preserved).toBe(1);
    });

    it('should add Electron AAOS_SUGGESTED comment for differing values', async () => {
      ...
      expect(result.content).toContain('# Electron AAOS_SUGGESTED: PORT=8080');
...
DB_PORT=5432`;
    ...
    it('should add new variables in Electron AAOS Variables section', async () => {
      const existing = 'EXISTING=value';
      const newContent = `Electron AAOS_VAR1=value1
Electron AAOS_VAR2=value2`;

      const result = await merger.merge(existing, newContent);

      // Header includes date: "# === Electron AAOS Variables (added YYYY-MM-DD) ==="
      expect(result.content).toMatch(/# === Electron AAOS Variables \(added \d{4}-\d{2}-\d{2}\) ===/);
      expect(result.content).toContain('Electron AAOS_VAR1=value1');
      expect(result.content).toContain('Electron AAOS_VAR2=value2');
    });
