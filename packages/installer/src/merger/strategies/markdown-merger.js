/**
 * @fileoverview Merge strategy for Markdown files with Electron AAOS-MANAGED sections
...
const { parseMarkdownSections, hasAiosMarkers } = require('../parsers/markdown-section-parser.js');

/**
 * Merge strategy for Markdown files (CLAUDE.md, rules.md).
 * Uses Electron AAOS-MANAGED markers to identify sections that can be updated.
...
class MarkdownMerger extends BaseMerger {
  ...
  async merge(existingContent, newContent, options = {}) {
    ...
    // If existing file has no Electron AAOS markers, do legacy migration
    if (!existing.hasAiosMarkers) {
      return this.migrateLegacy(existingContent, template, options);
    }

    const stats = createEmptyStats();
    const changes = [];
    const processedAiosSections = new Set();
    ...
    for (const section of existing.sections) {
      if (section.managed) {
        // Electron AAOS-managed section - update with template content if available
        ...
        if (templateSection) {
          ...
          merged += `<!-- Electron AAOS-MANAGED-START: ${section.id} -->\n`;
          merged += templateSection.lines.join('\n');
          if (!merged.endsWith('\n')) {
            merged += '\n';
          }
          merged += `<!-- Electron AAOS-MANAGED-END: ${section.id} -->\n\n`;
          processedAiosSections.add(section.id);
          changes.push({
            type: 'updated',
    ...
    // Add new Electron AAOS sections that don't exist in current file
    ...
    if (newSections.length > 0) {
      merged += '---\n\n';
      merged += '<!-- New Electron AAOS sections added -->\n\n';
  ...
  /**
   * Migrate a legacy file (without Electron AAOS markers) by appending Electron AAOS sections
  ...
  async migrateLegacy(existingContent, template, _options = {}) {
    ...
    // Get all Electron AAOS-managed sections from template
    const electron-aaosSections = template.sections.filter(s => s.managed);
    ...
    // Add separator and Electron AAOS sections
    if (electron-aaosSections.length > 0) {
      merged += '\n---\n\n';
      merged += '<!-- Electron AAOS-MANAGED SECTIONS -->\n';
      merged += '<!-- These sections are managed by Electron AAOS. Edit content between markers carefully. -->\n';
      merged += '<!-- Your custom content above will be preserved during updates. -->\n\n';

      for (const section of electron-aaosSections) {
        merged += `<!-- Electron AAOS-MANAGED-START: ${section.id} -->\n`;
        merged += section.lines.join('\n');
        if (!merged.endsWith('\n')) {
          merged += '\n';
   ...
   */
  getDescription() {
    return 'Merges Markdown files using Electron AAOS-MANAGED section markers';
  }
}
