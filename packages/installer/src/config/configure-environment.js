...
async function configureEnvironment(options = {}) {
  ...
  try {
    ...
    // Step 2: API keys are configured later via .env or electron-aaos-master
    // Skipping prompts during installation for better UX
    const apiKeys = {};

    if (!skipPrompts) {
      console.log('\n💡 API keys can be configured later in .env file or via electron-aaos-master');
    }
    ...
    const coreConfigDir = path.join(targetDir, '.electron-aaos-core');
    ...
    console.log('✅ Created .electron-aaos-core/core-config.yaml');
...
/**
 * Update .gitignore to include critical entries for Electron AAOS projects
 *
...
async function updateGitignore(targetDir) {
  ...
  const criticalEntries = {
    ...
    'Electron AAOS Local': ['.electron-aaos-core/local/', '.claude/settings.local.json', '.electron-aaos/install-log.txt'],
  };
