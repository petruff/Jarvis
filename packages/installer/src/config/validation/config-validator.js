...
function validateCoreConfigStructure(config) {
  ...
  if (config.project) {
    if (!config.project.type) {
      ...
      const validTypes = ['GREENFIELD', 'BROWNFIELD', 'EXISTING_Electron AAOS', 'greenfield', 'brownfield', 'existing_electron-aaos'];
      if (!validTypes.includes(config.project.type)) {
        errors.push(`Invalid project.type: ${config.project.type}. Expected greenfield, brownfield, or existing_electron-aaos`);
      }
    }
  }
