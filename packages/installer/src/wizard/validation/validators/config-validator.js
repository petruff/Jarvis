 ...
 * @private
 */
async function validateCoreConfig(results, configPath = '.electron-aaos-core/core-config.yaml') {
...
async function validateGitignore(results) {
  ...
  try {
    ...
    const recommendedEntries = ['.env.local', '*.key', '*.pem', '.electron-aaos/install-log.txt'];
