 ...
 * @param {string} options.projectType - 'BROWNFIELD' | 'GREENFIELD' | 'EXISTING_Electron AAOS'
...
async function promptFileExists(filePath, options = {}) {
  ...
  const isBrownfield = projectType === 'BROWNFIELD' || projectType === 'EXISTING_Electron AAOS';
...
function generateTemplateVariables(wizardState) {
  ...
  return {
    ...
    timestamp,
    electron-aaosVersion: '2.1.0', // From package.json in real implementation
  };
}

/**
 * Copy agent files from .electron-aaos-core/development/agents to IDE-specific agent folder
...
async function copyAgentFiles(projectRoot, agentFolder, ideConfig = null) {
  ...
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.electron-aaos-core', 'development', 'agents');
...
async function createAntiGravityConfigJson(projectRoot, ideConfig) {
  ...
  const config = {
    ...
    paths: {
      ...
      tasks: '.electron-aaos-core/tasks',
      workflows: '.electron-aaos-core/workflows',
    },
  };
  ...
  try {
    for (const ideKey of selectedIDEs) {
      ...
      try {
        ...
        // Load template from .electron-aaos-core/product/templates/
        const templatePath = path.join(__dirname, '..', '..', '..', '..', '.electron-aaos-core', 'product', 'templates', ide.template);
...
function showSuccessSummary(result) {
  ...
  console.log('  2. The IDE should automatically recognize Electron AAOS configuration');
