/**
 * Electron AAOS Interactive Wizard - Main Entry Point
 *
...
const {
  installAiosCore,
  hasPackageJson,
} = require('../installer/electron-aaos-core-installer');
const {
...
} = require('../../../../.electron-aaos-core/infrastructure/scripts/llm-routing/install-llm-routing');
...
async function getExistingUserProfile(targetDir = process.cwd()) {
  const coreConfigPath = path.join(targetDir, '.electron-aaos-core', 'core-config.yaml');
...
async function runWizard(options = {}) {
  try {
    ...
    // Show welcome message with Electron AAOS branding
    if (!options.quiet) {
      showWelcome();
    }
    ...
    // Story 1.4: Install Electron AAOS core framework (agents, tasks, workflows, templates)
    console.log('\n📦 Installing Electron AAOS core framework...');
    let electron-aaosCoreResult = null;
    try {
      electron-aaosCoreResult = await installAiosCore({
      ...
      if (electron-aaosCoreResult.success) {
        console.log(`✅ Electron AAOS core installed (${electron-aaosCoreResult.installedFolders.length} folders)`);
        console.log(
          `   - Agents: ${electron-aaosCoreResult.installedFolders.includes('agents') ? '✓' : '⨉'}`,
        );
        console.log(`   - Tasks: ${electron-aaosCoreResult.installedFolders.includes('tasks') ? '✓' : '⨉'}`);
        console.log(
          `   - Workflows: ${electron-aaosCoreResult.installedFolders.includes('workflows') ? '✓' : '⨉'}`,
        );
        console.log(
          `   - Templates: ${electron-aaosCoreResult.installedFolders.includes('templates') ? '✓' : '⨉'}`,
        );
      }
      answers.electron-aaosCoreInstalled = true;
      answers.electron-aaosCoreResult = electron-aaosCoreResult;
    } catch (error) {
      console.error('\n⚠️  Electron AAOS core installation failed:', error.message);
      answers.electron-aaosCoreInstalled = false;
    }
    ...
    if (answers.selectedTechPreset && answers.selectedTechPreset !== 'none') {
      ...
      try {
        ...
        const possiblePresetDirs = [
          path.join(__dirname, '..', '..', '.electron-aaos-core', 'data', 'tech-presets'),
          path.join(process.cwd(), '.electron-aaos-core', 'data', 'tech-presets'),
        ];
        ...
        if (sourcePresetDir) {
          ...
          if (fse.existsSync(presetFile)) {
            // Copy preset to project's .electron-aaos-core/data/tech-presets/
            const targetPresetDir = path.join(process.cwd(), '.electron-aaos-core', 'data', 'tech-presets');
            ...
            console.log(
              `   📁 Location: .electron-aaos-core/data/tech-presets/${answers.selectedTechPreset}.md`,
            );
    ...
    //     path.join(__dirname, '..', '..', '..', 'expansion-packs'),
    //     path.join(process.cwd(), 'node_modules', '@electron-aaos/electron-aaos-core', 'expansion-packs'),
    //   ];
    //
    ...
    try {
      ...
      if (envResult.envCreated && envResult.coreConfigCreated) {
        ...
        console.log('  - .electron-aaos-core/core-config.yaml created');
    ...
    //       console.log('\n💡 Check .electron-aaos/install-errors.log for details');
    //     }
    //
    ...
    try {
      const validation = await validateInstallation(
        {
          files: {
            ...
            env: '.env',
            coreConfig: '.electron-aaos-core/core-config.yaml',
            mcpConfig: '.mcp.json',
          },
          configs: {
      ...
      console.log('Installation may be incomplete. Check logs in .electron-aaos/ directory.');
    }
