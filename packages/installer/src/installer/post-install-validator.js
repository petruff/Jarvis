 ...
 * Comprehensive validation of Electron AAOS-Core installation with security hardening
 */
class PostInstallValidator {
  /**
   * Create a new PostInstallValidator instance
   *
   * @param {string} targetDir - Directory where Electron AAOS was installed (project root)
  ...
  constructor(targetDir, sourceDir = null, options = {}) {
    ...
    this.electron-aaosCoreTarget = path.join(this.targetDir, '.electron-aaos-core');
    this.electron-aaosCoreSource = this.sourceDir ? path.join(this.sourceDir, '.electron-aaos-core') : null;
  ...
  async loadManifest() {
    ...
    const sourceManifestPath = this.electron-aaosCoreSource
      ? path.join(this.electron-aaosCoreSource, 'install-manifest.yaml')
      : null;
    const targetManifestPath = path.join(this.electron-aaosCoreTarget, 'install-manifest.yaml');
  ...
  async validateFile(entry) {
    ...
    const absolutePath = path.resolve(this.electron-aaosCoreTarget, relativePath);
    ...
    if (!isPathContained(absolutePath, this.electron-aaosCoreTarget)) {
      ...
      if (error.code === 'ENOENT') {
        result.issue = {
          ...
          remediation: this.sourceDir
            ? "Run 'electron-aaos validate --repair' to restore"
            : 'Re-run installation',
          relativePath,
        };
    ...
    // allowing path traversal attacks (e.g., /install/.electron-aaos-core/symlinked-dir/../../../etc/passwd)
    //
    ...
    try {
      ...
      const realTargetDir = fs.realpathSync(this.electron-aaosCoreTarget);
      ...
      const expectedRelative = path.relative(this.electron-aaosCoreTarget, absolutePath);
  ...
  async detectExtraFiles() {
    ...
    await scanDir(this.electron-aaosCoreTarget, this.electron-aaosCoreTarget);
    return extraFiles;
  }
  ...
  async validate() {
    ...
    if (!fs.existsSync(this.electron-aaosCoreTarget)) {
      this.issues.push({
        type: IssueType.MISSING_FILE,
        severity: Severity.CRITICAL,
        message: 'Electron AAOS-Core directory not found',
        details: `Expected at: ${this.electron-aaosCoreTarget}`,
        remediation: 'Run `npx electron-aaos-core install`',
        relativePath: null,
      });
  ...
  generateRecommendations() {
    ...
    if (this.stats.missingFiles > 0) {
      if (this.stats.missingFiles > 50) {
        ...
        recommendations.push(
          `${this.stats.missingFiles} file(s) missing. Run 'electron-aaos validate --repair'.`,
        );
      }
    }
    ...
    if (this.stats.corruptedFiles > 0) {
      recommendations.push(
        `${this.stats.corruptedFiles} file(s) corrupted. Run 'electron-aaos validate --repair'.`,
      );
    }
  ...
  async repair(options = {}) {
    ...
    if (!this.sourceDir || !fs.existsSync(this.electron-aaosCoreSource)) {
      return {
        success: false,
    ...
    for (let i = 0; i < repairableIssues.length; i++) {
      ...
      const sourcePath = path.resolve(this.electron-aaosCoreSource, relativePath);
      const targetPath = path.resolve(this.electron-aaosCoreTarget, relativePath);

      onProgress(i + 1, repairableIssues.length, relativePath);

      // SECURITY: Path containment for source
      if (!isPathContained(sourcePath, this.electron-aaosCoreSource)) {
        result.skipped.push({ path: relativePath, reason: 'Source path traversal blocked' });
        continue;
      }

      // SECURITY: Path containment for target
      if (!isPathContained(targetPath, this.electron-aaosCoreTarget)) {
      ...
      try {
        ...
        // Walk each path component from electron-aaosCoreTarget to targetDir
        // and verify none are symlinks
        let currentPath = this.electron-aaosCoreTarget;
        const relativeParts = path.relative(this.electron-aaosCoreTarget, targetDir).split(path.sep);
        ...
        for (const part of relativeParts) {
          ...
          try {
            ...
            if (componentStat.isSymbolicLink()) {
              result.skipped.push({
                path: relativePath,
                reason: `Symlink detected in path component: ${path.relative(this.electron-aaosCoreTarget, currentPath)}`,
              });
              continue;
            }
          } catch (_statError) {
        ...
        // Final realpath verification: ensure resolved target stays within resolved electron-aaosCoreTarget
        ...
        if (fs.existsSync(targetDir)) {
          ...
          const realAiosCoreTarget = fs.realpathSync(this.electron-aaosCoreTarget);
...
function formatReport(report, options = {}) {
  ...
  lines.push('');
  lines.push(`${c.bold}Electron AAOS-Core Installation Validation Report${c.reset}`);
