/**
 * Brownfield Upgrader
 * Handles incremental upgrades for existing Electron AAOS-Core installations
 *
...
function loadInstalledManifest(targetDir) {
  return loadManifest(
    path.join(targetDir, '.electron-aaos-core'),
 ...
 * @param {string} sourceDir - Source package directory (.electron-aaos-core from npm)
...
function generateUpgradeReport(sourceManifest, installedManifest, targetDir) {
  ...
  for (const [filePath, sourceEntry] of sourceMap) {
    ...
    const absolutePath = path.join(electron-aaosCoreDir, filePath);
 ...
 * @param {string} sourceDir - Source directory (npm package .electron-aaos-core)
 * @param {string} targetDir - Target project directory
...
async function applyUpgrade(report, sourceDir, targetDir, options = {}) {
  ...
  // Ensure .electron-aaos-core directory exists
  if (!dryRun) {
  ...
  for (const file of report.newFiles) {
    ...
    const targetPath = path.join(electron-aaosCoreDir, file.path);
...
function updateInstalledManifest(targetDir, sourceManifest, sourcePackage) {
  const installedManifestPath = path.join(targetDir, '.electron-aaos-core', '.installed-manifest.yaml');
  ...
  const header = `# Electron AAOS-Core Installed Manifest
...
function formatUpgradeReport(report) {
  ...
  lines.push('Electron AAOS-Core Upgrade Report');
  lines.push('═'.repeat(60));
  lines.push('');
