/**
 * Electron AAOS Updater
 * Intelligent update system for Electron AAOS-Core installations
 *
...
/**
 * Electron AAOS Updater Class
 * Handles intelligent updates while preserving user customizations
 */
class Electron AAOSUpdater {
  /**
   * Create a new Electron AAOSUpdater instance
   *
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.electron-aaosCoreDir = path.join(this.projectRoot, '.electron-aaos-core');
    this.electron-aaosConfigDir = path.join(this.projectRoot, '.electron-aaos');
  ...
  async checkForUpdates() {
    ...
    try {
      ...
      if (!result.installed) {
        result.error = 'Electron AAOS not installed or version info not found';
        return result;
      }
      ...
      if (!result.latest) {
        ...
        if (!isOnline) {
          ...
          result.error = 'Package @electron-aaos/electron-aaos-core not found on npm registry. This may be a local development installation.';
        }
        return result;
      }
  ...
  async getInstalledVersion() {
    ...
    const versionJsonPath = path.join(this.electron-aaosCoreDir, 'version.json');
    ...
    const packageJsonPath = path.join(this.projectRoot, 'node_modules', '@electron-aaos', 'electron-aaos-core', 'package.json');
    ...
    if (fs.existsSync(localPackageJsonPath)) {
      try {
        ...
        if (pkg.name === '@electron-aaos/electron-aaos-core' || pkg.name === 'electron-aaos-core') {
  ...
  async getLatestVersion() {
    return new Promise((resolve) => {
      const request = https.get(
        'https://registry.npmjs.org/@electron-aaos/electron-aaos-core/latest',
  ...
  async detectCustomizations() {
    ...
    for (const [relativePath, originalHash] of Object.entries(this.versionInfo.fileHashes)) {
      const absolutePath = path.join(this.electron-aaosCoreDir, relativePath);
   ...
   */
  async createBackup() {
    this.backupDir = path.join(this.electron-aaosConfigDir, 'backup', `pre-update-${Date.now()}`);
    ...
    for (const file of filesToBackup) {
      const src = path.join(this.electron-aaosCoreDir, file);
  ...
  async rollback() {
    ...
    for (const file of backupFiles) {
      ...
      const dest = path.join(this.electron-aaosCoreDir, file);
  ...
  async applyUpdate(targetVersion, _customizedFiles = []) {
    ...
    try {
      ...
      const cmd = `npm install @electron-aaos/electron-aaos-core@${targetVersion} --save-exact`;
      ...
      // TODO: Copy new files from node_modules to .electron-aaos-core
  ...
  log(message) {
    if (this.options.verbose) {
      console.log(`[Electron AAOSUpdater] ${message}`);
    }
  }
}
...
function formatCheckResult(result, options = {}) {
  ...
  lines.push('');
  lines.push(`${c.bold}🔍 Electron AAOS Update Check${c.reset}`);
  lines.push('');
  ...
  switch (result.status) {
    ...
    case UpdateStatus.UPDATE_AVAILABLE:
      ...
      lines.push(`  Run ${c.cyan}npx electron-aaos-core update${c.reset} to update.`);
      break;
...
function formatUpdateResult(result, options = {}) {
  ...
  if (result.success) {
    ...
    lines.push('');
    lines.push(`Run ${c.cyan}npx electron-aaos-core validate${c.reset} to verify installation.`);
  } else {
